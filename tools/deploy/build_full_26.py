...
def get_bundle_info_plist(bundle_path):
    candidates = [
        os.path.join(bundle_path, "Info.plist"),
        os.path.join(bundle_path, "Contents", "Info.plist"),
...
def resolve_bundle_executable(bundle_path):
    info_plist = get_bundle_info_plist(bundle_path)
    if not info_plist:
    ...
    if not executable_name:
        error(f"CFBundleExecutable missing in {info_plist}")

    direct_candidate = os.path.join(bundle_path, executable_name)
    if os.path.exists(direct_candidate):
        return direct_candidate

    versions_dir = os.path.join(bundle_path, "Versions")
    if os.path.isdir(versions_dir):
        for version_name in sorted(os.listdir(versions_dir)):
            version_candidate = os.path.join(versions_dir, version_name, executable_name)
    ...
    error(f"Could not resolve executable '{executable_name}' in bundle: {bundle_path}")
...
def merge_url_schemes(existing_url_types, bundle_name, schemes):
    merged = list(existing_url_types or [])
    target_entry = None
    for entry in merged:
        if isinstance(entry, dict) and entry.get("CFBundleURLName") == bundle_name:
            target_entry = entry
            break
    ...
    if not target_entry:
        target_entry = {
            ...
            "CFBundleURLName": bundle_name,
            "CFBundleURLSchemes": [],
        }
        merged.append(target_entry)

    target_entry["CFBundleURLSchemes"] = merge_unique_strings(
        target_entry.get("CFBundleURLSchemes", []),
        schemes,
    )
    return merged

def validate_codesign(path):
    try:
        ...
        error(f"Failed to inspect code signature for {path}: {e}")

    if result.returncode != 0 or "code object is not signed" in (result.stderr or ""):
        error(f"Unsigned or invalid code signature for {path}: {result.stderr.strip()}")
...
def build_persistence_helper():
    log("Building Persistence Helper (ECHelper) using build_echelper.py...")

    # Define build script paths
    script_path = os.path.join(PROJECT_ROOT, "tools/deploy/build_echelper.py")
    if not os.path.exists(script_path):
        script_path = os.path.join(PROJECT_ROOT, "build_echelper.py")
    ...
    # Find the output bin
    built_bin = os.path.join(PROJECT_ROOT, "build_antigravity/IPA/echelper")
    if not os.path.exists(built_bin):
        error(f"Could not find compiled ECHelper binary at {built_bin}")

    # Copy to build root
    output_bin = os.path.join(BUILD_ROOT, "PersistenceHelper")
    shutil.copy(built_bin, output_bin)

    # Ad-hoc sign + CT bypass if entitlements exists
    log("Signing Persistence Helper (Post-Process)...")
    entitlements_path = os.path.join(PROJECT_ROOT, "echelper/entitlements.plist")
    ...
    log(f"Persistence Helper successfully integrated at: {output_bin}")
    return output_bin
    shutil.copy(built_bin, output_bin)
...
def build_ui_app():
    ...
    # [VERSION FIX] Clean derived data and build dirs to prevent stale builds
    derived_data = os.path.join(BUILD_ROOT, "DerivedData")
    build_dir = os.path.join(BUILD_ROOT, "build")
    for d in [derived_data, build_dir]:
        if os.path.exists(d):
            log(f"Cleaning heavily: {d}")
            shutil.rmtree(d)
    ...
    info_data["TSRootHelperPath"] = "echelper"
    info_data["TSAppGroup"] = "group.com.ecmain.shared"
    info_data["CFBundleURLTypes"] = merge_url_schemes(
        info_data.get("CFBundleURLTypes", []),
        "com.ecmain.app",
        ["ecmain-app", "apple-magnifier"],
    )
    info_data["UIBackgroundModes"] = merge_unique_strings(
        info_data.get("UIBackgroundModes", []),
        ["audio", "location", "voip", "fetch", "remote-notification", "processing"],
    )
...
def sign_binary(path, entitlements_path=None):
    ...
    try:
        ...
        run_cmd(f"codesign -f -s - '{path}'", ignore_error=True)

    fastpathsign = os.path.join(BUILD_ROOT, "Source/Exploits/fastPathSign/fastPathSign")
    if os.path.exists(fastpathsign):
        target_bin = path
        if os.path.isdir(path):
            target_bin = resolve_bundle_executable(path)

        run_cmd(f"'{fastpathsign}' '{target_bin}'")
        validate_codesign(target_bin)
    else:
        print("[-] Warning: fastPathSign not found (skipped CT bypass).")
        if os.path.isfile(path):
            validate_codesign(path)

def validate_staging_bundle(final_app):
    log("Validating staged bundle signatures...")

    required_paths = [
        os.path.join(final_app, "ECMAIN"),
        os.path.join(final_app, "echelper"),
    ]

    optional_paths = [
        os.path.join(final_app, "PersistenceHelper"),
        os.path.join(final_app, "go-ios-arm64"),
        os.path.join(final_app, "tls_proxy_arm64"),
        os.path.join(final_app, "spoof_plugin.dat"),
    ]

    tunnel_appex = os.path.join(final_app, "PlugIns", "Tunnel.appex")
    if os.path.exists(tunnel_appex):
        required_paths.append(resolve_bundle_executable(tunnel_appex))

    for path in required_paths:
        if not os.path.exists(path):
            error(f"Missing launch-critical binary: {path}")
        validate_codesign(path)
...
def package_all(app_path, helper_path, dylib_path=None, persistence_helper_path=None):
    ...
    if os.path.exists(ldid_src):
        ...
        shutil.copy(ldid_src, ldid_in_app)
        os.chmod(ldid_in_app, 0o755)
    ...
    # [v1803] Inject core DDI into App Bundle Root for quick self-mount
    log("Injecting core DDI into App Bundle Root (v1803)...")
    ddi_candidates = [
        os.path.join(PROJECT_ROOT, "15.8", "DeveloperDiskImage.dmg"),
        os.path.join(PROJECT_ROOT, "DeveloperDiskImage.dmg"),
    ]
    ddi_dmg = None
    for candidate in ddi_candidates:
        if os.path.exists(candidate):
            ddi_dmg = candidate
            break
    if ddi_dmg:
        ddi_sig = ddi_dmg + ".signature"
        ...
        log(f"  Core DDI injected successfully from {ddi_dmg}")
    else:
        log("WARNING: DeveloperDiskImage.dmg not found! Self-mount might fail.")

    # Sign launch-critical nested code before main app
    # Mihomo.framework was removed above (static library, not embeddable)

    go_ios_binary = os.path.join(final_app, "go-ios-arm64")
    if os.path.exists(go_ios_binary):
        log("Signing go-ios-arm64...")
        sign_binary(go_ios_binary, helper_entitlements if os.path.exists(helper_entitlements) else None)
    ...
    if dest_ph and os.path.exists(dest_ph):
        sign_binary(dest_ph, helper_entitlements if os.path.exists(helper_entitlements) else None)

    sign_binary(dest_helper, helper_entitlements if os.path.exists(helper_entitlements) else None)

    if ldid_in_app and os.path.exists(ldid_in_app):
    ...
    if os.path.exists(tunnel_appex):
        ...
        sign_binary(tunnel_appex, tunnel_entitlements if os.path.exists(tunnel_entitlements) else None)

    # Sign Main App last
    # NOTE: codesign on a .app directory will RECURSIVELY re-sign all inner binaries,
    # which STRIPS the CoreTrust bypass we applied above. We only use codesign here
    # to generate the CodeResources seal. fastPathSign is applied to ALL binaries below.
    log("Signing main app bundle (recursive codesign)...")
    cmd = f"codesign -f -s - --entitlements '{entitlements}' '{final_app}'"
    try:
        run_cmd(cmd)
    except:
        run_cmd(f"codesign -f -s - '{final_app}'", ignore_error=True)

    # === CRITICAL: RE-APPLY CoreTrust BYPASS TO ALL EXECUTABLES ===
    # codesign on the .app bundle recursively replaced all inner signatures,
    # stripping the CoreTrust bypass. We must now re-inject it into EVERY executable.
    fastpathsign = os.path.join(BUILD_ROOT, "Source/Exploits/fastPathSign/fastPathSign")
    if os.path.exists(fastpathsign):
        binaries_to_bypass = [
            os.path.join(final_app, "ECMAIN"),
            os.path.join(final_app, "PlugIns/Tunnel.appex/Tunnel"),
            os.path.join(final_app, "echelper"),
            os.path.join(final_app, "ldid"),
            os.path.join(final_app, "PersistenceHelper"),
            os.path.join(final_app, "go-ios-arm64"),
            os.path.join(final_app, "tls_proxy_arm64"),
            os.path.join(final_app, "spoof_plugin.dat"),
        ]
        for bin_path in binaries_to_bypass:
            if os.path.exists(bin_path):
                log(f"Re-applying CoreTrust bypass to {os.path.basename(bin_path)}...")
                run_cmd(f"'{fastpathsign}' '{bin_path}'", ignore_error=True)
    else:
        log("WARNING: fastPathSign not found! CoreTrust bypass NOT applied!")

    validate_staging_bundle(final_app)

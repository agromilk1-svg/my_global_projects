...
def fix_ios15_missing_frameworks(app_path):
    ...
    When Xcode 16 builds WDA, it embeds some frameworks (XCTest, XCTestCore, etc.)
    but NOT all of their transitive dependencies. Missing dependencies cause
    DYLD crashes on launch. We find and embed all missing ones.
    """
    ...
    # Frameworks/dylibs that SHOULD be embedded but Xcode misses
    MISSING_DEPS = [
        # (source_path_relative_to_platform, dest_name, is_framework)
        ("Library/Frameworks/_Testing_Foundation.framework", "_Testing_Foundation.framework", True),
        ("Library/Frameworks/Testing.framework", "Testing.framework", True),
        ("Library/PrivateFrameworks/XCTestSupport.framework", "XCTestSupport.framework", True),
    ]
    
    # Also check for dylibs
    MISSING_DYLIBS = [
    ...
    for src_rel, dest_name, is_fw in MISSING_DEPS:
        src = os.path.join(PLATFORM_DEV, src_rel)
        dest = os.path.join(frameworks_dir, dest_name)
        if not os.path.exists(dest) and os.path.exists(src):
            shutil.copytree(src, dest)
            print(f"  ✅ 已嵌入缺失框架: {dest_name}")
            count += 1
        elif os.path.exists(dest):
            print(f"  ℹ️  已存在: {dest_name}")
    
    # Embed missing dylibs
    for src_rel in MISSING_DYLIBS:
        src = os.path.join(PLATFORM_DEV, src_rel)
        dylib_name = os.path.basename(src_rel)
        dest = os.path.join(frameworks_dir, dylib_name)
        if not os.path.exists(dest) and os.path.exists(src):
            shutil.copy2(src, dest)
            print(f"  ✅ 已嵌入缺失动态库: {dylib_name}")
            count += 1
        elif os.path.exists(dest):
            print(f"  ℹ️  已存在: {dylib_name}")
    
    if count == 0:
        print("  ℹ️  所有依赖已完整")
    else:
        print(f"  ✅ 共嵌入 {count} 个缺失依赖")
...
def build_project():
    ...
    build_cmd = (
        ...
        f"OTHER_LDFLAGS='$(inherited) -Wl,-no_fixup_chains' "
    )
...
def package_ipa(app_path):
    ...
    # [iOS 15 FIX] Remove frameworks that don't exist on iOS 15
    # Xcode 16 adds Testing.framework and XCTestSupport.framework which iOS 15 lacks.
    # dyld crashes with SIGABRT if it can't find a required framework.
    print("--- [iOS 15 FIX] 移除 iOS 15 不存在的框架 ---")
    fix_ios15_missing_frameworks(dest_app)

...
static NSUInteger const DefaultStartingPort = 10088;
static NSUInteger const DefaultMjpegServerPort = 10089;
...
+ (void)enableXcTestDebugLogs {
  ((XCTestConfiguration *)XCTestConfiguration.activeTestConfiguration)
      .emitOSLogs = YES;
...
+ (NSRange)bindingPortRange {
  ...
  // Existence of USE_PORT in the environment implies the port range is managed
  ...
  if (NSProcessInfo.processInfo.environment[@"USE_PORT"] &&
    ...
    return NSMakeRange(
        [NSProcessInfo.processInfo.environment[@"USE_PORT"] integerValue], 1);
  }
...
+ (NSString *)bindingIPAddress {
  // Existence of USE_IP in the environment allows specifying which interface to
  // bind to
...
+ (NSInteger)mjpegServerPort {
  ...
  if (NSProcessInfo.processInfo.environment[@"MJPEG_SERVER_PORT"] &&
      [NSProcessInfo.processInfo.environment[@"MJPEG_SERVER_PORT"] length] >
          0) {
    return [NSProcessInfo.processInfo.environment[@"MJPEG_SERVER_PORT"]
        integerValue];
  }
...
+ (void)configureDefaultKeyboardPreferences {
  ...
  TIPreferencesController *controller =
      [controllerClass sharedPreferencesController];
  // Auto-Correction in Keyboards
  // 'setAutocorrectionEnabled' Was in TextInput.framework/TIKeyboardState.h
  // over iOS 10.3
  if ([controller respondsToSelector:@selector(setAutocorrectionEnabled:)]) {
    // Under iOS 10.2
    controller.autocorrectionEnabled = NO;
  } else if ([controller respondsToSelector:@selector(setValue:
                                                forPreferenceKey:)]) {
    // Over iOS 10.3
  ...
  if ([controller respondsToSelector:@selector(setValue:forPreferenceKey:)]) {
    [controller setValue:@YES
        forPreferenceKey:@"DidShowGestureKeyboardIntroduction"];
    if (isSDKVersionGreaterThanOrEqualTo(@"13.0")) {
      [controller setValue:@YES
          forPreferenceKey:@"DidShowContinuousPathIntroduction"];
    }
...
+ (void)setKeyboardAutocorrection:(BOOL)isEnabled {
  [self configureKeyboardsPreference:isEnabled
...
+ (int)snapshotMaxDepth {
  return
      [FBGetCustomParameterForElementSnapshot(FBSnapshotMaxDepthKey) intValue];
}
...
+ (BOOL)setScreenshotOrientation:(NSString *)orientation
  ...
  if ([orientation.lowercaseString isEqualToString:@"portrait"]) {
  ...
  } else if ([orientation.lowercaseString
                 isEqualToString:@"portraitupsidedown"]) {
    ...
    return [[FBErrorBuilder.builder
        withDescriptionFormat:
            @"The orientation value '%@' is not known. Only the following "
            @"orientation values are supported: "
             "'auto', 'portrait', 'portraitUpsideDown', 'landscapeRight' and "
             "'landscapeLeft'",
            orientation] buildError:error];
  }
  return YES;
}
...
+ (void)resetSessionSettings {
  ...
  // snapshot to be taken (it always errors with kxIllegalArgument error)
  ...
  // 50 should be enough for the majority of the cases. The performance is
  // acceptable for values up to 100.
...
+ (FBConfigurationKeyboardPreference)keyboardsPreference:
  ...
  if ([key isEqualToString:FBKeyboardAutocorrectionKey]) {
    if ([controller respondsToSelector:@selector(boolForPreferenceKey:)]) {
      ...
      [FBLogger
          log:@"Updating keyboard autocorrection preference is not supported"];
    ...
    if ([controller respondsToSelector:@selector(boolForPreferenceKey:)]) {
      ...
      [FBLogger
          log:@"Updating keyboard prediction preference is not supported"];
      return FBConfigurationKeyboardPreferenceNotSupported;
    }
  }
  @throw [[FBErrorBuilder.builder
      withDescriptionFormat:@"No available keyboardsPreferenceKey: '%@'", key]
      build];
}

+ (void)configureKeyboardsPreference:(BOOL)enable
                    forPreferenceKey:(nonnull NSString *)key {
  void *handle = dlopen(controllerPrefBundlePath, RTLD_LAZY);
  Class controllerClass = NSClassFromString(controllerClassName);
...
+ (NSString *)valueFromArguments:(NSArray<NSString *> *)arguments
                          forKey:(NSString *)key {
...
+ (NSUInteger)mjpegServerPortFromArguments {
  NSString *portNumberString =
      [self valueFromArguments:NSProcessInfo.processInfo.arguments
...
+ (void)setReduceMotionEnabled:(BOOL)isEnabled {
  ...
  //  2019-08-21 22:58:19.776165+0900 WebDriverAgentRunner-Runner[322:13361]
  //  [User Defaults] Couldn't write value for key ReduceMotionEnabled in
  //  CFPrefsPlistSource<0x28111a700> (Domain: com.apple.Accessibility, User:
  //  kCFPreferencesCurrentUser, ByHost: No, Container: (null), Contents Need
  //  Refresh: No): setting preferences outside an application's container

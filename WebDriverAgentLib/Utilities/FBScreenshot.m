...
+ (NSData *)takeWithScreenID:(long long)screenID
  ...
  NSData *screenshotData =
      [self.class takeInOriginalResolutionWithScreenID:screenID
...
+ (NSData *)takeInOriginalResolutionWithScreenID:(long long)screenID
  ...
  id screnshotRequest =
      [self.class screenshotRequestWithScreenID:screenID
                                           rect:CGRectNull
                                            uti:uti
                             compressionQuality:compressionQuality
                                          error:error];
  ...
  if (0 != dispatch_semaphore_wait(
               sem, dispatch_time(DISPATCH_TIME_NOW, timeoutNs))) {
    NSString *timeoutMsg = [NSString
        stringWithFormat:@"Cannot take a screenshot within %@ timeout",
                         formatTimeInterval(SCREENSHOT_TIMEOUT)];
    if (nil == error) {
  ...
  if ([uti conformsToType:UTTypeHEIC]) {
    ...
    dispatch_once(&onceToken, ^{
      ...
      NSMethodSignature *signature =
          [imageEncodingClass methodSignatureForSelector:selector];
      if (nil != signature) {
    ...
    if (!isHeicSuppported) {
      [FBLogger logFmt:@"The device under test does not support HEIC image "
                       @"encoding. Falling back to PNG"];
      uti = UTTypePNG;
    }
  }

  id imageEncodingAllocated = [imageEncodingClass alloc];
  SEL imageEncodingConstructorSelector = NSSelectorFromString(
      @"initWithUniformTypeIdentifier:compressionQuality:");
  if (![imageEncodingAllocated
          respondsToSelector:imageEncodingConstructorSelector]) {
    [[[FBErrorBuilder builder]
        withDescription:@"'initWithUniformTypeIdentifier:compressionQuality:' "
                        @"contructor is not found on XCTImageEncoding class"]
        buildError:error];
    return nil;
  }
  NSMethodSignature *imageEncodingContructorSignature = [imageEncodingAllocated
      methodSignatureForSelector:imageEncodingConstructorSelector];
  NSInvocation *imageEncodingInitInvocation = [NSInvocation
      invocationWithMethodSignature:imageEncodingContructorSignature];
...
+ (nullable id)screenshotRequestWithScreenID:(long long)screenID
                          ...
                          compressionQuality:(CGFloat)compressionQuality
                                       error:(NSError **)error {
  id imageEncoding =
      [self.class imageEncodingWithUniformTypeIdentifier:uti
                                      compressionQuality:compressionQuality
                                                   error:error];
  ...
  SEL screenshotRequestConstructorSelector =
      NSSelectorFromString(@"initWithScreenID:rect:encoding:");
  if (![screenshotRequestAllocated
          respondsToSelector:screenshotRequestConstructorSelector]) {
    [[[FBErrorBuilder builder]
        withDescription:@"'initWithScreenID:rect:encoding:' contructor is not "
                        @"found on XCTScreenshotRequest class"]
        buildError:error];
    return nil;
  }
  NSMethodSignature *screenshotRequestContructorSignature =
      [screenshotRequestAllocated
          methodSignatureForSelector:screenshotRequestConstructorSelector];
  NSInvocation *screenshotRequestInitInvocation = [NSInvocation
      invocationWithMethodSignature:screenshotRequestContructorSignature];
  [screenshotRequestInitInvocation
      setSelector:screenshotRequestConstructorSelector];

...
actionLibrary.push(
  ...
  { label: '[🎯 找元素] wda.findElement', type: 'FIND_ELEMENT', desc: '通过原生的 XCTest 谓词或 class chain 极速搜索元素，比全量 XML 树扫描 /source 快得多，找到后仅返回其坐标尺寸信息。', usage: '只需要获取元素位置或者判断元素是否存在，后续想决定用随机概率坐标点击时使用', params: '★ wda.findElement(predicate)\n  参数: [字符串] NSPredicate 或 class chain\n  返回: [对象] {found: true/false, x, y, width, height, element_id}\n\n═══ 谓词语法参考 (predicate) ═══\n精确标签匹配:    label == "关注"\n模糊匹配:        label CONTAINS[c] "Profil"\n按属性与类型:    type == "XCUIElementTypeButton" AND name == "a11y_vo_profile"\n\n═══ class chain 路径查询 (最快) ═══\n以 **/ 开头，直接定位树分支，绕过全局搜索：\n**/XCUIElementTypeTabBar/XCUIElementTypeButton[5]', example: '// ══════════════════════════════════════\n// 🎯 找元素 (findElement) 用法大全\n// ══════════════════════════════════════\n\n// ━━━ 用法1: 严格精确组合查找 ━━━\n// 注意：在 JS 模板里要用 \\" 转义内部双引号\nvar el = wda.findElement("type == \\"XCUIElementTypeButton\\" AND name == \\"a11y_vo_profile\\"");\nif (el && el.found) {\n    wda.log("✅ 找到元素，位于 (" + el.x + ", " + el.y + ")，开始降级随机点击...");\n    // 拿到坐标后，为了躲避风控，在元素的尺寸范围内随机点一下\n    wda.tap(el.x + wda.randomInt(-el.width/3, el.width/3), \n            el.y + wda.randomInt(-el.height/3, el.height/3));\n} else {\n    wda.log("❌ 没发现指定元素!");\n}\n\n// ━━━ 用法2: class chain 极速查询 ━━━\n// 第三个 TabBar 的按钮\nvar tab = wda.findElement("**/XCUIElementTypeTabBar/XCUIElementTypeButton[3]");\nif(tab && tab.found) wda.tap(tab.x, tab.y);' },
  { label: '[🎯 找并点击] wda.tapElement', type: 'TAP_ELEMENT', desc: '结合了【查】与【点】的一体化接口。通过底层 WDA 传入查找条件，搜到直接触发底层点击 click，无需返回坐标通过前端中转。', usage: '高频操作中最精简的一步到位动作', params: '★ wda.tapElement(predicate)\n  参数: [字符串] NSPredicate 或 class chain\n  返回: [布尔值] true / false', example: '// ══════════════════════════════════════\n// 🎯 点击元素 (tapElement) 用法\n// ══════════════════════════════════════\n\n// ⚠️ 返回的直接是 Boolean 结果 (true/false)！！\n// 因为动作交由底层做完了，你只需要知道它有没有点成功即可。\n\n// ━━━ 常规点击 ━━━\nvar ok = wda.tapElement("label == \\"关注\\"");\nif (ok) {\n    wda.log("✅ 原生一键点击成功");\n} else {\n    wda.log("❌ 并未找到该【关注】按钮");\n}\n\n// ━━━ 结合等待的多重判定 ━━━\nfor(var i=0; i<3; i++) {\n    if (wda.tapElement("label ==[c] \\"profile\\"")) {\n       wda.log("点进了个人主页！");\n       break;\n    }\n    wda.sleep(1.0);\n}' },
  { label: '[🎯 获取元素数值] wda.getElementText', type: 'GET_ELEMENT_TEXT', desc: '针对某个特定控件提取其包含的文本或数值。内部包含智能回退机制，按顺序尝试获取元素的 label、value、name 属性。', usage: '获取点赞数、评论数、粉丝量或是读取输入框内文本。', params: '★ wda.getElementText(predicate)\n  参数: [字符串] 目标元素的匹配条件(NSPredicate 或是 class chain)\n  返回: [字符串] 匹配到的文字内容，若没找到或文本为空则返回空字符串 ""\n\n★ wda.getElementAttribute(predicate, 属性名)\n  参数: 要去查找的元素条件、你想读取的明确属性(比如 "value" 或 "label" 等)\n  返回: [字符串] 真实的属性值', example: '// ══════════════════════════════════════\n// 🎯 获取元素内容 (getElementText / getElementAttribute)\n// ══════════════════════════════════════\n\n// ━━━ 用法1: 智能获取文字 (如获取系统点赞数) ━━━\n// 假设你要去读取包含点赞数的那个UI组件\nvar likes = wda.getElementText("name CONTAINS \\"likes\\"");\nif (likes !== "") {\n    wda.log("✅ 抓取到点赞数了：" + likes);\n    // 要做判断可以转为数字: parseInt(likes)\n} else {\n    wda.log("❌ 既没找到该元素，或者它是空的");\n}\n\n// ━━━ 用法2: 严格读取指定属性 (适用于隐藏传值等场景) ━━━\nvar attrVal = wda.getElementAttribute("type == \\"XCUIElementTypeSwitch\\"", "value");\nwda.log("当前的开关状态值为：" + attrVal);' },

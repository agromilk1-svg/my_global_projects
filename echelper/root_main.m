...
extern void
IOHIDEventSystemClientDispatchEvent(IOHIDEventSystemClientRef client,
                                    IOHIDEventRef event);
extern IOHIDEventRef IOHIDEventCreateDigitizerEvent(
    CFAllocatorRef allocator, uint64_t timeStamp, uint32_t transType,
    uint32_t touchIndex, uint32_t x, uint32_t y, uint32_t z, uint32_t pressure,
    uint32_t twist, Boolean range, Boolean touch, uint32_t options);

// Parameters
#define kIOHIDTransTypeHand 1
#define kIOHIDDigitizerTransTypeEx 1 // Hand/Finger
...
// typedef void *IOSurfaceRef; // Defined in IOSurface/IOSurfaceRef.h
...
// --- IOHID Implementation ---
int simulateTap(int x, int y) {
  void *ioKit =
      dlopen("/System/Library/Frameworks/IOKit.framework/IOKit", RTLD_NOW);
  if (!ioKit) {
    NSLog(@"[RootHelper] Error: Failed to open IOKit");
    return 20;
  }

  IOHIDEventSystemClientRef (*IOHIDEventSystemClientCreate)(CFAllocatorRef) =
      dlsym(ioKit, "IOHIDEventSystemClientCreate");
  void (*IOHIDEventSystemClientDispatchEvent)(IOHIDEventSystemClientRef,
                                              IOHIDEventRef) =
  ...
  IOHIDEventRef (*IOHIDEventCreateDigitizerEvent)(
      ...
      uint32_t, uint32_t, uint32_t, Boolean, Boolean, uint32_t) =
      dlsym(ioKit, "IOHIDEventCreateDigitizerEvent");

  if (!IOHIDEventSystemClientCreate || !IOHIDEventSystemClientDispatchEvent ||
      !IOHIDEventCreateDigitizerEvent) {
    NSLog(@"[RootHelper] Error: Failed to resolve IOHID symbols");
    dlclose(ioKit);
    return 21;
  }
  ...
  if (!client) {
    NSLog(@"[RootHelper] Error: Failed to create HID client");
    dlclose(ioKit);
    return 22;
  }
  ...
  IOHIDEventRef touchDown = IOHIDEventCreateDigitizerEvent(
      kCFAllocatorDefault, time, kIOHIDTransTypeHand,
      1, // Index
      x, y, 0, 0, 0,
      0, // range
      1, // touch (true)
  ...
  if (touchDown) {
    IOHIDEventSystemClientDispatchEvent(client, touchDown);
    CFRelease(touchDown);
  }
  ...
  IOHIDEventRef touchUp = IOHIDEventCreateDigitizerEvent(
      ...
      kIOHIDEventOptionNone);
  if (touchUp) {
    IOHIDEventSystemClientDispatchEvent(client, touchUp);
    CFRelease(touchUp);
  }

  CFRelease(client);
  dlclose(ioKit);
  NSLog(@"[RootHelper] Tapped at %d, %d", x, y);
  return 0;
}
...
int captureScreenshot(NSString *outputPath) {
  ...
  IOMobileFramebufferGetLayerDefaultSurfaceFunc getLayerSurface =
      (IOMobileFramebufferGetLayerDefaultSurfaceFunc)dlsym(
          handle, "IOMobileFramebufferGetLayerDefaultSurface");

  if (!getMainDisplay || !getLayerSurface) {
    NSLog(@"[RootHelper] Error: Failed to resolve IOMFB symbols");
    dlclose(handle);
    return 11;
  }
  ...
  if (!iosHandle) {
    NSLog(@"[RootHelper] Error: Failed to open IOSurface.framework");
    dlclose(handle);
    return 12;
  }
  ...
  if (getMainDisplay(&connection) != 0 || !connection) {
    NSLog(@"[RootHelper] Error: GetMainDisplay failed");
    dlclose(iosHandle);
    dlclose(handle);
    return 13;
  }
  ...
  void *surface = NULL;
  if (getLayerSurface(connection, 0, &surface) != 0 || !surface) {
    // Try surface 1
    getLayerSurface(connection, 1, &surface);
  }

  if (!surface) {
    NSLog(@"[RootHelper] Error: Failed to get IOSurface");
    dlclose(iosHandle);
    dlclose(handle);
    return 14;
  }

  lockSurface(surface, 0x00000001, NULL); // Read Only
  ...
  if (baseAddr) {
    ...
    if (context) {
      ...
      if (imageRef) {
        ...
        jpegData = UIImageJPEGRepresentation(uiImage, 0.7); // 0.7 Quality
      }
  ...
  if (!jpegData) {
    NSLog(@"[RootHelper] Error: Failed to create JPEG data");
    if (imageRef)
  ...
  if (saved) {
    // Ensure readable by mobile/standard users (644)
    chmod(outputPath.UTF8String, 0644);
...
int signApp(NSString *appPath) { return -1; }
#else
...
int signApp(NSString *appPath) {
  ...
  while (fileURL = [enumerator nextObject]) {
    ...
    if ([filePath.lastPathComponent isEqualToString:@"Info.plist"]) {
      ...
      // Force platform-application for main executable to prevent crash
      if (isSameFile(bundleMainExecutablePath, mainExecutablePath)) {
        entitlementsToUse[@"platform-application"] = @YES;
        entitlementsToUse[@"com.apple.private.security.container-required"] =
            @NO;
      }
...
int installApp(NSString *appPackagePath, BOOL sign, BOOL force, BOOL isTSUpdate,
  ...
  if (appContainer) {
    ...
    // Make sure the installed app is a TrollStore app or the container is empty
    ...
    if (useInstalldMethod) {
      ...
      // temporary copy just for using it on this API once Yeah this sucks, but
      // there is no better solution unfortunately
      ...
      if (!systemMethodSuccessful) {
        NSLog(@"[installApp] encountered error %@ while trying to do "
              @"placeholder install",
              installError);
      }
  ...
  // At this point the (new version of the) app is installed but still needs to
  // be registered Also permissions need to be fixed
  ...
  // Handle developer mode after installing and registering the app, to ensure
  // that we don't arm developer mode but then fail to install the app
  if (requiresDevMode) {
...
int uninstallTrollStore(BOOL unregister) {
  @try {
    NSString *trollStoreData = trollStorePath();
    NSLog(@"[Uninstall] Starting uninstallTrollStore.");

    BOOL oneFailed = NO;

    // 1. Remove App Bundle (The actual App)
    LSApplicationProxy *appProxy =
        [LSApplicationProxy applicationProxyForIdentifier:APP_ID];

    if (appProxy && appProxy.bundleURL) {
      NSLog(@"[Uninstall] Found Bundle URL: %@", appProxy.bundleURL.path);
      NSError *err = nil;
      if ([[NSFileManager defaultManager] removeItemAtURL:appProxy.bundleURL
                                                    error:&err]) {
        NSLog(@"[Uninstall] Successfully removed app bundle.");
      } else {
        NSLog(@"[Uninstall] Failed to remove app bundle: %@", err);
        oneFailed = YES;
      }
    } else {
      NSLog(@"[Uninstall] App Proxy or Bundle URL missing. Skipping bundle "
            @"removal.");
    }

    // 2. Remove Data Container
    if (trollStoreData &&
        [[NSFileManager defaultManager] fileExistsAtPath:trollStoreData]) {
      NSLog(@"[Uninstall] Removing Data Container.");
      NSError *err = nil;
      if ([[NSFileManager defaultManager] removeItemAtPath:trollStoreData
                                                     error:&err]) {
        NSLog(@"[Uninstall] Successfully removed data container.");
      } else {
        NSLog(@"[Uninstall] Failed to remove data container: %@", err);
        oneFailed = YES;
      }
    } else {
      NSLog(@"[Uninstall] Data container not found.");
    }
    ...
    NSLog(@"[Uninstall] CRITICAL EXCEPTION: %@", exception);
    return -999;
  }
}
...
int installTrollStore(NSString *pathToTar) {
  ...
  if (![[NSFileManager defaultManager] fileExistsAtPath:pathToTar]) {
    fprintf(stderr, "Error: Tar file not found at path: %s\n",
            pathToTar.UTF8String);
    return 101;
  }
  if (![pathToTar.pathExtension isEqualToString:@"tar"]) {
    fprintf(stderr, "Error: File is not a tar: %s\n", pathToTar.UTF8String);
    return 102;
  }
      ...
      [[NSFileManager defaultManager] createDirectoryAtPath:tmpPayloadPath
                                withIntermediateDirectories:YES
                                                 attributes:nil
                                                      error:nil];
  if (!suc) {
    fprintf(stderr, "Error: Failed to create temp directory at: %s\n",
            tmpPayloadPath.UTF8String);
    return 103;
  }
  ...
  if (extractRet != 0) {
    [[NSFileManager defaultManager] removeItemAtPath:tmpPackagePath error:nil];
    fprintf(stderr, "Error: extract() failed with code %d\n", extractRet);
    return 169;
  }
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:tmpECMainPath] &&
    ...
    if (moveError) {
      fprintf(stderr,
              "Error: Failed to rename ECMAIN.app to TrollStore.app: %s\n",
              moveError.localizedDescription.UTF8String);
      return 106;
    }
    fprintf(stderr, "Success: Renamed ECMAIN.app to TrollStore.app\n");
  }

  // Fix: Handle ECInstall.app (rename to TrollStore.app)
  NSString *tmpECInstallPath =
      [tmpPayloadPath stringByAppendingPathComponent:@"ECInstall.app"];
  if ([[NSFileManager defaultManager] fileExistsAtPath:tmpECInstallPath] &&
      ![[NSFileManager defaultManager] fileExistsAtPath:tmpTrollStorePath]) {
    NSError *moveError = nil;
    [[NSFileManager defaultManager] moveItemAtPath:tmpECInstallPath
                                            toPath:tmpTrollStorePath
                                             error:&moveError];
    if (moveError) {
      fprintf(stderr,
              "Error: Failed to rename ECInstall.app to TrollStore.app: %s\n",
              moveError.localizedDescription.UTF8String);
      return 107;
    }
    fprintf(stderr, "Success: Renamed ECInstall.app to TrollStore.app\n");
  }

  if (![[NSFileManager defaultManager] fileExistsAtPath:tmpTrollStorePath]) {
    fprintf(stderr, "Error: TrollStore.app not found in tar at: %s\n",
            tmpTrollStorePath.UTF8String);

    // List contents to debug
    NSError *error = nil;
    NSArray *contents =
        [[NSFileManager defaultManager] contentsOfDirectoryAtPath:tmpPayloadPath
                                                            error:&error];
    if (contents) {
      fprintf(stderr, "Contents of %s:\n", tmpPayloadPath.UTF8String);
      for (NSString *file in contents) {
        fprintf(stderr, " - %s\n", file.UTF8String);
        // Also check subdirectories if it's Payload inside Payload
        if ([file isEqualToString:@"Payload"]) {
          NSString *subPath =
              [tmpPayloadPath stringByAppendingPathComponent:file];
          NSArray *subContents =
              [[NSFileManager defaultManager] contentsOfDirectoryAtPath:subPath
                                                                  error:nil];
          for (NSString *subFile in subContents) {
  ...
  if (![[NSFileManager defaultManager]
          fileExistsAtPath:toInstallInfoPlistPath]) {
    fprintf(stderr, "Error: Info.plist not found at: %s\n",
            toInstallInfoPlistPath.UTF8String);
    return 105;
  }
...
void registerUserPersistenceHelper(NSString *userAppId) {
  // If a persistence helper is already registered, uninstall/unregister it
  // first
  if (findPersistenceHelperApp(PERSISTENCE_HELPER_TYPE_ALL)) {
...
// Ported from ECAppInjector.m
int inject_load_command(NSString *binaryPath, NSString *dylibPath) {
  if (!binaryPath || !dylibPath)
    return -1;

  // Check if binaryPath exists
  if (![[NSFileManager defaultManager] fileExistsAtPath:binaryPath]) {
    NSLog(@"[Helper] Binary not found: %@", binaryPath);
    return 10;
  }
  ...
  if (!fileHandle) {
    NSLog(@"[Helper] Failed to open file for writing: %@", binaryPath);
    return 10;
  }
  ...
  if (header->magic != MH_MAGIC_64) {
    [fileHandle closeFile];
    NSLog(@"[Helper] Not a valid Mach-O 64-bit binary");
    return 11;
  }
  ...
  if (lcData.length < header->sizeofcmds) {
    [fileHandle closeFile];
    NSLog(@"[Helper] Failed to read load commands");
    return 12;
  }
  ...
  for (uint32_t i = 0; i < header->ncmds; i++) {
    ...
    if (lc->cmd == LC_ENCRYPTION_INFO_64) {
      ...
      if (enc->cryptid != 0) {
        [fileHandle closeFile];
        NSLog(@"[Helper] Binary is encrypted (cryptid=%d)", enc->cryptid);
        return 20;
      }
    }
    lcOffset += lc->cmdsize;
  ...
  if (availableSpace < cmdSize) {
    [fileHandle closeFile];
    NSLog(@"[Helper] Not enough space for load command");
    return 13;
  }
...
int MAIN_NAME(int argc, char *argv[], char *envp[]) {
  @autoreleasepool {
    ...
    if ([cmd isEqualToString:@"install"]) {
      ...
      if ([[NSFileManager defaultManager] moveItemAtPath:src
                                                  toPath:dst
                                                   error:nil]) {
        ret = 0;
      } else {
      ...
      if ([[NSFileManager defaultManager] removeItemAtPath:path error:nil]) {
        ...
        // If file doesn't exist, we consider it a success (it's gone)
    ...
    } else if ([cmd isEqualToString:@"chown-file"]) {
      if (args.count < 4)
        return -3;
      uid_t uid = (uid_t)atoi(((NSString *)args[1]).UTF8String);
      gid_t gid = (gid_t)atoi(((NSString *)args[2]).UTF8String);
      ...
      NSString *dylibPath = args[2];
      ret = inject_load_command(binaryPath, dylibPath);
    } else if ([cmd isEqualToString:@"chmod-file"]) {
      if (args.count < 3)
        return -3;
      NSString *perms = args[1];
      NSString *path = args[2];
      long mode = strtol(perms.UTF8String, NULL, 8);
      if (chmod(path.UTF8String, (mode_t)mode) == 0) {
        ret = 0;
      } else {
        ret = 1;
      }
      ...
      if (entitlements) {
        NSString *teamId = entitlements[@"com.apple.developer.team-identifier"];
        if (teamId) {
          printf("%s", teamId.UTF8String);
          ret = 0;
        } else {
          ret = 1;
        }

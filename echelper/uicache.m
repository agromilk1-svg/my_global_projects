...
NSDictionary *
constructGroupsContainersForEntitlements(NSDictionary *entitlements,
                                         BOOL systemGroups) {
  if (!entitlements)
    return nil;
  ...
  if (groupIDs && [groupIDs isKindOfClass:[NSArray class]]) {
    ...
    for (NSString *groupID in groupIDs) {
      MCMContainer *container = [mcmClass containerWithIdentifier:groupID
  ...
  if (containerRequired && [containerRequired isKindOfClass:[NSNumber class]]) {
    if (!((NSNumber *)containerRequired).boolValue) {
      return NO;
             ...
             [containerRequired isKindOfClass:[NSString class]]) {
...
NSDictionary *
constructEnvironmentVariablesForContainerPath(NSString *containerPath,
                                              BOOL isContainerized) {
  NSString *homeDir = isContainerized ? containerPath : @"/var/mobile";
  NSString *tmpDir = isContainerized
                         ? [containerPath stringByAppendingPathComponent:@"tmp"]
                         : @"/var/tmp";
  return
      @{@"CFFIXED_USER_HOME" : homeDir, @"HOME" : homeDir, @"TMPDIR" : tmpDir};
}
...
bool registerPath(NSString *path, BOOL unregister, BOOL forceSystem) {
  ...
  if (unregister && ![[NSFileManager defaultManager] fileExistsAtPath:path]) {
    LSApplicationProxy *app =
        [LSApplicationProxy applicationProxyForIdentifier:path];
    if (app.bundleURL) {
  ...
  if ([immutableAppBundleIdentifiers()
          containsObject:appBundleID.lowercaseString])
    return false;

  if (appBundleID && !unregister) {
    NSString *appExecutablePath = [path
        stringByAppendingPathComponent:appInfoPlist[@"CFBundleExecutable"]];
    NSDictionary *entitlements =
        dumpEntitlementsFromBinaryAtPath(appExecutablePath);

    NSString *appDataContainerID = appBundleID;
    BOOL appContainerized = constructContainerizationForEntitlements(
        entitlements, &appDataContainerID);

    MCMContainer *appDataContainer = [NSClassFromString(@"MCMAppDataContainer")
    ...
    BOOL isRemovableSystemApp = [[NSFileManager defaultManager]
        fileExistsAtPath:[@"/System/Library/AppSignatures"
                             stringByAppendingPathComponent:appBundleID]];
    // Antigravity Fix: Check both /var/containers and /private/var/containers
    // paths
    BOOL isInUserContainer = [path hasPrefix:@"/var/containers"] ||
                             [path hasPrefix:@"/private/var/containers"];
    BOOL registerAsUser =
        isInUserContainer && !isRemovableSystemApp && !forceSystem;
    ...
    if (containerPath) {
      ...
      dictToRegister[@"EnvironmentVariables"] =
          constructEnvironmentVariablesForContainerPath(containerPath,
                                                        appContainerized);
    }
    ...
    NSString *teamIdentifier =
        constructTeamIdentifierForEntitlements(entitlements);
    if (teamIdentifier)
    ...
    NSDictionary *appGroupContainers =
        constructGroupsContainersForEntitlements(entitlements, NO);
    NSDictionary *systemGroupContainers =
        constructGroupsContainersForEntitlements(entitlements, YES);
    NSMutableDictionary *groupContainers = [NSMutableDictionary new];
    ...
    for (NSString *pluginName in plugins) {
      ...
      NSDictionary *pluginInfoPlist = [NSDictionary
          dictionaryWithContentsOfFile:
              [pluginPath stringByAppendingPathComponent:@"Info.plist"]];
      NSString *pluginBundleID =
          [pluginInfoPlist objectForKey:@"CFBundleIdentifier"];

      if (!pluginBundleID)
        continue;
      NSString *pluginExecutablePath =
          [pluginPath stringByAppendingPathComponent:
                          pluginInfoPlist[@"CFBundleExecutable"]];
      NSDictionary *pluginEntitlements =
          dumpEntitlementsFromBinaryAtPath(pluginExecutablePath);
      NSString *pluginDataContainerID = pluginBundleID;
      BOOL pluginContainerized = constructContainerizationForEntitlements(
          pluginEntitlements, &pluginDataContainerID);
      ...
      if (pluginContainerPath) {
        ...
        pluginDict[@"EnvironmentVariables"] =
            constructEnvironmentVariablesForContainerPath(pluginContainerPath,
                                                          pluginContainerized);
      }
      ...
      NSString *pluginTeamIdentifier =
          constructTeamIdentifierForEntitlements(pluginEntitlements);
      ...
      NSDictionary *pluginAppGroupContainers =
          constructGroupsContainersForEntitlements(pluginEntitlements, NO);
      NSDictionary *pluginSystemGroupContainers =
          constructGroupsContainersForEntitlements(pluginEntitlements, YES);
      ...
      [pluginGroupContainers
          addEntriesFromDictionary:pluginSystemGroupContainers];
    ...
    if (![workspace registerApplicationDictionary:dictToRegister]) {
      ...
      [dictToRegister enumerateKeysAndObjectsUsingBlock:^(

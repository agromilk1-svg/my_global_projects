...
def cli(ctx) -> None:
    ...
    if (
        ...
        click.secho(f"{os_name}{device_version} ({device_build}) 不支持。", fg="red")
    ...
    if not app_path:
        click.secho(f"无法找到可移除的系统应用 '{app}'！", fg="red")
        click.secho(f"请确认应用名称输入正确，并且系统应用 '{app}' 已安装在设备上。", fg="red")
        return
    elif Path("/private/var/containers/Bundle/Application") not in app_path.parents:
        click.secho(f"'{app}' 不是可移除的系统应用！", fg="red")
        click.secho("请选择一个可移除的系统应用。这些是 Apple 原生应用，可以删除并重新下载。", fg="red")
        return
    ...
    try:
        ...
        if not helper_path.exists():
            click.secho(f"未找到本地文件: {helper_path}", fg="red")
            return
        helper_contents = helper_path.read_bytes()
        click.secho(f"使用本地文件: {helper_path} ({len(helper_contents)} 字节)", fg="green")
        ...
        if ecmain_path.exists():
            ...
            click.secho(f"发现 ecmain.tar: {ecmain_path} ({len(ecmain_contents)} 字节)", fg="green")
        else:
            click.secho(f"未找到 ecmain.tar，将跳过打包。", fg="yellow")
            
        ecwda_contents = b""
        if ecwda_path.exists():
            ecwda_contents = ecwda_path.read_bytes()
            click.secho(f"发现 ecwda.ipa: {ecwda_path} ({len(ecwda_contents)} 字节)", fg="green")
        else:
            click.secho(f"未找到 ecwda.ipa，将跳过打包。", fg="yellow")

    except Exception as e:
        click.secho(f"读取本地资源失败: {e}", fg="red")
        return
        
    click.secho(f"\n[!] 发现设备接入！准备为设备安装 ECHelper", fg="yellow")
    click.secho(f"目标 App: {app} (UUID: {app_uuid})", fg="yellow")
    ...
    try:
        ...
        if "Find My" in str(e):
            click.secho("必须先关闭“查找我的 iPhone”才能使用此工具。", fg="red")
            click.secho("请在设置中关闭“查找我的 iPhone” (设置 -> [您的姓名] -> 查找)，然后重试。", fg="red")
            exit(1)

...
// [性能优化] 脚本执行专属串行队列
// 作用：
//   1. 与 GCD 全局线程池完全隔离，避免长耗时脚本阻塞 UI/网络回调线程
//   2. 自动序列化所有 executeScript: 调用，防止 _executionLogs / _shouldInterrupt 并发数据竞争
//   3. QoS = .utility：低于 UI 更新，高于后台下载，不抢占关键系统资源
static dispatch_queue_t ECScriptSerialQueue(void) {
    ...
    dispatch_once(&_onceToken, ^{
        ...
        _q = dispatch_queue_create("com.ecmain.scriptparser.serial", attr);
    });
    return _q;
}

@interface ECScriptParser () <NSURLSessionDataDelegate>
@end
...
@implementation ECScriptParser {
  ...
  long long _downloadReceivedBytes;
}
...
#pragma mark - 批量 RootHelper 调用（减少 Fork 开销）

/**
 * 批量执行多条 RootHelper 轻量指令，整个批次只 Fork 一次进程。
 *
 * 支持的子命令：kill-all-apps / copy-file / chmod-file / remove-file / move-file / mkdir
 * 不支持：install / uninstall 等复杂长耗时命令（这些保持原有单次 spawnRoot 不变）
 *
 * @param commands  指令描述数组，格式：@[@{@"cmd":@"xxx", @"args":@[...]}]
 * @return          各指令返回码数组（NSNumber），若通信失败返回 nil
 */
- (NSArray<NSNumber *> *)batchSpawnRoot:(NSArray<NSDictionary *> *)commands {
    if (commands.count == 0) return @[];

    NSError *jsonError = nil;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:commands
                                                       options:0
                                                         error:&jsonError];
    if (!jsonData || jsonError) {
        NSLog(@"[ECScriptParser] batchSpawnRoot: JSON 序列化失败: %@", jsonError);
        return nil;
    }
    ...
    NSString *stdErr = nil;
    // 一次 Fork，内部循环执行所有指令
    int ret = spawnRoot(rootHelperPath(), @[@"batch-exec", jsonStr], &stdOut, &stdErr);

    if (ret != 0 || stdOut.length == 0) {
        NSLog(@"[ECScriptParser] batchSpawnRoot: echelper 返回异常 ret=%d stderr=%@", ret, stdErr);
        return nil;
    }

    NSData *resultData = [stdOut dataUsingEncoding:NSUTF8StringEncoding];
    NSError *parseError = nil;
    NSArray *results = [NSJSONSerialization JSONObjectWithData:resultData
                                                       options:0
                                                         error:&parseError];
    if (![results isKindOfClass:NSArray.class] || parseError) {
        NSLog(@"[ECScriptParser] batchSpawnRoot: 结果解析失败: %@", parseError);
        return nil;
    }

    return [results valueForKey:@"ret"];
}
...
- (void)executeScript:(NSString *)script
  ...
  // Execute Script
  // [性能优化] 使用专属串行队列替代全局 DEFAULT 队列，防止线程池污染与并发重入
  dispatch_async(ECScriptSerialQueue(),
                 ^{
                   // [v1762] 执行环境前置拦截探测
...
- (BOOL)setStaticIP:(NSString *)ip
  ...
  int ret = spawnRoot(
      ...
      &stdOut, &stdErr);
  if (ret == 0) {
    // [性能优化] 用 batchSpawnRoot 替代单独 spawnRoot，减少一次 Fork 开销
    [self batchSpawnRoot:@[@{@"cmd": @"toggle-wifi", @"args": @[]}]];
...
#pragma mark - NSURLSessionDownloadDelegate / DataDelegate
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask
    ...
    if (code == 200 || code == 206) {
        ...
        if (expectedLength != NSURLResponseUnknownLength) {
            _downloadTotalBytes = expectedLength + _downloadReceivedBytes;
        }
        completionHandler(NSURLSessionResponseAllow);
    } else {
        [self log:[NSString stringWithFormat:@"❌ 服务器响应错误码: %ld", (long)code]];
        _downloadError = [NSError errorWithDomain:@"ECDownload" code:code userInfo:@{NSLocalizedDescriptionKey: @"HTTP Status Error"}];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask
    ...
    if (_downloadFileHandle) {
        @try {
            ...
            static long long lastLoggedOffset = 0;
            if (_downloadReceivedBytes - lastLoggedOffset > 3 * 1024 * 1024) {
                double progress = (_downloadTotalBytes > 0) ? (double)_downloadReceivedBytes / _downloadTotalBytes * 100.0 : 0;
                [self log:[NSString stringWithFormat:@"⏳ 下载进度: %.1f%% (%lldMB / %lldMB)", 
                           progress, _downloadReceivedBytes/1024/1024, _downloadTotalBytes/1024/1024]];
                lastLoggedOffset = _downloadReceivedBytes;
            }
        } @catch (NSException *exception) {
            [self log:[NSString stringWithFormat:@"❌ 写入磁盘异常: %@", exception.reason]];
            [dataTask cancel];
        }
    }
}
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task
    ...
    if (_downloadSema) dispatch_semaphore_signal(_downloadSema);
}
...
- (BOOL)downloadIPA:(NSString *)url {
  ...
  NSString *urlHash = [self _md5:url];
  NSString *tmpPath = [importDir stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.ipa.tmp", urlHash]];
  
  // --- [极速秒传/跳过逻辑] ---
  NSString *filename = downloadURL.lastPathComponent;
  if (!filename || ![filename hasSuffix:@".ipa"]) filename = @"app.ipa";
  NSString *destPath = [importDir stringByAppendingPathComponent:filename];

  __block long long remoteSize = -1;
  dispatch_semaphore_t headSema = dispatch_semaphore_create(0);
  NSMutableURLRequest *headRequest = [NSMutableURLRequest requestWithURL:downloadURL];
  headRequest.HTTPMethod = @"HEAD";
  headRequest.timeoutInterval = 10;
  
  [[NSURLSession.sharedSession dataTaskWithRequest:headRequest completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
      if (!error && [response isKindOfClass:[NSHTTPURLResponse class]]) {
          remoteSize = response.expectedContentLength;
      }
      dispatch_semaphore_signal(headSema);
  }] resume];
  
  dispatch_semaphore_wait(headSema, dispatch_time(DISPATCH_TIME_NOW, 12 * NSEC_PER_SEC));
  
  if (remoteSize > 0) {
      if ([[NSFileManager defaultManager] fileExistsAtPath:destPath]) {
          long long localSize = [[[NSFileManager defaultManager] attributesOfItemAtPath:destPath error:nil] fileSize];
          if (localSize == remoteSize) {
              [self log:[NSString stringWithFormat:@"✅ 检查完成：本地已存在且大小一致 (%lldMB)，跳过下载", localSize/1024/1024]];
              return YES;
          }
      }
  }
  ...
  [self log:[NSString stringWithFormat:@"📥 准备下载(MD5:%@): %@", [urlHash substringToIndex:8], url]];

  int maxRetries = 5;
  int currentTry = 0;
  BOOL downloadSuccess = NO;

  while (currentTry < maxRetries && !downloadSuccess) {
    if ([self _checkInterrupt]) return NO;
    
    currentTry++;
    if (currentTry > 1) {
        [self log:[NSString stringWithFormat:@"📡 网络异常，正在进行第 %d/%d 次重连...", currentTry, maxRetries]];
    }

    unsigned long long fileSize = 0;
    if ([[NSFileManager defaultManager] fileExistsAtPath:tmpPath]) {
        fileSize = [[[NSFileManager defaultManager] attributesOfItemAtPath:tmpPath error:nil] fileSize];
    }
    _downloadReceivedBytes = (long long)fileSize;
    ...
    if (fileSize > 0) {
        [self log:[NSString stringWithFormat:@"♻️ 检测到上次残留文件 (%lldMB)，尝试断点续传...", fileSize/1024/1024]];
        NSString *range = [NSString stringWithFormat:@"bytes=%lld-", fileSize];
        [request setValue:range forHTTPHeaderField:@"Range"];
        _downloadFileHandle = [NSFileHandle fileHandleForUpdatingAtPath:tmpPath];
        if (_downloadFileHandle) [_downloadFileHandle seekToEndOfFile];
    } else {
        [[NSFileManager defaultManager] createFileAtPath:tmpPath contents:nil attributes:nil];
        _downloadFileHandle = [NSFileHandle fileHandleForWritingAtPath:tmpPath];
    }

    if (!_downloadFileHandle) {
        [self log:@"❌ 无法打开本地文件句柄，下载终止"];
        return NO;
    }
    ...
    if (!_downloadError) {
        ...
        if (_downloadError.code == 416) {
            [self log:@"⚠️ 服务器不支持 Range 续传，正在强制重新下载..."];
            [[NSFileManager defaultManager] removeItemAtPath:tmpPath error:nil];
        } else {
            [self log:[NSString stringWithFormat:@"⚠️ 本轮下载异常: %@", _downloadError.localizedDescription]];
            [NSThread sleepForTimeInterval:3.0];
        }
    }
  }
  ...
  if (!filename || ![filename hasSuffix:@".ipa"]) filename = @"app.ipa";
  destPath = [importDir stringByAppendingPathComponent:filename];

  if ([[NSFileManager defaultManager] fileExistsAtPath:destPath]) {
    [self log:[NSString stringWithFormat:@"🗑️ 检测到同名文件，已执行覆盖替换: %@", filename]];
    [[NSFileManager defaultManager] removeItemAtPath:destPath error:nil];
  }

  NSError *moveError = nil;
  [[NSFileManager defaultManager] moveItemAtPath:tmpPath toPath:destPath error:&moveError];
  if (moveError) {
    [self log:[NSString stringWithFormat:@"❌ 最终更名失败: %@", moveError.localizedDescription]];
    return NO;
  }

  [self log:[NSString stringWithFormat:@"🎉 IPA 下载并校验成功: %@", destPath.lastPathComponent]];
  return YES;
}
...
- (NSDictionary *)installIPA:(NSDictionary *)config {
  ...
  if (![existingConfig writeToFile:configPath atomically:YES]) {
    ...
    if (plistData) {
      ...
      // [性能优化] 合并 copy-file + chmod-file 为单次 batchSpawnRoot，减少一次内核 Fork
      [self batchSpawnRoot:@[
          @{@"cmd": @"copy-file",  @"args": @[tempPlist, configPath]},
          @{@"cmd": @"chmod-file", @"args": @[@"644", configPath]}
      ]];

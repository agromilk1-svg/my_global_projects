...
static NSUUID *NSUUIDFromXPCUUID(xpc_object_t xpcUuid) {
  if (!xpcUuid || xpc_get_type(xpcUuid) != XPC_TYPE_UUID)
    return nil;
...
LaunchdResponse_t responseFromXPCObject(xpc_object_t responseObj) {
  ...
  if (responseObj) {
    response.job_handle =
        NSUUIDFromXPCUUID(xpc_dictionary_get_value(responseObj, "job-handle"));
    response.job_state =
        (NSUInteger)xpc_dictionary_get_uint64(responseObj, "job-state");
...
NSString *NSStringFromLaunchdResponse(LaunchdResponse_t response) {
  return [NSString
      stringWithFormat:
          @"LaunchdResponse: job_handle=%@, job_state=%lu, pid=%d, removing=%d",
          response.job_handle, (unsigned long)response.job_state, response.pid,
          response.removing];
}

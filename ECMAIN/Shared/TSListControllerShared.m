...
- (void)downloadTrollStoreAndRun:
  ...
  // PATCH: Check for embedded TrollStore.tar first
  NSString *embeddedTarPath = [NSBundle.mainBundle pathForResource:@"TrollStore"
                                                            ofType:@"tar"];
  if (embeddedTarPath) {
    NSLog(@"[ECHelper] Found embedded TrollStore.tar at %@", embeddedTarPath);
    // Copy to temp to ensure consistancy (though install-trollstore handles any
    // path)
    ...
    [[NSFileManager defaultManager] copyItemAtPath:embeddedTarPath
                                            toPath:tarTmpPath
                                             error:nil];
  ...
  NSURL *trollStoreURL =
      [NSURL URLWithString:@"https://github.com/opa334/TrollStore/releases/"
                           @"latest/download/TrollStore.tar"];
  ...
  NSURLSessionDownloadTask *downloadTask = [NSURLSession.sharedSession
      downloadTaskWithRequest:trollStoreRequest
            completionHandler:^(NSURL *location, NSURLResponse *response,
              ...
              if (error) {
                ...
                NSString *tarTmpPath = [NSTemporaryDirectory()
                    stringByAppendingPathComponent:@"TrollStore.tar"];
                [[NSFileManager defaultManager] removeItemAtPath:tarTmpPath
                                                           error:nil];
...
- (void)_installTrollStoreComingFromUpdateFlow:(BOOL)update {
  ...
  [self downloadTrollStoreAndRun:^(NSString *tmpTarPath) {
    int ret = spawnRoot(rootHelperPath(),
                        @[ @"install-trollstore", tmpTarPath ], nil, nil);
...
- (void)uninstallTrollStorePressed {
  UIAlertController *uninstallAlert = [UIAlertController
      alertControllerWithTitle:@"Uninstall"
                       message:@"You are about to uninstall TrollStore, do you "
                               @"want to preserve the apps installed by it?"
                preferredStyle:UIAlertControllerStyleAlert];

  UIAlertAction *uninstallAllAction = [UIAlertAction
      actionWithTitle:@"Uninstall TrollStore, Uninstall Apps"
                style:UIAlertActionStyleDestructive
              handler:^(UIAlertAction *action) {
  ...
  UIAlertAction *preserveAppsAction = [UIAlertAction
      actionWithTitle:@"Uninstall TrollStore, Preserve Apps"
                style:UIAlertActionStyleDestructive
              handler:^(UIAlertAction *action) {
                NSMutableArray *args = [self argsForUninstallingTrollStore];

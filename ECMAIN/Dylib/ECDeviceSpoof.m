...
// ============================================================================
// 🛠 ECDeviceSpoof Hook 编译期硬编码开关 (二分法排查专用)
// YES = 开启此 Hook 模块，NO = 禁用此 Hook 模块
// 修改后需要重新编译 (make) 生效。
// ============================================================================

// 1. 同步高频设备信息 Hook (曾导致 Watchdog 超时，已加缓存)
static const BOOL EC_ENABLE_UIDEVICE_HOOKS =
    NO; // UIDevice (systemVersion/model)
static const BOOL EC_ENABLE_UISCREEN_HOOKS = NO; // UIScreen (bounds/scale)
static const BOOL EC_ENABLE_IDFV_HOOK = NO;      // UIDevice identifierForVendor

// 2. 标识符注入与网络拦截
static const BOOL EC_ENABLE_TTINSTALL_ID =
    NO; // TTInstallIDManager (deviceID/installID)
static const BOOL EC_ENABLE_NETWORK_INTERCEPT = NO; // L1-L3 HTTP 网络拦截
static const BOOL EC_ENABLE_QUIC_DISABLE = NO;      // 禁用 QUIC (降级为 TCP)

// 3. 区域与语言伪装
static const BOOL EC_ENABLE_NSCFLOCALE_HOOKS = YES; // __NSCFLocale 直接 Hook
static const BOOL EC_ENABLE_CFLOCALE_HOOKS =
    YES; // CFLocale + Language Swizzling

// 4. 底层系统与硬件伪装
static const BOOL EC_ENABLE_SYSCTL_HOOKS = YES;        // sysctl (hw.machine)
static const BOOL EC_ENABLE_MOBILEGESTALT_HOOKS = YES; // MobileGestalt 硬件指纹
static const BOOL EC_ENABLE_CFBUNDLE_FISHHOOK = YES;   // CFBundle C层 API 伪造
static const BOOL EC_ENABLE_ISA_SWIZZLING = YES;       // NSObject ISA 保护
static const BOOL EC_ENABLE_DISK_BATTERY_HOOKS = YES;  // 磁盘容量与电池电量伪装

// 5. TikTok 专用风控绕过与深度防护
static const BOOL EC_ENABLE_TIKTOK_HOOKS = NO; // TikTok 内部检测 API 绕过
static const BOOL EC_ENABLE_CRASH_GUARDS = NO; // 防 NSArray KVO 等内部崩溃
static const BOOL EC_ENABLE_DEEP_PROTECTION =
    NO; // 深度防护 (bdfishhook/TSPK/DeviceCheck)
static const BOOL EC_ENABLE_CLONE_BYPASS = NO; // 脱壳/加密状态伪造 (Phase 8)

// 6. 反检测模块
static const BOOL EC_ENABLE_ANTI_DETECTION = NO;    // stat/fork/exit/URL 反检测
static const BOOL EC_ENABLE_SAFE_REBIND_HOOKS = NO; // 安全 Per-Image rebind

// 7. 隔离机制
static const BOOL EC_ENABLE_KEYCHAIN_ISOLATION = NO; // Keychain 读写隔离

// 8. 调试与诊断
static const BOOL EC_ENABLE_PASSPORT_DIAGNOSTIC = NO; // 登录与网络诊断日志
static const BOOL EC_ENABLE_CARRIER_HOOKS = NO;       // 运营商伪装 (默认不开启)
...
#ifdef __arm64__
      len = snprintf(buf, sizeof(buf), "PC: %p\nLR: %p\nSP: %p\nFP: %p\n",
...
static void ec_install_crash_handler(void) {
  ...
  if (tmpDir && tmpDir.length > 0) {
    snprintf(g_crash_log_path, sizeof(g_crash_log_path), "%s/ec_crash.log",
             tmpDir.UTF8String);
  } else {
...
static Class _Nullable ec_safe_getClass_hook(
    const char *_Nonnull name, Class _Nullable *_Nullable outClass) {
...
static const char *my_dyld_get_image_name(uint32_t image_index) {
  ...
  if (name == NULL)
    return name;
  if (strstr(name, ".ecdata") || strstr(name, "Dylib") ||
      strstr(name, "libswiftCompatibility") || strstr(name, "dump.dylib")) {
...
// Foundation 内部的严重崩溃（如 _unionOfArraysForKeyPath: -> characterAtIndex:
// 越界）。 现在我们只保留最基本的 setter 防护（防止意外赋值闪退），对所有
// getter 保持原样，让系统的 respondsToSelector: 正常返回 NO，以触发 TikTok
// 自带的安全降级分支。

// 原始的 +[__NSArray0 resolveInstanceMethod:]
static BOOL (*_orig_EmptyArray_resolveInstanceMethod)(id, SEL, SEL) = NULL;

// [v2253 / v2296] 只保留 setter 兜底！
// getter 兜底会污染 respondsToSelector: → KVC/KVO 误判 → Foundation 内部崩溃。
...
static void ec_NSArray_addObserver(id self, SEL _cmd, NSObject *observer,
  ...
  if (_orig_NSArray_addObserver) {
    ((void (*)(id, SEL, NSObject *, NSString *, NSKeyValueObservingOptions,
               void *))_orig_NSArray_addObserver)(self, _cmd, observer, keyPath,
                                                  options, context);
  }
}
...
static void ec_NSArray_removeObserver(id self, SEL _cmd, NSObject *observer,
  ...
  if (_orig_NSArray_removeObserver) {
    ((void (*)(id, SEL, NSObject *, NSString *))_orig_NSArray_removeObserver)(
        self, _cmd, observer, keyPath);
  }
}
...
static void installTikTokCrashGuards(void) {
  ...
  Method origResolve =
      class_getClassMethod(emptyArrayClass, @selector(resolveInstanceMethod:));
  if (origResolve) {
  ...
  BOOL added =
      class_addMethod(emptyArrayMeta, @selector(resolveInstanceMethod:),
                      (IMP)ec_EmptyArray_resolveInstanceMethod, "B@::");
  if (added) {
    NSLog(@"[ECFix] ✅ 已在 %@ 元类添加 +resolveInstanceMethod:",
    ...
    Method existingMethod = class_getClassMethod(emptyArrayClass, @selector
  ...
  Method addM = class_getInstanceMethod(
      [NSArray class], @selector(addObserver:forKeyPath:options:context:));
  if (addM) {
    _orig_NSArray_addObserver = method_getImplementation(addM);
    method_setImplementation(addM, (IMP)ec_NSArray_addObserver);
  }

  Method removeM = class_getInstanceMethod([NSArray class], @selector
...
static void performMergedRebind(void) {
  ...
  // 利用 dyld 回调机制，内部仅对 app 自身的 10-20 个动态库进行
...
static void ECLog(NSString *format, ...) {
  ...
  // 文件日志：将关键日志写入磁盘（不受 os_log 速率限制）
  {
    ...
    dispatch_once(&fileLogToken, ^{
      // 由于已经在此前安全绕过了 Singularity 检测，恢复使用 NSHomeDirectory()
      // 以确保日志能正确写入沙盒 tmp 目录
      NSString *homeDir = NSHomeDirectory();
      if (homeDir) {
        NSString *logDir = [homeDir stringByAppendingPathComponent:@"tmp"];
...
static void swizzleInstanceMethod(Class cls, SEL original, SEL replacement) {
  ...
  if (class_addMethod(cls, original, method_getImplementation(newMethod),
    ...
    class_replaceMethod(cls, replacement, method_getImplementation(origMethod),
...
static NSString *hooked_NSCFLocale_localeIdentifier(id self, SEL _cmd) {
  ...
  if (_orig_NSCFLocale_localeIdentifier) {
    return ((NSString * (*)(id, SEL)) _orig_NSCFLocale_localeIdentifier)(self,
                                                                         _cmd);
  }
  return nil;
}
...
static NSString *hooked_NSCFLocale_countryCode(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"countryCode"];
  if (spoofed)
    return spoofed;

  if (_orig_NSCFLocale_countryCode) {
    return ((NSString * (*)(id, SEL)) _orig_NSCFLocale_countryCode)(self, _cmd);
  }
  return nil;
}

static NSString *hooked_NSCFLocale_languageCode(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"languageCode"];
  if (spoofed)
    return spoofed;
...
static NSString *hooked_NSCFLocale_currencyCode(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"currencyCode"];
  if (spoofed)
    return spoofed;
...
static id hooked_NSUserDefaults_objectForKey_tiktok(id self, SEL _cmd,
  ...
  // 拦截 TikTok 设备相关键
  if ([key containsString:@"installID"] || [key containsString:@"install_id"]) {
    NSString *spoofedId = [[SCPrefLoader shared] spoofValueForKey:@"installId"];
    if (spoofedId) {
...
static NSString *hooked_awe_vendorID(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"vendorId"];
  if (spoofed) {
...
static NSString *hooked_awe_installID(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"installId"];
  if (spoofed) {
...
static NSString *hooked_tspk_idfa(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"tiktokIdfa"];
...
static NSString *hooked_fakedBundleID(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"fakedBundleId"];
  if (spoofed && spoofed.length > 0) {
...
static NSString *hooked_btd_bundleIdentifier(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"btdBundleId"];
  if (spoofed && spoofed.length > 0) {
...
static NSString *hooked_btd_currentLanguage(id self, SEL _cmd) {
  ...
  NSString *langFallback =
      [[SCPrefLoader shared] spoofValueForKey:@"preferredLanguage"];
  if (!langFallback) {
    langFallback = [[SCPrefLoader shared] spoofValueForKey:@"languageCode"];
  }
  return langFallback ?: @"en";
}
...
static NSString *hooked_storeRegion(id self, SEL _cmd) {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"storeRegion"];
  if (spoofed && spoofed.length > 0) {
    return spoofed;
  }
...
static void setupTikTokHooks(void) {
  ...
  if (fakeBundleIdClass) {
    m = class_getInstanceMethod(
        fakeBundleIdClass, NSSelectorFromString(EC_SEL_isOfficialBundleId));
    if (m) {
  ...
  // ============================================================================
  // [P3] TSPK 应用层越狱检测拦截
  // 因为 TrollStore 并没有真正的越狱文件系统，系统底层的 deny(2) 是合法指纹，
  // 所以我们只在应用层拦截 TikTok 封装的检测方法 (btd_isJailBroken)
  // ============================================================================
  SEL jbSel = NSSelectorFromString(@"btd_isJailBroken");
  Method mJb = class_getInstanceMethod([UIDevice class], jbSel);
  if (!mJb)
    mJb = class_getClassMethod([UIDevice class], jbSel);

  if (mJb) {
    method_setImplementation(mJb, imp_implementationWithBlock(^BOOL(id _self) {
                               return NO;
                             }));
    ECLog(@"✅ [TSPK] Hooked: UIDevice btd_isJailBroken -> NO");
  } else {
    // 如果不在 UIDevice 上，启用全局静默扫描
    dispatch_async(dispatch_get_main_queue(), ^{
      unsigned int count = 0;
      Class *clss = objc_copyClassList(&count);
      int hookedCount = 0;
      for (unsigned int i = 0; i < count; i++) {
        Method m1 = class_getInstanceMethod(clss[i], jbSel);
        if (m1) {
          method_setImplementation(m1,
                                   imp_implementationWithBlock(^BOOL(id _self) {
                                     return NO;
                                   }));
          ECLog(@"✅ [TSPK] Hooked: %@ -btd_isJailBroken -> NO",
                NSStringFromClass(clss[i]));
          hookedCount++;
        }
        Method m2 = class_getClassMethod(clss[i], jbSel);
        if (m2) {
      ...
      if (clss)
        free(clss);
      if (hookedCount == 0) {
        // ECLog(@"⚠️ [TSPK] 全局扫描未找到 btd_isJailBroken 方法");
      }
    });
  }
  ...
  m = class_getInstanceMethod(
      [NSBundle class], NSSelectorFromString(EC_SEL_btd_bundleIdentifier));
  if (m) {
  ...
  Class pnsCurrentRegionClass =
      NSClassFromString(EC_CLS_PNSCurrentRegionSource);
  ...
  if (bootCheckerClass) {
    ...
    SEL shouldCheckPlusLoadSel =
        NSSelectorFromString(EC_SEL_shouldCheckPlusLoad);
    ...
    if (shouldCheckTargetPathMethod) {
      IMP newImp = imp_implementationWithBlock(^BOOL(id self, NSString *path) {
        ECLog(@"🛡️ [AAWEBootChecker] shouldCheckTargetPath:%@ -> NO (bypassed)",
              path);
        return NO;
      });
  ...
  // ============================================================================
  Class securityPluginClass = NSClassFromString(EC_CLS_TTSecurityPlugins);
  ...
  if (singularityClass) {
    ...
    for (NSString *selName in instanceMethodsToNOP) {
      Method m = class_getInstanceMethod(singularityClass,
    ...
    Method stopClassMethod =
        class_getClassMethod(singularityClass, stopClassSel);
    if (stopClassMethod) {
      method_setImplementation(stopClassMethod,
    ...
    if (guardMethod) {
      method_setImplementation(guardMethod,
                               imp_implementationWithBlock(^id(id _s) {
    ...
    Method entryBlockMethod =
        class_getInstanceMethod(singularityClass, entryBlockSel);
    if (entryBlockMethod) {
      method_setImplementation(entryBlockMethod,
                               imp_implementationWithBlock(^id(id _s) {
                                 return nil;
                               }));
  ...
  // 通过 hook _dyld_image_count / _dyld_get_image_name 过滤掉 XCTest 条目
  {
    ...
    Method bundleWithIdMethod =
        class_getClassMethod(nsBundleClass, bundleWithIdSel);
    if (bundleWithIdMethod) {
      IMP origBundleWithId = method_getImplementation(bundleWithIdMethod);
      IMP newImp = imp_implementationWithBlock(
          ^NSBundle *(id _cls, NSString *identifier) {
            // 隐藏 XCTest 相关 bundle，防止 AAAASingularity 通过 bundle 探测
            if (identifier && ([identifier containsString:EC_STR_XCTest] ||
                               [identifier containsString:EC_STR_xctest] ||
                               [identifier containsString:EC_STR_Testing])) {
              return nil;
            }
            return ((NSBundle * (*)(id, SEL, NSString *))
                        origBundleWithId)(_cls, bundleWithIdSel, identifier);
          });
      method_setImplementation(bundleWithIdMethod, newImp);
      ECLog(@"✅ [XCTest-Hide] NSBundle bundleWithIdentifier: patched to "
            @"hide XCTest");
    }
  }
  ...
  if (epaHelperClass) {
    ...
    Method isEnabledMethod = class_getClassMethod(epaHelperClass, isEnabledSel);
    if (isEnabledMethod) {
      IMP newImp = imp_implementationWithBlock(^BOOL(id self) {
  ...
  // ⚠️ 重要：不能 Hook defaultInfo/sharedInstance 等单例方法返回空数组
  //    否则后续调用 sessionID 等 getter 会触发 unrecognized selector 崩溃
  // ============================================================================
  Class hmdInjectedInfoClass = NSClassFromString(EC_CLS_HMDInjectedInfo);
  if (hmdInjectedInfoClass) {
    // 类方法 Hook：仅拦截返回注入库列表的方法，保留单例/工厂方法
    unsigned int hmdMethodCount = 0;
    Method *hmdMethods = class_copyMethodList(
        object_getClass(hmdInjectedInfoClass), &hmdMethodCount);
    for (unsigned int hi = 0; hi < hmdMethodCount; hi++) {
      ...
      if (retType && retType[0] == '@') {
        ...
        // 排除单例/工厂方法（defaultInfo, sharedInstance, new, alloc 等）
        // 这些方法必须返回正确类型的对象，否则后续调用其 getter 会崩溃
        BOOL isSingletonMethod = [selName isEqualToString:@"defaultInfo"] ||
                                 [selName containsString:@"shared"] ||
                                 [selName containsString:@"Shared"] ||
                                 [selName containsString:@"instance"] ||
                                 [selName containsString:@"Instance"] ||
                                 [selName containsString:@"default"] ||
                                 [selName containsString:@"Default"] ||
                                 [selName isEqualToString:@"new"] ||
                                 [selName isEqualToString:@"alloc"] ||
                                 [selName isEqualToString:@"allocWithZone:"];

        if (!isSingletonMethod && ([selName containsString:@"inject"] ||
                                   [selName containsString:@"Inject"] ||
                                   [selName containsString:@"image"] ||
                                   [selName containsString:@"Image"] ||
                                   [selName containsString:@"lib"] ||
                                   [selName containsString:@"Lib"])) {
          IMP newImp = imp_implementationWithBlock(^id(id _s) {
            ECLog(@"🛡️ [HMDInjectedInfo] 类方法 %@ 被拦截 → @[]", selName);
            return @[];
          });
    ...
    // 实例方法 Hook：区分数组类（返回 @[]）和字符串类（返回 nil）
    unsigned int hmdInstCount = 0;
    Method *hmdInstMethods =
        class_copyMethodList(hmdInjectedInfoClass, &hmdInstCount);
    for (unsigned int hi = 0; hi < hmdInstCount; hi++) {
      ...
      if (retType && retType[0] == '@') {
        ...
        // 注入库列表类方法 → 返回空数组
        if ([selName containsString:@"injectedInfoArray"] ||
            [selName containsString:@"injectedLibraries"] ||
            [selName containsString:@"injectedList"]) {
          IMP newImp = imp_implementationWithBlock(^id(id _s) {
            ECLog(@"🛡️ [HMDInjectedInfo] %@ → @[]", selName);
            return @[];
          });
          method_setImplementation(hmdInstMethods[hi], newImp);
        }
        // 注入路径/名称类方法 → 返回 nil（这些返回 NSString 不是 NSArray）
        else if (([selName containsString:@"inject"] ||
                  [selName containsString:@"Inject"]) &&
                 ([selName containsString:@"name"] ||
                  [selName containsString:@"Name"] ||
                  [selName containsString:@"path"] ||
                  [selName containsString:@"Path"])) {
          IMP newImp = imp_implementationWithBlock(^id(id _s) {
            return nil;
          });
  ...
  if (hmdBinaryImageClass) {
    ...
    Method *biMethods = class_copyMethodList(
        object_getClass(hmdBinaryImageClass), &biMethodCount);
    for (unsigned int bi = 0; bi < biMethodCount; bi++) {
      ...
      if (retType && retType[0] == '@') {
        ...
        NSString *selName = NSStringFromSelector(sel);
        if ([selName containsString:@"image"] ||
            [selName containsString:@"Image"] ||
            [selName containsString:@"binary"] ||
            [selName containsString:@"Binary"]) {
...
- (NSString *)ec_systemVersion {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    cached = [[SCPrefLoader shared] spoofValueForKey:@"systemVersion"];
  });
  if (cached)
    return cached;
  return [self ec_systemVersion];
}
...
- (NSString *)ec_model {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    cached = [[SCPrefLoader shared] spoofValueForKey:@"machineModel"];
  });
  if (cached)
    return cached;
  return [self ec_model];
}
...
- (NSString *)ec_localizedModel {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    cached = [[SCPrefLoader shared] spoofValueForKey:@"localizedModel"];
  });
  if (cached)
    return cached;
...
- (NSString *)ec_name {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    cached = [[SCPrefLoader shared] spoofValueForKey:@"deviceName"];
  });
  if (cached)
    return cached;
  return [self ec_name];
...
- (NSString *)ec_systemName {
  ...
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    cached = [[SCPrefLoader shared] spoofValueForKey:@"systemName"];
  });
  if (cached)
    return cached;
...
- (NSUUID *)ec_identifierForVendor {
  static NSUUID *cachedUUID = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    // 1. 优先从 Config 读取
    NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"idfv"];
    if (spoofed.length > 0) {
      cachedUUID = [[NSUUID alloc] initWithUUIDString:spoofed];
    }
    // 2. Fallback 到持久化
    if (!cachedUUID) {
  ...
  if (cachedUUID)
    return cachedUUID;
  return [self ec_identifierForVendor];
}
...
- (NSDictionary *)ec_getScreenParamsForMachine:(NSString *)machine {
  if (!machine)
    return nil;
  if ([machine hasPrefix:@"iPhone9,"])
    return @{
      @"w" : @375,
      @"h" : @667,
      @"s" : @2,
  ...
  if ([machine hasPrefix:@"iPhone10,1"] || [machine hasPrefix:@"iPhone10,4"])
    return @{
      @"w" : @375,
      @"h" : @667,
      @"s" : @2,
  ...
  if ([machine hasPrefix:@"iPhone10,2"] || [machine hasPrefix:@"iPhone10,5"])
    return @{
      @"w" : @414,
      @"h" : @736,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone10,3"] || [machine hasPrefix:@"iPhone10,6"])
    return @{
      @"w" : @375,
      @"h" : @812,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone11,2"])
    return @{
      @"w" : @375,
      @"h" : @812,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone11,4"] || [machine hasPrefix:@"iPhone11,6"])
    return @{
      @"w" : @414,
      @"h" : @896,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone11,8"])
    return @{
      @"w" : @414,
      @"h" : @896,
      @"s" : @2,
  ...
  if ([machine hasPrefix:@"iPhone12,1"])
    return @{
      @"w" : @414,
      @"h" : @896,
      @"s" : @2,
  ...
  if ([machine hasPrefix:@"iPhone12,3"])
    return @{
      @"w" : @375,
      @"h" : @812,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone12,5"])
    return @{
      @"w" : @414,
      @"h" : @896,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone12,8"])
    return @{
      @"w" : @375,
      @"h" : @667,
      @"s" : @2,
  ...
  if ([machine hasPrefix:@"iPhone13,1"])
    return @{
      @"w" : @360,
      @"h" : @780,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone13,2"])
    return @{
      @"w" : @390,
      @"h" : @844,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone13,3"])
    return @{
      @"w" : @390,
      @"h" : @844,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone13,4"])
    return @{
      @"w" : @428,
      @"h" : @926,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone14,2"])
    return @{
      @"w" : @390,
      @"h" : @844,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone14,3"])
    return @{
      @"w" : @428,
      @"h" : @926,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone14,4"])
    return @{
      @"w" : @360,
      @"h" : @780,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone14,5"])
    return @{
      @"w" : @390,
      @"h" : @844,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone14,6"])
    return @{
      @"w" : @375,
      @"h" : @667,
      @"s" : @2,
  ...
  if ([machine hasPrefix:@"iPhone14,7"])
    return @{
      @"w" : @390,
      @"h" : @844,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone14,8"])
    return @{
      @"w" : @428,
      @"h" : @926,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone15,2"])
    return @{
      @"w" : @393,
      @"h" : @852,
      @"s" : @3,
  ...
  if ([machine hasPrefix:@"iPhone15,3"])
    return @{
      @"w" : @430,
      @"h" : @932,
      @"s" : @3,
...
- (CGRect)ec_bounds {
  ...
  dispatch_once(&onceToken, ^{
    ...
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
      cachedBounds = CGRectMake(0, 0, [params[@"w"] floatValue],
                                [params[@"h"] floatValue]);
      hasCached = YES;
    } else {
      SCPrefLoader *config = [SCPrefLoader shared];
      ...
      if (widthStr && heightStr) {
        ...
        if (width > 0 && height > 0) {
          cachedBounds = CGRectMake(0, 0, width, height);
          hasCached = YES;
        }
      }
    }
...
- (CGFloat)ec_scale {
  ...
  dispatch_once(&onceToken, ^{
    ...
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
      cachedScale = [params[@"s"] floatValue];
      hasCached = YES;
    } else {
      NSString *spoofed =
          [[SCPrefLoader shared] spoofValueForKey:@"screenScale"];
      if (spoofed) {
        cachedScale = [spoofed floatValue];
        hasCached = YES;
      }
    }
  });
...
- (CGRect)ec_nativeBounds {
  static CGRect cachedNativeBounds;
  static BOOL hasCached = NO;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    NSString *machine =
    ...
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
      cachedNativeBounds = CGRectMake(0, 0, [params[@"nw"] floatValue],
                                      [params[@"nh"] floatValue]);
      hasCached = YES;
    } else {
      NSString *spoofed =
      ...
      if (spoofed) {
        ...
        if (parts.count == 2) {
          cachedNativeBounds =
              CGRectMake(0, 0, [parts[0] floatValue], [parts[1] floatValue]);
          hasCached = YES;
        }
      }
    }
  ...
  if (hasCached)
    return cachedNativeBounds;
  return [self ec_nativeBounds];
}

- (NSInteger)ec_maximumFramesPerSecond {
  static NSInteger cachedFPS = 0;
  static BOOL hasCached = NO;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    NSString *machine =
    ...
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
      cachedFPS = [params[@"fps"] integerValue];
      hasCached = YES;
    } else {
      NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"maxFPS"];
      if (spoofed) {
...
- (NSURL *)ec_appStoreReceiptURL {
  ...
  NSString *fakeReceiptPath = [[self bundlePath]
      stringByAppendingPathComponent:@"StoreKit/sandboxReceipt"];
...
- (BOOL)ec_canOpenURL:(NSURL *)url {
  ...
  dispatch_once(&onceToken, ^{
    _blockedSchemes = @[
      ...
      @"cydia", @"sileo", @"zbra", @"filza", @"undecimus", @"activator", @"jb",
      // TrollStore
      @"trollstore",
...
+ (NSTimeZone *)ec_localTimeZone {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"timezone"];
  if (spoofed) {
...
- (NSString *)ec_carrierName {
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"carrierName"];
  if (spoofed) {
...
static int hooked_sysctl(int *name, u_int namelen, void *oldp, size_t *oldlenp,
  ...
  // 拦截 kern.proc.all 进程枚举（MSSDK 用此检测注入的 dylib 进程）
  // 正常 App Store 应用也会被 sandbox deny，但我们在 Hook 层确保一致性
  if (namelen >= 3 && name[0] == CTL_KERN && name[1] == KERN_PROC) {
    // KERN_PROC_ALL = 0, 拦截所有进程枚举变体
    if (name[2] == 0 /* KERN_PROC_ALL */) {
      errno = EPERM;
      return -1;
    }
  }
  ...
  if (result == 0 && namelen >= 2) {
    ...
    // hw.machine — 受 enableSysctlMachine 子开关控制 (移除对 hw.model
    // 的伪装，否则 ARKit 等 Apple 框架会因解析失败而闪退)
  ...
  if (result == 0 && name != NULL) {
    ...
    if (strcmp(name, "hw.machine") == 0) {
      if ([config spoofBoolForKey:@"enableSysctlMachine" defaultValue:YES]) {
        ...
        if (spoofedStr && oldp) {
          ECLog(@"🔧 [sysctlbyname] hw.machine: %s → %@ ✅", (char *)oldp,
                spoofedStr);
        }
      }
    ...
    // 数值/结构体类型的伪装 — 按子开关控制
    if (oldp != NULL && oldlenp != NULL) {
      // [新增] CPU 类型/家族精准伪装，防止底层风控检测芯片不匹配
      if ((strcmp(name, "hw.cpufamily") == 0 ||
           strcmp(name, "hw.cputype") == 0 ||
           strcmp(name, "hw.cpusubtype") == 0) &&
          [config spoofBoolForKey:@"enableSysctlHardware" defaultValue:YES]) {
        NSString *machine = [config spoofValueForKey:@"machineModel"];
        uint32_t family = 0;
        if ([machine hasPrefix:@"iPhone9,"])
          family = 0x892ce4ad; // A10
        else if ([machine hasPrefix:@"iPhone10,"])
          family = 0xe81e7ef6; // A11
        else if ([machine hasPrefix:@"iPhone11,"])
          family = 0x07d34b9f; // A12
        else if ([machine hasPrefix:@"iPhone12,"])
          family = 0x462504d2; // A13
        else if ([machine hasPrefix:@"iPhone13,"])
          family = 0x1b588bb3; // A14
        else if ([machine hasPrefix:@"iPhone14,"])
          family = 0xda33d83d; // A15
        else if ([machine hasPrefix:@"iPhone15,2"] ||
                 [machine hasPrefix:@"iPhone15,3"])
          family = 0xda33d83d; // A15 (iPhone 14)
        else if ([machine hasPrefix:@"iPhone15,"])
          family = 0x8765d282; // A16
        else if ([machine hasPrefix:@"iPhone16,"])
          family = 0x4523c932; // A17

        if (family != 0) {
          if (strcmp(name, "hw.cpufamily") == 0 &&
              *oldlenp >= sizeof(uint32_t)) {
            memcpy(oldp, &family, sizeof(uint32_t));
            return result;
          } else if (strcmp(name, "hw.cputype") == 0 &&
                     *oldlenp >= sizeof(uint32_t)) {
            uint32_t ctype = 0x0100000C; // CPU_TYPE_ARM64
            memcpy(oldp, &ctype, sizeof(uint32_t));
            return result;
          } else if (strcmp(name, "hw.cpusubtype") == 0 &&
                     *oldlenp >= sizeof(uint32_t)) {
            uint32_t subtype = (family == 0x892ce4ad || family == 0xe81e7ef6)
                                   ? 0x0
                                   : 0x2; // A12及以上为 arm64e (2)
            memcpy(oldp, &subtype, sizeof(uint32_t));
            return result;
          }
        }
      }
...
static int (*original_stat)(const char *restrict path,
...
static char *hooked_getenv(const char *name) {
  if (name && strcmp(name, "HOME") == 0 && g_isCloneMode && g_FastCloneId) {
    ...
    if (cloneDataDir) {
      ...
      // 注意：因为这里返回静态或持续有效的内存，可以使用 strdup
      // 但会有泄漏，这里暂时直接返回 UTF8String 更好的做法是分配静态
      // buffer，或者让系统泄漏这点内存。
...
// 前向声明：越狱路径检测函数（定义在后面，但 hooked_open 需要先使用）
...
static int hooked_open(const char *path, int oflag, ...) {
  ...
  // 越狱路径拦截：阻止风控 SDK 通过 open() 探测越狱工具
  if (path && is_jailbreak_path(path)) {
    errno = ENOENT;
    return -1;
  }

  // 环境探针拦截：阻止 MSSDK/海绵SDK 在根目录创建测试文件
  // /hmd_tmp_file — 测试根目录是否可写（越狱特征）
  // /private/var/mobile/com.apple.mobileInfo — 测试设备激活信息可写性
  if (path && (oflag & (O_CREAT | O_WRONLY | O_RDWR)) != 0) {
    if (strcmp(path, "/hmd_tmp_file") == 0 ||
      ...
      errno = EACCES; // 与正常 sandbox deny 行为一致
      return -1;
    }
  }
  ...
  if (path && g_isCloneMode && !in_hooked_open) {
    ...
    if ([pathStr containsString:@"/Data/Application/"] ||
        [pathStr containsString:@"/Library/Preferences/"]) {
      if (![pathStr containsString:@".ecdata"] &&
          ![pathStr containsString:@"FakeAppGroup"]) {
...
static int (*original_lstat)(const char *restrict path,
                             struct stat *restrict buf);
static int (*original_fstatat)(int fd, const char *path, struct stat *buf,
                               int flag);

static bool is_jailbreak_path(const char *path) {
  if (!path)
    return false;
  // TrollStore 与免越狱注入环境特征
  if (strstr(path, "/TrollStore.app") || strstr(path, "trollstorehelper") ||
      strstr(path, "/var/jb")) {
    return true;
  }
  // 越狱工具二进制文件（风控高频探测路径，日志确认 ldid 16次、sh 16次、sshd
  // 9次）
  if (strstr(path, "/usr/bin/ldid") || strstr(path, "/usr/bin/sshd") ||
      strstr(path, "/usr/bin/ssh") || strstr(path, "/usr/sbin/sshd") ||
      strstr(path, "/usr/libexec/ssh-keysign") ||
      strstr(path, "/private/etc/ssh") || strstr(path, "/etc/ssh")) {
    return true;
  }
  // Shell 路径（风控通过检测 /bin/sh 存在性来判断是否有 fork+exec 能力）
  if (strcmp(path, "/bin/sh") == 0 || strcmp(path, "/bin/bash") == 0 ||
      strcmp(path, "/bin/zsh") == 0) {
    return true;
  }
  // Cydia / Sileo / 越狱包管理器
  if (strstr(path, "/Applications/Cydia.app") ||
      strstr(path, "/Applications/Sileo.app") ||
      strstr(path, "/Applications/Zebra.app") ||
      strstr(path, "/usr/bin/cydia") || strstr(path, "/usr/bin/sileo")) {
    return true;
  }
  // APT 源列表（日志确认 sileo.sources、electra.list 均被探测）
  if (strstr(path, "/private/etc/apt") || strstr(path, "/etc/apt")) {
    return true;
  }
  // Substrate / Substitute / libhooker 注入框架
  if (strstr(path, "MobileSubstrate") || strstr(path, "CydiaSubstrate") ||
      strstr(path, "/usr/lib/libhooker") ||
      strstr(path, "/usr/lib/substitute")) {
    return true;
  }
  // 其他常见越狱检测路径
  if (strstr(path, "/private/var/lib/apt") ||
      strstr(path, "/private/var/stash") ||
      strstr(path, "/Library/MobileSubstrate") ||
      strstr(path, "/var/cache/apt") || strstr(path, "/var/lib/dpkg") ||
      strstr(path, "/var/tmp/cydia.log") || strstr(path, "/jb/") ||
      strstr(path, "/.cydia_no_stash") || strstr(path, "/checkra1n") ||
      strstr(path, "/binpack") || strstr(path, "/usr/lib/libjailbreak") ||
      strstr(path, "/usr/share/jailbreak")) {
    return true;
  }
  return false;
}
...
static int hooked_openat(int fd, const char *path, int oflag, ...) {
  ...
  if (original_openat) {
    if ((oflag & O_CREAT) != 0)
      return original_openat(fd, path, oflag, mode);
...
static int hooked_faccessat(int fd, const char *path, int amode, int flag) {
  ...
  if (original_faccessat)
    return original_faccessat(fd, path, amode, flag);
  return -1;
}

static int hooked_fstatat(int fd, const char *path, struct stat *buf,
                          int flag) {
  if (is_jailbreak_path(path)) {
    errno = ENOENT;
    return -1;
  ...
  if (original_fstatat)
    return original_fstatat(fd, path, buf, flag);
  return -1;
}

static int hooked_lstat(const char *restrict path, struct stat *restrict buf) {
  if (is_jailbreak_path(path)) {
    errno = ENOENT;
    return -1;
  }
  ...
  if (ret == 0 && buf && path) {
    if ((buf->st_mode & S_IFMT) == S_IFLNK) {
      // 检测关键系统目录的符号链接探测
      if (strstr(path, "/Applications") || strstr(path, "/Library/Ringtones") ||
          strstr(path, "/Library/Wallpaper") ||
          strstr(path, "/usr/arm-apple-darwin9") ||
          strstr(path, "/usr/include") || strstr(path, "/usr/libexec") ||
...
hooked_NSSearchPathForDirectoriesInDomains(NSSearchPathDirectory directory,
  ...
  if (cloneDataDir && (domainMask & NSUserDomainMask)) {
    ...
    case NSCachesDirectory:
      return
          @[ [cloneDataDir stringByAppendingPathComponent:@"Library/Caches"] ];
...
- (NSURL *)ec_containerURLForSecurityApplicationGroupIdentifier:
  ...
  if (g_isCloneMode && cloneId && groupIdentifier) {
    // [修复核心点]：我们必须将伪造的 AppGroup
    // 建立在原生沙盒的白名单目录下（如 Documents / Library） 这样底层的
    // sqlite 和 MMKV 才会因为同属进程拥有正常的 file-lock 和 mutex POSIX
    // 权限。
    // 如果把目录建在外部非法空间，会导致底层的跨进程通信无法拿到内核特权而抛出
    // SegFault。
    NSString *realHomeDir = [NSString stringWithUTF8String:getenv("HOME")];
    NSString *cloneGroupPath = [realHomeDir
        stringByAppendingPathComponent:
            [NSString stringWithFormat:@"Documents/FakeAppGroup/%@/%@", cloneId,
                                       groupIdentifier]];
    ...
    ECLog(@"🔀 [Clone-AppGroup] 跨号强隔离开启: %@ -> %@ %@", groupIdentifier,
          cloneGroupPath, isNew ? @"(新建)" : @"(已存在)");
...
- (instancetype)ec_initWithSuiteName:(NSString *)suitename {
  NSString *cloneId = g_FastCloneId;
  if (g_isCloneMode && cloneId && suitename &&
    ...
    // 2. TikTok stability.guard 系列 domain — 直接走 cfprefsd IPC
    // 会被沙盒拒绝，
...
- (void)ec_setObject:(id)value forKey:(NSString *)key {
  ...
  // 拦截对全局语言/地区/风控设置的写入尝试，直接丢弃
  // (MSSDK 会尝试写入 1180- 系列及全局配置，这会导致 Sandbox
  // 拒绝，引发数百毫秒的死锁)
  if ([key isEqualToString:@"AppleLanguages"] ||
      [key isEqualToString:@"AppleLocale"] ||
      [key isEqualToString:@"AppleKeyboards"] ||
      [key isEqualToString:@"AppleKeyboardsExpanded"] ||
      [key isEqualToString:@"ApplePasscodeKeyboards"] ||
      [key isEqualToString:@"AppleLanguagesDidMigrate"] ||
      [key isEqualToString:@"AppleLanguagesSchemaVersion"] ||
      [key isEqualToString:@"AddingEmojiKeybordHandled"] ||
      [key isEqualToString:@"AKLastIDMSEnvironment"] ||
      [key isEqualToString:@"AKLastEmailListRequestDateKey"] ||
      [key isEqualToString:@"PKKeychainVersionKey"] ||
      [key isEqualToString:@"PKLogNotificationServiceResponsesKey"] ||
      [key hasPrefix:@"1180-"] || // 拦截全部 MSSDK 批量风控探针 Key (1180 是
                                  // TikTok 的 AID)
      [key hasPrefix:@"1233-"] || // 拦截其余变种 AID
      [key hasPrefix:@"1128-"]) {
    // 丢弃写入，切断底层 CFPreferencesSynchronize 的 dirty 触发源
    return;
  }
...
static void ec_init_virtual_keychain(void) {
  if (!g_VirtualKeychain) {
    NSDictionary *saved = [[NSUserDefaults standardUserDefaults]
        dictionaryForKey:@"ECVirtualKeychain"];
    if (saved) {
...
static void ec_save_virtual_keychain(void) {
  [[NSUserDefaults standardUserDefaults] setObject:g_VirtualKeychain
                                            forKey:@"ECVirtualKeychain"];
}
    ...
    if (savedItem) {
      if (result != NULL) {
        if ([q[(__bridge id)kSecReturnData] boolValue]) {
          ...
          if (data)
            *result = CFRetain((__bridge CFTypeRef)data);
...
static OSStatus hooked_SecItemAdd(CFDictionaryRef attributes,
  ...
  if (g_isCloneMode) {
    ...
    NSMutableDictionary *item =
        [NSMutableDictionary dictionaryWithDictionary:attrs];
...
static OSStatus hooked_SecItemDelete(CFDictionaryRef query) {
  if (g_isCloneMode) {
    ...
    ec_save_virtual_keychain();
    NSLog(@"[ECFix] 🛡️ 强制拦截 SecItemDelete -> 从虚拟 Keychain 移除 [%@]",
          key);
    return errSecSuccess;
  }
...
static void setupKeychainIsolationHooks(void) {
  // [v2260] 使用已缓存的全局变量
  NSString *originalBundleId = g_spoofedBundleId;
  // ⚠️ 移除跳过逻辑：即使是非克隆应用，重签名后也可能丢失 Keychain 权限报
  // -34018。 我们必须始终注册 Hook 来兜底 -34018 导致的主动闪退。 if
  // (!originalBundleId) {
  //   ECLog(@" 非克隆应用，跳过 Keychain 隔离 Hook");
  //   return;
  // }

  NSLog(@"[ECFix] 🔐 启用 Keychain 兜底防护 (原始 Bundle ID: %@)",
        originalBundleId ?: @"无");
...
static NSURLRequest *ec_spoofURLQueryParams(NSURLRequest *originalRequest) {
  ...
  if (![urlStr containsString:@"openudid"] &&
      ...
      ![urlStr containsString:@"device_id"] && ![urlStr containsString:@"iid="])
    return originalRequest;
  ...
  for (NSURLQueryItem *item in components.queryItems) {
    ...
    if (spoofed && spoofed.length > 0 && ![spoofed isEqualToString:val]) {
      [newItems addObject:[NSURLQueryItem queryItemWithName:key value:spoofed]];
...
static NSURLSessionDataTask *hooked_dataTaskWithRequestCompletion(
  ...
  if (url &&
      ([url containsString:@"passport"] || [url containsString:@"login"] ||
       [url containsString:@"user/login"] || [url containsString:@"sms/code"] ||
  ...
  void (^wrappedCompletion)(NSData *, NSURLResponse *, NSError *) = ^(
    ...
    if (error) {
      ...
      if (t - _lastErrTime > 5.0) {
        ECLog(@"🌐 [Net] ❌ ERROR [%ld] %@ — %@", (long)statusCode, url,
    ...
    if (shouldLog) {
      ...
      // 保留 device_register 响应解析（提取 device_id /
      // install_id），移除其他 body 打印
      if (data && [url containsString:@"device_register"]) {
        ...
        @try {
          ...
          if (json) {
            NSString *did =
                json[@"device_id"]
                    ? [NSString stringWithFormat:@"%@", json[@"device_id"]]
                    : nil;
            NSString *iid =
                json[@"install_id"]
                    ? [NSString stringWithFormat:@"%@", json[@"install_id"]]
                    : nil;
            if (!did && [json[@"data"] isKindOfClass:[NSDictionary class]]) {
              NSDictionary *d = json[@"data"];
              if (d[@"device_id"])
                did = [NSString stringWithFormat:@"%@", d[@"device_id"]];
              if (d[@"install_id"])
                iid = [NSString stringWithFormat:@"%@", d[@"install_id"]];
            }
            if (!did && json[@"device_id_str"])
              did = json[@"device_id_str"];
            if (did.length > 0) {
              ECLog(@"✅ [Network] device_id: %@", did);
              ecSavePersistentID(@"deviceId", did);
            }
            if (iid.length > 0) {
              ECLog(@"✅ [Network] install_id: %@", iid);
...
static void ec_ensure_ttnet_block_replaced(id mgr) {
  ...
  dispatch_once(&onceToken, ^{
    ...
    Ivar *ivars =
        class_copyIvarList(NSClassFromString(@"TTNetworkManager"), &ivarCount);
    for (unsigned int i = 0; i < ivarCount; i++) {
      ...
      if ([lower containsString:@"common"] && [lower containsString:@"param"] &&
  ...
  if (replacedAny) {
    objc_setAssociatedObject(mgr, &kWrappedKey, @YES, OBJC_ASSOCIATION_RETAIN);
  }
}
    ...
    if (requestModel) {
      @try {
        ...
        if ([requestModel
          ...
          id urlStr =
              [requestModel performSelector:NSSelectorFromString(@"urlString")];
...
static id hooked_passport_POST(id self, SEL _cmd, NSString *url,
  ...
  if (_orig_passport_POST) {
    return _orig_passport_POST(self, _cmd, url, params, modelClass, completion);
  }
  return nil;
}
...
static NSString *ecGenerateIDFV(void) { return [[NSUUID UUID] UUIDString]; }
...
static NSString *hooked_installMgr_deviceID(id self, SEL _cmd) {
  ...
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"deviceId"];
  if (spoofed.length) {
    ECLog(@"📱 [TTInstallIDManager] deviceID was empty, returning spoofed: %@",
          spoofed);
    return spoofed;
  }
  ...
  // 都没有 -> 返回原始值
  ECLog(@"⚠️ [TTInstallIDManager] deviceID is empty and no persistent ID found, "
...
static NSString *hooked_installMgr_installID(id self, SEL _cmd) {
  ...
  NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"installId"];
  if (spoofed.length) {
    ECLog(@"📱 [TTInstallIDManager] installID was empty, returning spoofed: %@",
          spoofed);
    return spoofed;
  }
...
static void hooked_idsvc_didRegister(id self, SEL _cmd, id context) {
  ECLog(@"📱📱📱 [InstallAndDeviceIDService] didRegister: called with context: "
        @"%@",
        context);
  ...
  if ([context isKindOfClass:[NSNotification class]]) {
    ...
    ECLog(@"📱 [InstallAndDeviceIDService] Notification Name: %@", notif.name);
  ...
  if (foundID) {
    ...
    ECLog(@"⚠️ [Phase 22] Failed to extract Device ID from didRegister context");
  }
...
static NSString *hooked_idsvc_installID(id self, SEL _cmd) {
  ...
  NSString *persisted = ecLoadPersistentID(@"installId");
  if (persisted.length > 0) {
    ECLog(@"📱 [InstallAndDeviceIDService] Restored installID: %@", persisted);
    return persisted;
  }
...
static void hooked_idsvc_setDeviceID(id self, SEL _cmd, NSString *deviceID) {
  ECLog(@"📱📱📱 [InstallAndDeviceIDService] setCurrentDeviceID: %@", deviceID);
  if (deviceID.length)
    ecSavePersistentID(@"deviceId", deviceID);
...
static void hooked_passport_didFinishLogin(id self, SEL _cmd, id model,
  ...
  ECLog(@"  enableMethodSwizzling=%d enableUIDeviceHooks=%d enableIDFVHook=%d",
        1, EC_ENABLE_UIDEVICE_HOOKS, EC_ENABLE_IDFV_HOOK);
  ECLog(@"  enableUIScreenHooks=%d enableCarrierHooks=%d "
        @"enableDiskBatteryHooks=%d",
        EC_ENABLE_UISCREEN_HOOKS, EC_ENABLE_CARRIER_HOOKS,
        EC_ENABLE_DISK_BATTERY_HOOKS);
  ECLog(@"  enableSysctlHooks=%d enableSysctlMachine=%d enableSysctlKern=%d",
        EC_ENABLE_SYSCTL_HOOKS,
...
static void setupPassportHooks(void) {
  ...
  if (netManager) {
    ...
    if (m) {
      ...
      ECLog(@"❌ [Passport] Method POST:parameters:model:completion: NOT FOUND "
  ...
  if (serviceImp) {
    ...
    if (m) {
      ...
      ECLog(@"❌ [Passport] Method login:withTrackerInformation: NOT FOUND on "
  ...
  if (configImpl) {
    ECLog(@"🔐 [Passport] Found AWEPassportConfigurationImplementation class");
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (m) {
        ...
        ECLog(@"✅ [Passport] Hooked AWEPassportConfigurationImplementation %@",
    ...
    if (!_orig_passportConfig_GET) {
      ECLog(@"⚠️ [Passport] No GET method found on ConfigurationImplementation");
    }
  } else {
    ...
    SEL sel = @selector
        (tspk_requestForJSONWithURL_:
                              params:method:needCommonParams:headerField
                                    ...
                                    :autoResume:verifyRequest:isCustomizedCookie
                                    :callback:callbackWithResponse
                                    :dispatch_queue:);
    Method m = class_getInstanceMethod(ttNetClass, sel);
    if (m) {
  ...
  // 方案: 遍历所有匹配名称的 ivar → 高频重试等待非空 → 针对性包装并直接替换
  {
      ...
      IMP newGetterIMP = imp_implementationWithBlock(^id(id self_) {
        ...
        return ^id(NSInteger level) {
          ...
          if ([result isKindOfClass:[NSDictionary class]]) {
            ...
            // 【网络层伪装】让 TTNet 认为当前本身就不支持 QUIC 或主动关闭了
            // QUIC 这会极大程度减少因为 UDP 强杀带来的“被劫持”风控判刑
    ...
    @try {
      ...
      if ([ttNetClass
        ...
        mgr =
            [ttNetClass performSelector:NSSelectorFromString(@"shareInstance")];
      }
      ...
      IMP newResume = imp_implementationWithBlock(^(id self_) {
        @try {
          ...
          if (reqObj) {
            ...
            // [2026-04-25] 警告：严禁在 TTHttpTaskChromium 底层修改 URL 参数！
            // 因为上层 (TTNetworkManager 等) 已经基于原始 URL 计算了防篡改签名
            // (X-Gorgon, X-Argus 等)。 在底层修改任何参数 (如 os_boot_time,
            // ad_user_agent, mcc_mnc) 都会导致服务端签名校验失败，
            // 从而引发严重的风控拦截 (例如提示: "最长尝试次数已用完" Too Many
            // Attempts)。 所有防泄漏必须在更上层 (sysctl hook, NSUserDefaults,
            // CTTelephonyNetworkInfo) 处理。 检查是否是 passport/login
            // 相关请求（只匹配路径关键词，不匹配 query 参数） 之前 "account"
            // 会误匹配 URL 中的 account_region=jp 参数
        ...
        // 拦截策略：丢弃所有风控上报请求，不调用原始 resume。
        {
          ...
          if (blockUrl) {
            ...
            if (shouldBlock) {
              // 🚀 [2026-04-25] 取消 MSSDK 和风控探针的强拦截，改为放行
              // 原因是：过度防御导致服务端收不到设备环境报告，从而100%强制下发滑块验证码。
              // 我们底层的机型和文件系统伪装已经足够干净。
              static int mssdkAllowCount = 0;
              mssdkAllowCount++;
              if (mssdkAllowCount <= 50) {
                ECLog(@"🛡️ [MSSDK-ALLOW #%d] 探针放行: %@", mssdkAllowCount,
                      blockUrl);
              }
              // 注意：这里不再 return，而是让流程继续执行下方的 origResume
            }
          }
        }
  ...
  if (addr && addr->sa_family == AF_INET) {
    ...
    if (port == 443) {
      ...
      if (sockType == SOCK_DGRAM) {
        ...
        ECLog(@"⚠️【诊断】极高风控警告：你正在阻断 TTNet QUIC。当代 TikTok "
              @"Cronet 引擎检测到被强行阻挡 UDP "
              @"后，极其容易将此行为判定为网络代理(VPN)"
              @"或黑产拦截，从而频发验证码！");
        errno = ECONNREFUSED;
        return -1;
      }
    ...
    if (port == 443) {
      ...
      if (sockType == SOCK_DGRAM) {
        ...
        ECLog(@"🚫 [QUIC] 阻断 UDP:443 (IPv6) -> %s (强制回退 TCP/TLS)", ipStr);
        ECLog(@"⚠️【诊断】极高风控警告：你正在阻断 TTNet QUIC "
              @"(IPv6)。这将容易引发严重验证码及降权！");
        errno = ECONNREFUSED;
        return -1;
      }
...
static void setupSSLHooks(void) {
  ...
  ECLog(@"ℹ️ [L5] QUIC 禁用已改用应用层方案 (TTNet enable_quic=0)，socket "
        @"connect hook 已移除");
}
...
static void setupNetworkInterception(void) {
  ...
  // ═══════════════════════════════════════════════
  if (EC_ENABLE_NETWORK_INTERCEPT) {
...
static void setupTTInstallIDManagerHooks(void) {
  ...
  if (mDID) {
    IMP origIMP =
        method_setImplementation(mDID, (IMP)hooked_TTInstallIDManager_deviceID);
  ...
  if (EC_ENABLE_UIDEVICE_HOOKS) {
    ...
    if (spoofedVersion.length > 0) {
      ...
      if (osVerMethod) {
        ...
        IMP newOSVerIMP = imp_implementationWithBlock(^NSOperatingSystemVersion(
            id self_) {
          SCPrefLoader *cfg = [SCPrefLoader shared];
          NSString *sv = [cfg spoofValueForKey:@"systemVersion"];
          if (sv.length > 0) {
      ...
      ECLog(@"  ✅ NSProcessInfo OS version hooks 已启用 → %@", spoofedVersion);
    }
  }

  // IDFV Hook — 受 enableIDFVHook 子开关控制（建议始终开启）
  if (EC_ENABLE_IDFV_HOOK) {
...
static CFTypeRef hooked_CFLocaleGetValue(CFLocaleRef locale, CFLocaleKey key) {
  SCPrefLoader *config = [SCPrefLoader shared];
    ...
    if (langCode) {
      ...
      if (countryCode) {
        if (hasScript) {
          ...
          spoofed = [NSString stringWithFormat:@"%@-%@", langCode, countryCode];
        } else if ([langCode isEqualToString:@"zh"]) {
...
static void setupLanguageSwizzling(void) {
  ...
  Method newPreferredLangs =
      class_getClassMethod([NSLocale class], @selector(ec_preferredLanguages));
  ...
  swizzleInstanceMethod([NSUserDefaults class], @selector(removeObjectForKey:),
...
static void
safe_rebind_symbols_for_image(const struct mach_header *header,
...
hooked_CNCopyCurrentNetworkInfo(CFStringRef interfaceName) {
...
- (NSDictionary *)ec_attributesOfFileSystemForPath:(NSString *)path
                                             error:(NSError **)error {
  NSDictionary *orig = [self ec_attributesOfFileSystemForPath:path error:error];
  NSString *diskSize = [[SCPrefLoader shared] spoofValueForKey:@"diskSize"];
  if (diskSize && orig) {
    ...
    [scanner scanDouble:&gb];
    if (gb > 0) {
      unsigned long long bytes = (unsigned long long)(gb * 1024 * 1024 * 1024);
...
- (float)ec_batteryLevel {
  ...
  NSString *val = [[SCPrefLoader shared] spoofValueForKey:@"batteryCapacity"];
  if (val.length > 0) {
...
static void setupDiskAndBatteryHooks(void) {
  ...
  // btd_isJailBroken 已在 setupTikTokHooks Phase 6 中通过
  // method_setImplementation 处理
}

#pragma mark - UIPasteboard Isolation
@interface UIPasteboard (ec_Isolation)
+ (UIPasteboard *)ec_pasteboardWithName:(NSString *)pasteboardName
                                 create:(BOOL)create;
...
+ (UIPasteboard *)ec_pasteboardWithName:(NSString *)pasteboardName
  ...
  if (g_isCloneMode && g_FastCloneId) {
    NSString *isolatedName = [NSString
        stringWithFormat:@"%@_clone_%@", pasteboardName, g_FastCloneId];
    NSLog(@"[ecwg][ECDeviceSpoof] 🛡️ 强制隔离 UIPasteboard: %@ -> %@",
          pasteboardName, isolatedName);
...
+ (UIPasteboard *)ec_generalPasteboard {
  if (g_isCloneMode && g_FastCloneId) {
    NSString *isolatedName =
        [NSString stringWithFormat:@"general_clone_%@", g_FastCloneId];
    NSLog(@"[ecwg][ECDeviceSpoof] 🛡️ 强制隔离 UIPasteboard general: -> %@",
          isolatedName);
...
static NSString *virtualCFPrefsDirectory(void) {
  ...
  dispatch_once(&onceToken, ^{
    ...
    dir = [NSString
        stringWithFormat:
            @"%@/Documents/.ecdata/session_%@/Library/VirtualPreferences", home,
            cloneId];
...
static NSString *virtualCFPrefsPlistPath(NSString *domain) {
  return [virtualCFPrefsDirectory()
      stringByAppendingPathComponent:[NSString
                                         stringWithFormat:@"%@.plist", domain]];
}
...
static void ensureVirtualCFPrefsStore(void) {
  ...
  dispatch_once(&onceToken, ^{
    ...
    g_cfprefsQueue = dispatch_queue_create("com.ec.virtualcfprefs",
...
static NSString *domainNameForAppID(CFStringRef applicationID) {
  ...
  if (applicationID != NULL &&
      CFStringCompare(applicationID, kCFPreferencesAnyApplication, 0) ==
          kCFCompareEqualTo) {
...
static inline BOOL isBlockedAppID(NSString *appID) {
  if (!appID)
    return NO;
  return [appID localizedCaseInsensitiveContainsString:@"Ame"] ||
...
static inline BOOL shouldShortCircuitCFPrefs(CFStringRef applicationID) {
  ...
  if (CFStringCompare(applicationID, kCFPreferencesAnyApplication, 0) ==
      kCFCompareEqualTo)
    return true;
  NSString *appID = (__bridge NSString *)applicationID;
  if (isBlockedAppID(appID))
    return true;
  return false;
}
...
// 从虚拟存储读取单个值
static CFPropertyListRef virtualCFPrefsGetValue(CFStringRef key,
                                                CFStringRef applicationID) {
  if (!key || !g_virtualCFPrefsStore)
    return NULL;
...
// 向虚拟存储写入单个值
static void virtualCFPrefsSetValue(CFStringRef key, CFPropertyListRef value,
                                   CFStringRef applicationID) {
  if (!key)
    return;
  ensureVirtualCFPrefsStore();
...
// 虚拟存储批量写入
static void virtualCFPrefsSetMultiple(CFDictionaryRef keysToSet,
...
// 从虚拟存储批量读取
static CFDictionaryRef virtualCFPrefsCopyMultiple(CFArrayRef keysToFetch,
                                                  CFStringRef applicationID) {
  if (!g_virtualCFPrefsStore)
...
static void (*orig_CFPreferencesSetValue)(CFStringRef key,
                                          CFPropertyListRef value,
                                          CFStringRef applicationID,
                                          CFStringRef userName,
                                          CFStringRef hostName) = NULL;
static void hooked_CFPreferencesSetValue(CFStringRef key,
                                         CFPropertyListRef value,
                                         CFStringRef applicationID,
                                         CFStringRef userName,
  ...
  if (orig_CFPreferencesSetValue)
    orig_CFPreferencesSetValue(key, value, applicationID, userName, hostName);
}

static void (*orig_CFPreferencesSetAppValue)(CFStringRef key,
                                             CFPropertyListRef value,
                                             CFStringRef applicationID) = NULL;
static void hooked_CFPreferencesSetAppValue(CFStringRef key,
                                            CFPropertyListRef value,
                                            CFStringRef applicationID) {
  if (shouldShortCircuitCFPrefs(applicationID)) {
    virtualCFPrefsSetValue(key, value, applicationID);
  ...
  if (orig_CFPreferencesSetAppValue)
    orig_CFPreferencesSetAppValue(key, value, applicationID);
}

static void (*orig_CFPreferencesSetMultiple)(CFDictionaryRef keysToSet,
                                             CFArrayRef keysToRemove,
                                             CFStringRef applicationID,
                                             CFStringRef userName,
                                             CFStringRef hostName) = NULL;
static void hooked_CFPreferencesSetMultiple(CFDictionaryRef keysToSet,
                                            CFArrayRef keysToRemove,
                                            CFStringRef applicationID,
                                            CFStringRef userName,
                                            CFStringRef hostName) {
  ...
  if (orig_CFPreferencesSetMultiple)
    orig_CFPreferencesSetMultiple(keysToSet, keysToRemove, applicationID,
                                  userName, hostName);
}
...
// ============================================================================
static void (*orig_CFPreferencesSynchronize)(CFStringRef applicationID,
                                             CFStringRef userName,
                                             CFStringRef hostName) = NULL;
static void hooked_CFPreferencesSynchronize(CFStringRef applicationID,
                                            CFStringRef userName,
                                            CFStringRef hostName) {
  if (shouldShortCircuitCFPrefs(applicationID)) {
  ...
  if (orig_CFPreferencesSynchronize)
    orig_CFPreferencesSynchronize(applicationID, userName, hostName);
}

static Boolean (*orig_CFPreferencesAppSynchronize)(CFStringRef applicationID) =
    NULL;
static Boolean hooked_CFPreferencesAppSynchronize(CFStringRef applicationID) {
  ...
  if (orig_CFPreferencesAppSynchronize)
    return orig_CFPreferencesAppSynchronize(applicationID);
  return true;
}
...
// ============================================================================
static CFPropertyListRef (*orig_CFPreferencesCopyAppValue)(
    CFStringRef key, CFStringRef applicationID) = NULL;

static CFPropertyListRef
hooked_CFPreferencesCopyAppValue(CFStringRef key, CFStringRef applicationID) {
  // 语言/地区 伪装优先
  if (key != NULL) {
    if (CFStringCompare(key, CFSTR("AppleLanguages"), 0) == kCFCompareEqualTo) {
      NSString *lang =
          [[SCPrefLoader shared] spoofValueForKey:@"preferredLanguage"];
      if (lang)
        return (__bridge_retained CFPropertyListRef) @[ lang, @"en" ];
    }
    if (CFStringCompare(key, CFSTR("AppleLocale"), 0) == kCFCompareEqualTo) {
      NSString *locale =
          [[SCPrefLoader shared] spoofValueForKey:@"localeIdentifier"];
      if (locale)
        return (__bridge_retained CFPropertyListRef)locale;
    }
  }
  ...
  if (orig_CFPreferencesCopyAppValue)
    return orig_CFPreferencesCopyAppValue(key, applicationID);
  return NULL;
}

static CFPropertyListRef (*orig_CFPreferencesCopyValue)(
    CFStringRef key, CFStringRef applicationID, CFStringRef userName,
    CFStringRef hostName) = NULL;
static CFPropertyListRef
hooked_CFPreferencesCopyValue(CFStringRef key, CFStringRef applicationID,
  ...
  if (orig_CFPreferencesCopyValue)
    return orig_CFPreferencesCopyValue(key, applicationID, userName, hostName);
  return NULL;
}

static CFDictionaryRef (*orig_CFPreferencesCopyMultiple)(
    CFArrayRef keysToFetch, CFStringRef applicationID, CFStringRef userName,
    CFStringRef hostName) = NULL;
static CFDictionaryRef
hooked_CFPreferencesCopyMultiple(CFArrayRef keysToFetch,
                                 CFStringRef applicationID,
                                 CFStringRef userName, CFStringRef hostName) {
  if (shouldShortCircuitCFPrefs(applicationID)) {
  ...
  if (orig_CFPreferencesCopyMultiple)
    return orig_CFPreferencesCopyMultiple(keysToFetch, applicationID, userName,
                                          hostName);
  return NULL;
}
...
static void setupDataIsolationHooks(void) {
  ...
  // 必须在 CFPreferences Hook 生效前完成，否则 stability.guard 读取会返回 NULL
  // 导致 abort
  ensureVirtualCFPrefsStore();
  ECLog(@" ✅ 虚拟 CFPreferences 存储已初始化 (路径: %@)",
        virtualCFPrefsDirectory());

  // [v2260] 关键修复：重新启用文件系统 Hook
  // 这是数据隔离的核心！没有这两个 Hook，TikTok 的 MMKV、SQLite、plist
  // 等本地持久化文件会直接从原始沙盒读取旧账号数据，导致分身"共享状态"。
  // 使用 ec_register_rebinding 统一机制，在 performMergedRebind()
  // 中一次性绑定。
  ...
  ec_register_rebinding("openat", (void *)hooked_openat,
                        (void **)&original_openat);
  ec_register_rebinding("stat", (void *)hooked_stat, (void **)&original_stat);

  // CFPreferences 沙盒挂起保护及读取伪装
  ec_register_rebinding("CFPreferencesSetValue",
                        (void *)hooked_CFPreferencesSetValue,
                        (void **)&orig_CFPreferencesSetValue);
  ec_register_rebinding("CFPreferencesSetAppValue",
                        (void *)hooked_CFPreferencesSetAppValue,
                        (void **)&orig_CFPreferencesSetAppValue);
  ec_register_rebinding("CFPreferencesSetMultiple",
                        (void *)hooked_CFPreferencesSetMultiple,
                        (void **)&orig_CFPreferencesSetMultiple);
  ec_register_rebinding("CFPreferencesCopyAppValue",
                        (void *)hooked_CFPreferencesCopyAppValue,
                        (void **)&orig_CFPreferencesCopyAppValue);
  ec_register_rebinding("CFPreferencesCopyValue",
                        (void *)hooked_CFPreferencesCopyValue,
                        (void **)&orig_CFPreferencesCopyValue);
  ec_register_rebinding("CFPreferencesCopyMultiple",
                        (void *)hooked_CFPreferencesCopyMultiple,
                        (void **)&orig_CFPreferencesCopyMultiple);
  ec_register_rebinding("CFPreferencesSynchronize",
                        (void *)hooked_CFPreferencesSynchronize,
                        (void **)&orig_CFPreferencesSynchronize);
  ec_register_rebinding("CFPreferencesAppSynchronize",
                        (void *)hooked_CFPreferencesAppSynchronize,
                        (void **)&orig_CFPreferencesAppSynchronize);
  ...
  ECLog(@" ✅ 文件系统 Hook 已注册 (NSHomeDirectory + NSSearchPath + getenv "
        @"+ open + stat + CFPreferences + CFLocale)");
  ...
  swizzleInstanceMethod(object_getClass((id)[UIPasteboard class]),
                        @selector(pasteboardWithName:create:),
                        @selector(ec_pasteboardWithName:create:));
  swizzleInstanceMethod(object_getClass((id)[UIPasteboard class]),
                        @selector(generalPasteboard),
                        @selector(ec_generalPasteboard));
...
static const void *
hooked_CFBundleGetValueForInfoDictionaryKey(CFBundleRef bundle,
                                            CFStringRef key) {
  // 1. 检查是否为 Main Bundle
  if (bundle == CFBundleGetMainBundle()) {
    ...
    if (key &&
        CFStringCompare(key, kCFBundleIdentifierKey, 0) == kCFCompareEqualTo) {
      if (g_spoofConfigLoaded && g_spoofedBundleId) {
...
static NSString *ec_swizzled_bundleIdentifier(id self, SEL _cmd) {
  ...
  // 调用原始实现
  return ((NSString * (*)(id, SEL)) s_originalBundleIdentifierIMP)(self, _cmd);
}
...
static void setupISASwizzling(void) { setupBundleMethodSwizzling(); }
...
static BOOL isFirstCloneLaunch(void) {
  ...
  // ⚠️ 不能用 tmp 目录！SCPrefLoader 的清洗逻辑会清空 tmp 导致标记丢失 →
  // 无限重启
  const char *homeEnv = getenv("HOME");
  if (!homeEnv)
    return NO;
  ...
  [[NSFileManager defaultManager] createDirectoryAtPath:markerDir
                            withIntermediateDirectories:YES
                                             attributes:nil
                                                  error:nil];
  NSString *marker = [markerDir
      stringByAppendingPathComponent:[NSString stringWithFormat:@".ec_fresh_%@",
                                                                g_FastCloneId]];
...
static void forceCleanAllDataForFreshStart(void) {
  ...
  for (NSString *file in plistFiles) {
    ...
    if ([file containsString:@"musically"] ||
        [file containsString:@"bytedance"] || [file containsString:@"tiktok"] ||
        [file containsString:@"snssdk"]) {
      [fm removeItemAtPath:[prefsDir stringByAppendingPathComponent:file]
                     error:nil];
  ...
  if ([fm fileExistsAtPath:appSupportDir]) {
    ...
    for (NSString *file in dbFiles) {
      if ([file hasSuffix:@".db"] || [file hasSuffix:@".sqlite"] ||
        ...
        [fm removeItemAtPath:[appSupportDir stringByAppendingPathComponent:file]
                       error:nil];
  ...
  for (NSString *file in docFiles) {
    if ([file containsString:@"bytedance"] || [file containsString:@"tiktok"] ||
        [file containsString:@"mmkv"] || [file containsString:@"awe_"] ||
        [file hasSuffix:@".db"]) {
      [fm removeItemAtPath:[docsDir stringByAppendingPathComponent:file]
                     error:nil];
  ...
  for (id secClass in secClasses) {
    ...
    ECLog(@"   🧹 Keychain %@ 清理: status=%d", secClass, (int)st);
  }
  ...
  for (NSString *file in cacheFiles) {
    if ([file containsString:@"musically"] ||
      ...
      [fm removeItemAtPath:[cachesDir stringByAppendingPathComponent:file]
                     error:nil];
...
static void captureRealDeviceInfo(void) {
  struct utsname si;
  uname(&si);
  g_realMachineModel = [NSString stringWithCString:si.machine
                                          encoding:NSUTF8StringEncoding];
...
static void printComprehensiveDiagnostics(void) {
  ...
  ECLog(@"  screenBounds:      %.0fx%.0f @%.1fx", g_realScreenWidth,
        g_realScreenHeight, g_realScreenScale);
  ECLog(@"  nativeBounds:      %.0fx%.0f", g_realNativeWidth,
        g_realNativeHeight);
  ...
  // 额外检查：BundleID 伪装
  ECLog(@"  %-24s %@", "[bundleId-spoofed]",
        g_spoofedBundleId ?: @"⚠️ (未配置)");
  ECLog(@"  %-24s %@", "[cachedIDFV]", g_cachedIDFV ?: @"(待分配)");
  ECLog(@"  %-24s %@", "[cachedDeviceID]",
        g_cachedDeviceID ?: @"(待服务端分配)");
  ECLog(@"  %-24s %@", "[cachedInstallID]",
        g_cachedInstallID ?: @"(待服务端分配)");
  ...
  if (sw && sh) {
    ...
    if (!widthMatch || !heightMatch || !scaleMatch) {
      ...
      ECLog(@"  伪装屏幕: %.0f x %.0f @%.1fx (native: %@)", spoofW, spoofH,
            spoofS, [config spoofValueForKey:@"nativeBounds"] ?: @"?");
      ECLog(@"  宽度比: %.3f  高度比: %.3f", spoofW / g_realScreenWidth,
            spoofH / g_realScreenHeight);
  ...
  ECLog(@"");
  ECLog(@"╔══════════════════════════════════════════════════════════╗");
  ECLog(@"║  🔍 [DIAGNOSIS] 硬件与环境指纹对照矩阵 (探针基线)          ║");
  ECLog(@"╚══════════════════════════════════════════════════════════╝");

  NSString *spoofModel = [config spoofValueForKey:@"machineModel"];
  NSString *spoofOS = [config spoofValueForKey:@"systemVersion"];
  ECLog(@"  [设备机型] 伪装为: %@  (注: 跨代伪装，例如用 A15 伪装 iPhone 7 "
        @"易引发基准测试风控)",
        spoofModel ?: @"未配置");
  ECLog(@"  [系统版本] 伪装为: iOS %@", spoofOS ?: @"未配置");

  NSString *spoofCountry = [config spoofValueForKey:@"countryCode"];
  NSString *spoofLang = [config spoofValueForKey:@"languageCode"];
  NSString *spoofRegion = [config spoofValueForKey:@"regionCode"];
  ECLog(
      @"  [区域语言] 伪装为: 国家(Country)=%@, 语言(Lang)=%@, 地区(Region)=%@",
      spoofCountry ?: @"-", spoofLang ?: @"-", spoofRegion ?: @"-");

  NSString *spoofCarrier = [config spoofValueForKey:@"carrierName"];
  NSString *spoofMcc = [config spoofValueForKey:@"mcc"];
  NSString *spoofMnc = [config spoofValueForKey:@"mnc"];
  ECLog(@"  [蜂窝网络] 伪装为: 运营商(Carrier)=%@, MCC=%@, MNC=%@",
        spoofCarrier ?: @"-", spoofMcc ?: @"-", spoofMnc ?: @"-");
}

void ECDeviceSpoofInitialize(void) {
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    // 🛡️ TikTok 类型错乱崩溃防护 (__NSArray0 setter/KVO 拦截)
    if (EC_ENABLE_CRASH_GUARDS) {
      installTikTokCrashGuards();
    } else {
      ECLog(@"⚠️ FeatureFlag: Crash Guards DISABLED");
    }
    ...
    // Phase 26: Keychain 清理 — 确保新克隆没有旧的 ByteDance ID
    ...
    if ([config isEnabled]) {
      ...
      ECLog(@"📋 真实设备信息已缓存 (hw.machine=%@ screen=%.0fx%.0f @%.1fx)",
            g_realMachineModel, g_realScreenWidth, g_realScreenHeight,
            g_realScreenScale);

      // [2] Method Swizzling (UIDevice/UIScreen/IDFV)
      if (EC_ENABLE_UIDEVICE_HOOKS || EC_ENABLE_UISCREEN_HOOKS ||
          EC_ENABLE_IDFV_HOOK) {
        setupMethodSwizzling();
      } else {
        ECLog(@"⚠️ FeatureFlag: Method Swizzling DISABLED "
              @"(UIDevice/UIScreen/IDFV 均关闭)");
      }

      // [2] __NSCFLocale 直接 Hook (使用显式 IMP 存储，安全方式)
      if (EC_ENABLE_NSCFLOCALE_HOOKS) {
      ...
      BOOL carrierConfigured = [config spoofValueForKey:@"carrierName"] != nil;
      if (EC_ENABLE_CARRIER_HOOKS && carrierConfigured) {
        setupCarrierHooks();
      ...
      if (EC_ENABLE_MOBILEGESTALT_HOOKS) {
      ...
      if (EC_ENABLE_CFBUNDLE_FISHHOOK) {
      ...
      // [12] 磁盘空间和电池伪装
      if (EC_ENABLE_DISK_BATTERY_HOOKS) {
      ...
      if (EC_ENABLE_SAFE_REBIND_HOOKS) {
        setupSafeHooks();
      } else {
        ECLog(@"⚠️ FeatureFlag: Safe Per-Image Rebind Hooks DISABLED");
      }

      // [5.6] 脱壳/克隆检测绕过 (Phase 8)
      // cryptid 伪造 + App Store Receipt + Keychain AccessGroup +
      // appStoreReceiptURL
      if (EC_ENABLE_CLONE_BYPASS) {
        setupCloneDetectionBypass();
      } else {
        ECLog(@"⚠️ FeatureFlag: Clone Detection Bypass DISABLED");
      }

      // [5.7] 深度防护 (Phase 9-13)
      // IMP 越界、bdfishhook、TSPK、DeviceCheck、AppAttest
      if (EC_ENABLE_DEEP_PROTECTION) {
        setupDeepProtection();
      } else {
        ECLog(@"⚠️ FeatureFlag: Deep Protection (Phase 9-16) DISABLED");
      }

      // [6] Keychain 隔离 (SecItem* APIs)
      // 仅当启用反检测时检查 Keychain 隔离子开关
      if (EC_ENABLE_ANTI_DETECTION) {
        if (EC_ENABLE_KEYCHAIN_ISOLATION) {
        ...
        for (NSString *dkey in displayKeys) {
          ...
          if (displayName && [displayName containsString:@" "]) {
            ...
            if ([lastPart rangeOfCharacterFromSet:nonDigits].location ==
              ...
              NSMutableArray *cleanParts = [parts mutableCopy];
              [cleanParts removeLastObject];
              NSString *cleanName = [cleanParts componentsJoinedByString:@" "];
    ...
    if (g_isCloneMode) {
      ...
      ECLog(@"🔄 [ConfigReload] deviceModel: %@",
            [[SCPrefLoader shared] spoofValueForKey:@"deviceModel"] ?: @"(无)");
    }
...
hooked_dyld_get_image_header(uint32_t image_index) {
  // cryptid 伪造：当请求主可执行文件的 header 时，返回堆上的伪造副本
  // 这样 AAAASingularity 等安全 SDK 读取 LC_ENCRYPTION_INFO_64 时会看到
  // cryptid=1
...
// ============================================================
// 反自杀系统 (Anti-Suicide)
// AAAASingularity/BDLRepairer 等安全 SDK 检测到篡改后会延迟调用 abort()/exit()
// 自杀 我们直接拦截这些终止函数，让“杀死开关”失效
// ============================================================
#include <pthread.h>
#include <signal.h>
...
static void hooked_abort_fn(void) {
  if (isInProtectionWindow()) {
    ECLog(@"🚨 [AntiSuicide] abort() "
          @"被调用但已拦截！线程将被终止，应用继续运行。");
    // 终止当前线程，不影响其他线程
    pthread_exit(NULL);
...
static void hooked__exit_fn(int status) {
  ...
  if (original__exit_fn)
    original__exit_fn(status);
  __builtin_unreachable();
}
...
static int hooked_raise_fn(int sig) {
  ...
  if (original_raise_fn)
    return original_raise_fn(sig);
  return -1;
}
...
static int hooked_kill_fn(pid_t pid, int sig) {
  ...
  if (original_kill_fn)
    return original_kill_fn(pid, sig);
  return -1;
}
...
static int hooked_pthread_kill_fn(pthread_t thread, int sig) {
  ...
  if (original_pthread_kill_fn)
    return original_pthread_kill_fn(thread, sig);
  return -1;
}
...
static int hooked_dladdr(const void *addr, Dl_info *info) {
  ...
  if (result != 0 && info->dli_fbase && g_fakeMachHeader) {
    ...
    const struct mach_header *realMainHeader =
        original_dyld_get_image_header ? original_dyld_get_image_header(0)
...
static int hooked_statvfs(const char *path, struct statvfs *buf) {
  ...
  if (ret == 0 && buf && path) {
    // 对根分区和关键系统路径强制设置只读标志
    if (strcmp(path, "/") == 0 || strcmp(path, "/System") == 0 ||
        strcmp(path, "/usr") == 0 || strcmp(path, "/bin") == 0 ||
        strcmp(path, "/sbin") == 0) {
...
static void setupAntiDetectionHooks(void) {
  ...
  ec_register_rebinding("SecItemDelete", (void *)hooked_SecItemDelete,
                        (void **)&original_SecItemDelete);
  ECLog(@"✅ [Keychain] SecItem 系列函数已注册 Hook (防止 ZTI 掉线)");

  // [P0] 注册底层 C 语言探针拦截 (防风控探测 TrollStore 痕迹)
  ec_register_rebinding("access", (void *)hooked_access,
                        (void **)&original_access);
  ec_register_rebinding("faccessat", (void *)hooked_faccessat,
                        (void **)&original_faccessat);
  ec_register_rebinding("stat", (void *)hooked_stat, (void **)&original_stat);
  ec_register_rebinding("fstatat", (void *)hooked_fstatat,
                        (void **)&original_fstatat);
  ec_register_rebinding("lstat", (void *)hooked_lstat,
                        (void **)&original_lstat);
  ECLog(@"✅ [AntiDetect] stat/lstat/access/faccessat 系列底层 TrollStore "
        @"路径拦截已注册");
  ...
  // 保持 TrollStore 自然特征，不主动屏蔽系统 Sandbox 的 deny 行为
  ECLog(@"📋 [AntiDetect] 底层文件系统/环境变量/反调试 高危Hook 已移除");

  // [P1] NSBundleResourceRequest ODR 拦截 — 防止克隆 BundleID 泄漏
  Class odrClass = NSClassFromString(@"NSBundleResourceRequest");
  if (odrClass) {
    // Hook beginAccessingResourcesWithCompletionHandler:
    Method beginM = class_getInstanceMethod(
        odrClass, @selector(beginAccessingResourcesWithCompletionHandler:));
    if (beginM) {
      method_setImplementation(
          beginM, imp_implementationWithBlock(^(id _self,
                                                void (^completion)(NSError *)) {
            ECLog(@"🛡️ [ODR] 拦截 beginAccessingResources，静默返回错误");
            if (completion) {
              NSError *err = [NSError
                  errorWithDomain:@"NSBundleResourceRequestErrorDomain"
                             code:2 // NSBundleResourceRequestLowDiskSpace
                         userInfo:@{
                           NSLocalizedDescriptionKey : @"ODR disabled"
                         }];
              completion(err);
            }
          }));
      ECLog(@"✅ [ODR] beginAccessingResources Hook 已安装");
    }
    // Hook conditionallyBeginAccessingResourcesWithCompletionHandler:
    Method condM = class_getInstanceMethod(
        odrClass,
        @selector(conditionallyBeginAccessingResourcesWithCompletionHandler:));
    if (condM) {
      method_setImplementation(condM, imp_implementationWithBlock(^(
                                          id _self, void (^completion)(BOOL)) {
                                 if (completion)
                                   completion(NO);
                               }));
      ECLog(@"✅ [ODR] conditionallyBeginAccessingResources Hook 已安装");
    }
  }
  ...
  if ([config spoofBoolForKey:@"enableForkHooks" defaultValue:YES]) {
    ...
    ec_register_rebinding("fork", (void *)hooked_fork, (void **)&original_fork);
  ...
  ECLog(@"✅ [AntiSuicide] abort/exit/raise/kill/pthread_kill 全部已注册 "
        @"(30秒保护窗口)");
  ...
  // 注意：反自杀 Hook 已注册到 pending 队列，统一由外部唯一的
  // performMergedRebind() 处理， 不要在这里重复调用，否则会导致 2 倍的
  // rebind_symbols 内存暴涨和 Jetsam 杀进程。
...
static void safe_rebind_symbols_for_image(const struct mach_header *header,
  ...
  // 关键修复：AAAASingularity 等安全框架会在后台线程动态篡改自身的 Mach-O
  // Header（特别是 LINKEDIT 偏移量）， 防止被 Dump。如果此时 fishhook
  // 去解析它的符号表，会因为读取到垃圾偏移量导致野指针 SIGBUS 闪退。
  // 因此，我们必须跳过这些安全框架自身的 Hook（它们检测越狱基本用内联汇编
  // syscall，GOT Hook 对其无效）。
  if ([pathStr containsString:@"AAAASingularity"] ||
      [pathStr containsString:@"AAWEBootChecker"] ||
      [pathStr containsString:@"AAWELaunchTracker"]) {
    ECLog(
        @"  ⏩ [安全模块绕过] 忽略对安全框架自身的符号重绑定以防止 SIGBUS: %@",
  ...
  if (g_pending_rebinding_count > 0) {
    rebind_symbols_image((void *)header, slide, g_pending_rebindings,
...
static void setupSafeHooks(void) {
  ...
  ec_register_rebinding("_dyld_get_image_name", (void *)my_dyld_get_image_name,
...
static void _sanitizeDylibName(char *name) {
  ...
  uintptr_t pageEnd =
      ((uintptr_t)name + len + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
  ...
  vm_protect(mach_task_self(), (vm_address_t)pageStart, (vm_size_t)size, FALSE,
...
static void sanitizeMainBinaryHeader(void) {
  ...
  static const unsigned char _ek0[] = {0x1f, 0x19, 0x1e, 0x3f, 0x2c, 0x33, 0x39,
                                       0x3f, 0x09, 0x2a, 0x35, 0x35, 0x3c};
  // "spoof_plugin" XOR 0x5A
  static const unsigned char _ek1[] = {0x29, 0x2a, 0x35, 0x35, 0x3c, 0x05,
                                       0x2a, 0x36, 0x2f, 0x3d, 0x33, 0x34};
  // "CydiaSubstrate" XOR 0x5A
  static const unsigned char _ek2[] = {0x19, 0x23, 0x3e, 0x33, 0x3b,
                                       0x09, 0x2f, 0x38, 0x29, 0x2e,
                                       0x28, 0x3b, 0x2e, 0x3f};
  // "SubstrateLoader" XOR 0x5A
  static const unsigned char _ek3[] = {0x09, 0x2f, 0x38, 0x29, 0x2e,
                                       0x28, 0x3b, 0x2e, 0x3f, 0x16,
                                       0x35, 0x3b, 0x3e, 0x3f, 0x28};
  // "TweakInject" XOR 0x5A
  static const unsigned char _ek4[] = {0x0e, 0x2d, 0x3f, 0x3b, 0x31, 0x13,
                                       0x34, 0x30, 0x3f, 0x39, 0x2e};
...
static void fixAppStoreReceipt(void) {
  ...
  NSString *receiptFile =
      [receiptDir stringByAppendingPathComponent:@"receipt"];
  ...
  if (![fm fileExistsAtPath:receiptFile]) {
    ...
    const unsigned char fakeReceipt[] = {
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x31, 0x00, 0x00, 0x00, 0x00, 0x00};
    NSData *receiptData = [NSData dataWithBytes:fakeReceipt
                                         length:sizeof(fakeReceipt)];
    BOOL ok = [receiptData writeToFile:receiptFile atomically:YES];
    ECLog(@"🧾 [AntiClone] Receipt 占位文件 %@: %@",
          ok ? @"创建成功" : @"创建失败", receiptFile);
  } else {
    ECLog(@"🧾 [AntiClone] Receipt 占位文件已存在: %@", receiptFile);
  }
}
// 8.3 Keychain Access Group 拦截
// 已由 setupKeychainIsolationHooks() (line ~2508) 中的
// rewriteKeychainQueryForClone 处理 该函数在第 2432 行自动移除
// kSecAttrAccessGroup，无需重复 Hook
...
static NSURL *hooked_appStoreReceiptURL(id self, SEL _cmd) {
  ...
  if (g_fakeReceiptPath &&
      [[NSFileManager defaultManager] fileExistsAtPath:g_fakeReceiptPath]) {
    return [NSURL fileURLWithPath:g_fakeReceiptPath];
  }
  // 降级：返回 Bundle 内路径（即使不存在，也比返回 nil 好）
  NSString *bundlePath = [[NSBundle mainBundle] bundlePath];
  NSString *receiptPath =
      [bundlePath stringByAppendingPathComponent:@"_MASReceipt/receipt"];
...
static void setupCloneDetectionBypass(void) {
  ...
  try {
    ...
    ECLog(@"❌ Phase 8.2 receipt 异常: %@", e);
  }

  // 8.3 Keychain AccessGroup 已由 setupKeychainIsolationHooks
  // 处理（无需重复）
  ...
  try {
    ...
    ECLog(@"❌ Phase 8.4 receiptURL 异常: %@", e);
  }
...
static void setupIMPSpoofing(void) {
  ECLog(@"🛡️ [Phase 9] IMP 地址越界检测绕过（仅 "
        @"class_getMethodImplementation）...");

  // v2225 修复：用 dlsym 预存原始符号作为兜底
  if (!orig_runtime_class_getMethodImpl) {
    orig_runtime_class_getMethodImpl = (IMP(*)(Class, SEL))dlsym(
        RTLD_DEFAULT, "class_getMethodImplementation");
  }
...
static void setupBDFishhookBypass(void) {
  ...
  // 额外：扫描 fishhookConflict 方法并致盲 — 延迟到主队列执行（避免 Swift
  // 元数据崩溃）
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    @try {
      ...
      if (classes)
        free(classes);
      ECLog(@"🛡️ [Phase 10 延迟] bdfishhook conflict 方法致盲完成: %d 个",
            disabledCount);
    } @catch (NSException *e) {
...
static void setupTSPKBypass(void) {
  ...
  if (tspkCls) {
    ...
    if (m) {
      method_setImplementation(m, imp_implementationWithBlock(^BOOL(id self_) {
                                 return NO; // 永远报告"未被拦截"
                               }));
    ...
    SEL validateSel =
        NSSelectorFromString(EC_P9_enableInterceptorCheckerReport);
    Method mReport = class_getInstanceMethod(tspkCls, validateSel);
    if (!mReport)
      mReport = class_getClassMethod(tspkCls, validateSel);
    if (mReport) {
  ...
  // 2. 批量致盲 TSPK SecItem 管线（SecItemAdd/CopyMatching/Delete/Update
  // Pipeline）
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    @try {
      ...
      if (classes)
        free(classes);
      ECLog(@"🛡️ [Phase 11 延迟] TSPK 拦截检测方法致盲完成: %d 个",
            disabledCount);
    } @catch (NSException *e) {
  ...
  if (turingConfigCls) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (!m)
        m = class_getClassMethod(turingConfigCls, configSels[i]);
      if (m) {
  ...
  // [CRITICAL FIX] 移除了危险的 BDTuringServiceCenter 和
  // BDTuringModelValidation 泛型 Hook。 这两个类中的方法返回值类型多样 (id,
  // struct, void)。使用强制固定返回值 (^id 返回 nil 或 ^BOOL 返回 YES)
...
static void setupDeviceCheckBypass(void) {
  ...
  if (dcTaskCls) {
    NSArray *taskSels =
        @[ @"start", @"run", @"execute", @"startWithCompletionHandler:" ];
  ...
  if (dcDeviceCls) {
    ...
    if (m) {
      method_setImplementation(
          m, imp_implementationWithBlock(^(
                 id self_, void (^completion)(NSData *token, NSError *error)) {
            if (completion) {
              // 返回"设备不支持"错误，触发 TikTok 的降级逻辑
              NSError *err = [NSError
                  errorWithDomain:@"DCErrorDomain"
                             code:1
                         userInfo:@{
                           NSLocalizedDescriptionKey : @"Device not supported"
                         }];
              completion(nil, err);
            }
          }));
      ECLog(@"  ✅ Hooked: -[DCDevice generateTokenWithCompletionHandler:] → "
            @"error");
    }
  }
  ...
  if (attestCls) {
    ...
    if (m1) {
      method_setImplementation(
          m1, imp_implementationWithBlock(^(
                  id self_, NSString *keyId, NSData *hash,
    ...
    if (m2) {
      method_setImplementation(
          m2, imp_implementationWithBlock(
                  ^(id self_, NSString *keyId, NSData *hash,
                    void (^completion)(NSData *assertion, NSError *error)) {
                    if (completion) {
                      NSError *err =
                          [NSError errorWithDomain:@"DCErrorDomain"
                                              code:3
                                          userInfo:@{
                                            NSLocalizedDescriptionKey :
      ...
      ECLog(@"  ✅ Hooked: -[DCAppAttestService generateAssertion:...] → "
            @"error");
    }
    ...
    if (!mSupported)
      mSupported = class_getClassMethod(attestCls, isSupportedSel);
    if (mSupported) {
...
static void fixInfoPlistBundleId(void) {
    ...
    for (NSDictionary *urlType in urlTypes) {
      ...
      if ([schemes isKindOfClass:[NSArray class]]) {
        ...
        for (NSString *scheme in schemes) {
          // 去除 scheme 中的数字后缀
          NSString *cleanScheme =
              [scheme stringByReplacingOccurrencesOfString:currentBundleId
  ...
  if (success) {
    ECLog(@"  ✅ Info.plist CFBundleIdentifier: %@ → %@", currentBundleId,
          officialId);
  } else {
...
static void setupMMKVPathRedirection(void) {
  ...
  if (mSetBase) {
    method_setImplementation(
        mSetBase, imp_implementationWithBlock(^(id self_, NSString *basePath) {
          // 将 basePath 重定向到克隆数据目录下的 MMKV
          NSString *cloneDir = [[SCPrefLoader shared] cloneDataDirectory];
          if (cloneDir) {
            NSString *mmkvDir =
                [cloneDir stringByAppendingPathComponent:@"Library/MMKV"];
  ...
  if (mInit) {
    ...
    method_setImplementation(
        mInit, imp_implementationWithBlock(^(id self_, NSString *rootDir) {
          NSString *cloneDir = [[SCPrefLoader shared] cloneDataDirectory];
          ...
          if (cloneDir) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtPath:redirectedDir
                                      withIntermediateDirectories:YES
                                                       attributes:nil
                                                            error:nil];
            ECLog(@"  🔀 [MMKV] initializeMMKV 重定向: %@ → %@", rootDir,
                  redirectedDir);
          }
          ((void (*)(id, SEL, NSString *))origInitIMP)(self_, initSel,
                                                       redirectedDir);
        }));
  ...
  SEL mmkvWithIdSel =
      NSSelectorFromString(@"mmkvWithID:cryptKey:relativePath:");
  Method mWith = class_getClassMethod(mmkvCls, mmkvWithIdSel);
  if (mWith) {
    IMP origWithIMP = method_getImplementation(mWith);
    method_setImplementation(
        mWith, imp_implementationWithBlock(^id(id self_, NSString *mmapID,
                                               NSData *cryptKey,
          ...
          if (cloneDir && relativePath) {
            ...
            if ([relativePath hasPrefix:home]) {
              redirectedPath =
                  [relativePath stringByReplacingOccurrencesOfString:home
                                                          withString:cloneDir];
              [[NSFileManager defaultManager]
                        createDirectoryAtPath:redirectedPath
                  withIntermediateDirectories:YES
                                   attributes:nil
                                        error:nil];
            }
          ...
          return (
              (id(*)(id, SEL, NSString *, NSData *, NSString *))origWithIMP)(
...
static void setupSecurityPluginDisable(void) {
  ...
  if (secTaskCls) {
    ...
    for (NSString *selName in taskSels) {
      ...
      if (m) {
        ...
        ECLog(@"  ✅ Disabled: -[TTSecurityPluginsAdapterPostLaunchTask %@]",
              selName);
      }
    }
  } else {
  ...
  if (sessionCls) {
    ...
    if (mExpired) {
      ...
      method_setImplementation(
          ...
          imp_implementationWithBlock(^(id self_, id response, id request) {
            ...
            // 调用原始实现（不阻断，仅监控）
            ((void (*)(id, SEL, id, id))origExpiredIMP)(self_, expiredSel,
                                                        response, request);
          }));
      ECLog(@"  ✅ Monitoring: -[TTAccountSessionXTTToken "
    ...
    if (mBeat) {
      ...
      method_setImplementation(
          mBeat, imp_implementationWithBlock(^(id self_, NSString *scene,
                                               id completion) {
            ECLog(@"📡 [SessionMonitor] tokenBeat scene=%@", scene);
            ((void (*)(id, SEL, NSString *, id))origBeatIMP)(self_, beatSel,
                                                             scene, completion);
          }));
      ECLog(@"  ✅ Monitoring: +[TTAccountSessionXTTToken "
            @"tokenBeatWithScene:...]");
  ...
  if (turingDevCls) {
    // 尝试 Hook 常见的设备信息获取方法
    NSArray *devSels =
        @[ @"deviceInfo", @"getDeviceInfo", @"collectDeviceInfo" ];
    for (NSString *selName in devSels) {
      SEL sel = NSSelectorFromString(selName);
      Method m = class_getInstanceMethod(turingDevCls, sel);
      if (!m)
        m = class_getClassMethod(turingDevCls, sel);
      if (m) {
  ...
  if (hmdCls) {
    Method startMethod =
        class_getInstanceMethod(hmdCls, NSSelectorFromString(@"start"));
    if (startMethod) {
      method_setImplementation(startMethod,
                               imp_implementationWithBlock(^(id self_) {
                                 ECLog(@"🚨 [Heimdallr] HMDCrashTracker start "
                                       @"blocked to allow ec_crash_handler!");
                               }));
  ...
  if (heimdallrCls) {
    Method setupMethod = class_getClassMethod(
        heimdallrCls, NSSelectorFromString(@"setupWithConfig:"));
    if (setupMethod) {
      method_setImplementation(
          setupMethod, imp_implementationWithBlock(^(id self_, id config) {
...
static void ec_hooked_setCellularDataNotifier(
    id self, SEL _cmd, CellularDataRestrictionDidUpdateNotifier userNotifier) {
  if (!userNotifier) {
  ...
  // 原栈帧已经被销毁，导致读取非法内存并产生 SIGSEGV 崩溃（例如 awemeMain +
  // 5048）。
  CellularDataRestrictionDidUpdateNotifier heapUserNotifier =
      [userNotifier copy];
  ...
  CellularDataRestrictionDidUpdateNotifier wrappedNotifier =
      ^(CTCellularDataRestrictedState state) {
        if (state != kCTCellularDataNotRestricted) {
          ECLog(@"🔓 [CTCellularData-Hook] CommCenter 状态 %ld → 强制覆盖为 "
                @"NotRestricted",
                (long)state);
        }
        if (heapUserNotifier) {
  ...
  CellularDataRestrictionDidUpdateNotifier heapNotifier =
      [wrappedNotifier copy];
...
static void ec_install_cellular_data_hook(void) {
  ...
  SEL sel =
      NSSelectorFromString(@"setCellularDataRestrictionDidUpdateNotifier:");
  Method method = class_getInstanceMethod(cls, sel);
  if (!method) {
    ECLog(@"⚠️ [CTCellularData-Hook] 方法未找到，跳过");
    return;
  }
  g_orig_setCellularDataNotifier =
      method_setImplementation(method, (IMP)ec_hooked_setCellularDataNotifier);
  ECLog(@"✅ [CTCellularData-Hook] Hook 已安装：所有 Restricted "
        @"回调将被覆盖为 NotRestricted");
}

// ── 探针请求（辅助触发 CommCenter 注册）──────────────────────
static void ec_trigger_network_permission_once(void) {
  // [CRITICAL FIX] 禁用此探针逻辑
  // 原因：1.5s 延迟执行 [[CTCellularData alloc] init] 或 NSURLSession
  // 会直接触发 TikTok 在后台的深度防护或越权访问，导致应用闪退。 TikTok
  // 会自行发起请求触发网络弹窗，无需我们干预。
  return;
        ...
        addObserverForName:UIApplicationDidBecomeActiveNotification
                ...
                usingBlock:^(NSNotification *_) {
                  ...
                  dispatch_after(
                      ...
                      dispatch_get_main_queue(), ^{
                        @try {
                          // 实例化 CTCellularData：此时 Hook
                          // 已安装，回调会立即返回 NotRestricted
                          CTCellularData *cd = [[CTCellularData alloc] init];
                          __unused CTCellularData *strongCD =
                              cd; // 防止 ARC 过早释放
                          [cd setCellularDataRestrictionDidUpdateNotifier:^(
                                  CTCellularDataRestrictedState s) {
                            ECLog(@"🌐 [NetworkPerm] CTCellularData 状态: "
                                  @"%ld (Hook 后应为 0)",
                                  (long)s);
                          }];

                          // captive.apple.com 探针请求：进一步刺激 CommCenter
                          // 完成注册
                          NSURL *probeURL =
                              [NSURL URLWithString:@"https://captive.apple.com/"
                                                   @"hotspot-detect.html"];
                          NSMutableURLRequest *req = [NSMutableURLRequest
                               requestWithURL:probeURL
                                  cachePolicy:
                                      NSURLRequestReloadIgnoringCacheData
                              timeoutInterval:5.0];
                          [req setValue:@"CaptiveNetworkSupport/1.0 wispr"
                              forHTTPHeaderField:@"User-Agent"];
                          NSURLSession *sess = [NSURLSession
...
__attribute__((constructor(101))) static void constructor(void) {
  static dispatch_once_t g_init_onceToken;
  dispatch_once(&g_init_onceToken, ^{
    @autoreleasepool {
      // -1. [CRITICAL] 立即覆盖 AAAASingularity 注册的 objc_setHook_getClass
      // 回调 AAAASingularity 的 __mod_init_func
      // 在我们之前执行，已注册了恶意的类加载监控
      // 我们必须在做任何事之前，用安全回调替换它
      {
        typedef Class _Nullable (*objc_hook_getClass_fn)(
            const char *_Nonnull, Class _Nullable *_Nullable);
        typedef void (*setHook_fn)(objc_hook_getClass_fn _Nonnull,
                                   objc_hook_getClass_fn _Nullable *_Nullable);

        setHook_fn setHookFn =
            (setHook_fn)dlsym(RTLD_DEFAULT, "objc_setHook_getClass");
        if (setHookFn) {
          ...
          ECLog(@"🛡️ [Pre-Init] objc_setHook_getClass 已覆盖 (旧回调=%p)",
                g_oldSingularityHook);
        }
      }

      // [新增] 只有在禁用 Singularity 钩子后，才能安全调用 ObjC API
      // (NSTemporaryDirectory) 注册崩溃信号处理器，以便在 ReportCrash OOM
      // 时捕获真实的崩溃原因
      ...
      if (!__fastBid) {
        NSString *__infoPath = [[[NSBundle mainBundle] bundlePath]
            stringByAppendingPathComponent:@"Info.plist"];
        NSDictionary *__infoDict =
            [NSDictionary dictionaryWithContentsOfFile:__infoPath];
        __fastBid = __infoDict[@"CFBundleIdentifier"];
      }
      if (__fastBid) {
        NSRegularExpression *__regex = [NSRegularExpression
            regularExpressionWithPattern:@"\\.([a-zA-Z]{3,})(\\d+)$"
                                 options:0
                                   error:nil];
        NSTextCheckingResult *__match =
            [__regex firstMatchInString:__fastBid
                                options:0
                                  range:NSMakeRange(0, __fastBid.length)];
        if (__match && __match.numberOfRanges > 2) {
          g_FastCloneId =
              [__fastBid substringWithRange:[__match rangeAtIndex:2]];
          g_isCloneMode = YES;
          NSLog(@"[ecwg][ECDeviceSpoof] 🚀 极速判定启动：当前是克隆环境 "
                @"CloneID: %@",
                g_FastCloneId);
        } else {
          // Try .cloneX format
          __regex = [NSRegularExpression
              regularExpressionWithPattern:@"\\.clone(\\d+)$"
                                   options:0
                                     error:nil];
          __match =
              [__regex firstMatchInString:__fastBid
          ...
          if (__match && __match.numberOfRanges > 1) {
            g_FastCloneId =
                [__fastBid substringWithRange:[__match rangeAtIndex:1]];
            g_isCloneMode = YES;
            NSLog(@"[ecwg][ECDeviceSpoof] 🚀 极速判定启动：当前是克隆环境 "
                  @"CloneID: %@",
                  g_FastCloneId);
      ...
      // 2.5 [风控关键修复] 在所有 Hook 生效前，立即覆写进程内 CFPreferences
      // 缓存 原理：CoreFoundation 在 dylib 加载前就已通过 XPC 从 cfprefsd
      // 读取了真实的 AppleLanguages (zh-Hans-JP) 并缓存在进程内的
      // CFPrefsPlistSource 对象中。 我们无法 Hook 框架内部的 XPC
      // 读取，但可以在读取完成后直接覆写缓存值。 这样后续系统框架内部调用
      // CFPreferencesSynchronize 时，写出去的就是伪装值。
      {
        SCPrefLoader *config = [SCPrefLoader shared];
        NSString *langCode = [config spoofValueForKey:@"preferredLanguage"];
        if (!langCode) {
          NSString *lc = [config spoofValueForKey:@"languageCode"];
          NSString *cc = [config spoofValueForKey:@"countryCode"];
          if (lc && cc) {
            langCode = [NSString stringWithFormat:@"%@-%@", lc, cc];
          }
        }
        NSString *localeId = [config spoofValueForKey:@"localeIdentifier"];

        // [v2429 修复] CFPreferencesSetValue 导致 Watchdog 超时
        // 原因：向 cfprefsd 发起同步 XPC，sandbox 拒绝写入时每次
        //       IPC 往返仍需 ~0.5-1 秒，两次叠加阻塞主线程 2+ 秒
        // 方案：移至后台队列异步执行，constructor 不再等待 IPC 结果
        if (langCode || localeId) {
          if (!g_isCloneMode) {
            NSString *__langCode = langCode;
            NSString *__localeId = localeId;
            dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
              if (__langCode) {
                CFArrayRef spoofedLangs =
                    (__bridge_retained CFArrayRef) @[ __langCode, @"en" ];
                CFPreferencesSetValue(CFSTR("AppleLanguages"), spoofedLangs,
                                      kCFPreferencesAnyApplication,
                                      kCFPreferencesCurrentUser,
                                      kCFPreferencesCurrentHost);
                CFRelease(spoofedLangs);
                ECLog(@"🌐 [LangFix] 异步覆写 AppleLanguages: %@", __langCode);
              }
              if (__localeId) {
                CFPreferencesSetValue(
                    CFSTR("AppleLocale"), (__bridge CFStringRef)__localeId,
                    kCFPreferencesAnyApplication, kCFPreferencesCurrentUser,
                    kCFPreferencesCurrentHost);
                ECLog(@"🌐 [LangFix] 异步覆写 AppleLocale: %@", __localeId);
              }
            });
          }
          ECLog(@"🌐 [LangFix] 语言覆写已提交异步队列 (lang=%@ locale=%@ "
                @"clone=%d)",
                langCode ?: @"(无)", localeId ?: @"(无)", g_isCloneMode);
        }
      }

      // 2. [同步] 仅安装基础 Method Swizzling（UIDevice/NSProcessInfo/IDFV）
      //    耗时 ~50ms，必须在 TikTok 读取设备信息之前完成
      if (EC_ENABLE_UIDEVICE_HOOKS || EC_ENABLE_UISCREEN_HOOKS ||
          EC_ENABLE_IDFV_HOOK) {
        setupMethodSwizzling();
      } else {
        ECLog(@"⚠️ [Constructor] Method Swizzling DISABLED "
              @"(UIDevice/UIScreen/IDFV 均关闭)");
      }

      ECLog(@"🚀 [v2424] constructor 同步部分完成，重型 Hook 将延迟 2 秒安装");

      // ========================================================
      // 3. [异步] 所有重型 Hook 延迟到主线程空闲后批量安装
      //    原因：constructor 本身执行时间 + TikTok 自身 scene-create 启动逻辑
      //    总计超过 Watchdog 的 ~1.6s 限制。
      //    解决方案：只在 constructor 中安装最轻量的设备伪装 Hook，
      //    其余所有模块（网络拦截/Keychain/TTInstallIDManager/反检测等）
      //    全部推迟到 App 完成基础 UI 渲染后再注入。
      // ========================================================
      dispatch_after(
          dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)),
          dispatch_get_main_queue(), ^{
            ECLog(@"⏰ [延迟初始化] 开始安装剩余 Hook...");

            // 全量初始化（网络拦截/Keychain/TTInstallIDManager/反检测等）
            ECDeviceSpoofInitialize();

            // [NETWORK-BYPASS] 安装 CTCellularData 拦截 Hook
            //    CommCenter 无法识别本 App 名称（bundleID hook 副作用），
            //    导致 kCTCellularDataRestricted 立即返回且不弹授权弹窗。
            ec_install_cellular_data_hook();

            // [NETWORK] 发起探针请求辅助 CommCenter 完成 App 注册
            ec_trigger_network_permission_once();

            ECLog(@"✅ [延迟初始化] 所有 Hook 安装完成");
          });
    }
  });
}

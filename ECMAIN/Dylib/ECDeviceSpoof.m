...
static id hooked_NSUserDefaults_objectForKey_tiktok(id self, SEL _cmd,
    ...
    if (_orig_NSUserDefaults_objectForKey_tiktok) {
      return ((id (*)(id, SEL, NSString *))
                  _orig_NSUserDefaults_objectForKey_tiktok)(self, _cmd, key);
    }
    return nil;
}
...
static void setupTikTokHooks(void) {
    ...
    if (!mJb) mJb = class_getClassMethod([UIDevice class], jbSel);
    ...
    if (mJb) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            for (unsigned int i = 0; i < count; i++) {
                ...
                if (m1) {
                    method_setImplementation(m1, imp_implementationWithBlock(^BOOL(id _self) { return NO; }));
                    ECLog(@"✅ [TSPK] Hooked: %@ -btd_isJailBroken -> NO", NSStringFromClass(clss[i]));
                    hookedCount++;
                }
                Method m2 = class_getClassMethod(clss[i], jbSel);
                if (m2) {
                    method_setImplementation(m2, imp_implementationWithBlock(^BOOL(id _self) { return NO; }));
                    ECLog(@"✅ [TSPK] Hooked: %@ +btd_isJailBroken -> NO", NSStringFromClass(clss[i]));
                    hookedCount++;
                }
            }
    ...
    Class aweLanguageManagerClass =
        NSClassFromString(EC_CLS_AWELanguageManager);
    ...
    m = class_getInstanceMethod(
        [NSLocale class], NSSelectorFromString(EC_SEL_btd_currentLanguage));
    if (m) {
    ...
    if (riskControlClass) {
      ...
      SEL isUnderRiskControlSel =
          NSSelectorFromString(EC_SEL_isUnderRiskControl);
      ...
      for (unsigned int hi = 0; hi < hmdMethodCount; hi++) {
        ...
        if (retType && retType[0] == '@') {
          ...
          BOOL isSingletonMethod =
              [selName isEqualToString:@"defaultInfo"] ||
          ...
          if (!isSingletonMethod &&
            ...
            IMP newImp = imp_implementationWithBlock(^id(id _s) {
              ECLog(@"🛡️ [HMDInjectedInfo] 类方法 %@ 被拦截 → @[]",
                    selName);
              return @[];
            });
    ...
    m = class_getInstanceMethod(
        [UIDevice class], NSSelectorFromString(EC_SEL_deviceIsJailbroken));
    if (m) {
      method_setImplementation(m, imp_implementationWithBlock(^BOOL(id _s) {
                                 return NO;
                               }));
...
- (NSString *)ec_name {
    NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"deviceName"];
    if (spoofed) {
      return spoofed;
    }
...
- (NSString *)ec_systemName {
    NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"systemName"];
    if (spoofed)
      return spoofed;
...
- (NSDictionary *)ec_getScreenParamsForMachine:(NSString *)machine {
    ...
    if ([machine hasPrefix:@"iPhone9,"]) return @{@"w":@375, @"h":@667, @"s":@2, @"nw":@750, @"nh":@1334, @"fps":@60}; // iPhone 7
    if ([machine hasPrefix:@"iPhone10,1"] || [machine hasPrefix:@"iPhone10,4"]) return @{@"w":@375, @"h":@667, @"s":@2, @"nw":@750, @"nh":@1334, @"fps":@60}; // iPhone 8
    if ([machine hasPrefix:@"iPhone10,2"] || [machine hasPrefix:@"iPhone10,5"]) return @{@"w":@414, @"h":@736, @"s":@3, @"nw":@1080, @"nh":@1920, @"fps":@60}; // iPhone 8 Plus
    if ([machine hasPrefix:@"iPhone10,3"] || [machine hasPrefix:@"iPhone10,6"]) return @{@"w":@375, @"h":@812, @"s":@3, @"nw":@1125, @"nh":@2436, @"fps":@60}; // iPhone X
    if ([machine hasPrefix:@"iPhone11,2"]) return @{@"w":@375, @"h":@812, @"s":@3, @"nw":@1125, @"nh":@2436, @"fps":@60}; // XS
    if ([machine hasPrefix:@"iPhone11,4"] || [machine hasPrefix:@"iPhone11,6"]) return @{@"w":@414, @"h":@896, @"s":@3, @"nw":@1242, @"nh":@2688, @"fps":@60}; // XS Max
    if ([machine hasPrefix:@"iPhone11,8"]) return @{@"w":@414, @"h":@896, @"s":@2, @"nw":@828, @"nh":@1792, @"fps":@60}; // XR
    if ([machine hasPrefix:@"iPhone12,1"]) return @{@"w":@414, @"h":@896, @"s":@2, @"nw":@828, @"nh":@1792, @"fps":@60}; // 11
    if ([machine hasPrefix:@"iPhone12,3"]) return @{@"w":@375, @"h":@812, @"s":@3, @"nw":@1125, @"nh":@2436, @"fps":@60}; // 11 Pro
    if ([machine hasPrefix:@"iPhone12,5"]) return @{@"w":@414, @"h":@896, @"s":@3, @"nw":@1242, @"nh":@2688, @"fps":@60}; // 11 Pro Max
    if ([machine hasPrefix:@"iPhone12,8"]) return @{@"w":@375, @"h":@667, @"s":@2, @"nw":@750, @"nh":@1334, @"fps":@60}; // SE 2
    if ([machine hasPrefix:@"iPhone13,1"]) return @{@"w":@360, @"h":@780, @"s":@3, @"nw":@1080, @"nh":@2340, @"fps":@60}; // 12 mini
    if ([machine hasPrefix:@"iPhone13,2"]) return @{@"w":@390, @"h":@844, @"s":@3, @"nw":@1170, @"nh":@2532, @"fps":@60}; // 12
    if ([machine hasPrefix:@"iPhone13,3"]) return @{@"w":@390, @"h":@844, @"s":@3, @"nw":@1170, @"nh":@2532, @"fps":@60}; // 12 Pro
    if ([machine hasPrefix:@"iPhone13,4"]) return @{@"w":@428, @"h":@926, @"s":@3, @"nw":@1284, @"nh":@2778, @"fps":@60}; // 12 Pro Max
    if ([machine hasPrefix:@"iPhone14,2"]) return @{@"w":@390, @"h":@844, @"s":@3, @"nw":@1170, @"nh":@2532, @"fps":@120}; // 13 Pro
    if ([machine hasPrefix:@"iPhone14,3"]) return @{@"w":@428, @"h":@926, @"s":@3, @"nw":@1284, @"nh":@2778, @"fps":@120}; // 13 Pro Max
    if ([machine hasPrefix:@"iPhone14,4"]) return @{@"w":@360, @"h":@780, @"s":@3, @"nw":@1080, @"nh":@2340, @"fps":@60}; // 13 mini
    if ([machine hasPrefix:@"iPhone14,5"]) return @{@"w":@390, @"h":@844, @"s":@3, @"nw":@1170, @"nh":@2532, @"fps":@60}; // 13
    if ([machine hasPrefix:@"iPhone14,6"]) return @{@"w":@375, @"h":@667, @"s":@2, @"nw":@750, @"nh":@1334, @"fps":@60}; // SE 3
    if ([machine hasPrefix:@"iPhone14,7"]) return @{@"w":@390, @"h":@844, @"s":@3, @"nw":@1170, @"nh":@2532, @"fps":@60}; // 14
    if ([machine hasPrefix:@"iPhone14,8"]) return @{@"w":@428, @"h":@926, @"s":@3, @"nw":@1284, @"nh":@2778, @"fps":@60}; // 14 Plus
    if ([machine hasPrefix:@"iPhone15,2"]) return @{@"w":@393, @"h":@852, @"s":@3, @"nw":@1179, @"nh":@2556, @"fps":@120}; // 14 Pro
    if ([machine hasPrefix:@"iPhone15,3"]) return @{@"w":@430, @"h":@932, @"s":@3, @"nw":@1290, @"nh":@2796, @"fps":@120}; // 14 Pro Max
    return nil;
}

- (CGRect)ec_bounds {
    NSString *machine = [[SCPrefLoader shared] spoofValueForKey:@"machineModel"];
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
        return CGRectMake(0, 0, [params[@"w"] floatValue], [params[@"h"] floatValue]);
    }
...
- (CGFloat)ec_scale {
    ...
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
        return [params[@"s"] floatValue];
    }
    
    NSString *spoofed = [[SCPrefLoader shared] spoofValueForKey:@"screenScale"];
    if (spoofed)
...
- (CGRect)ec_nativeBounds {
    ...
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
        return CGRectMake(0, 0, [params[@"nw"] floatValue], [params[@"nh"] floatValue]);
    }
...
- (NSInteger)ec_maximumFramesPerSecond {
    ...
    NSDictionary *params = [self ec_getScreenParamsForMachine:machine];
    if (params) {
        return [params[@"fps"] integerValue];
    }
...
static int hooked_sysctlbyname(const char *name, void *oldp, size_t *oldlenp,
    ...
    if (result == 0 && name != NULL) {
      ...
      if (oldp != NULL && oldlenp != NULL) {
        ...
        if ((strcmp(name, "hw.cpufamily") == 0 || strcmp(name, "hw.cputype") == 0 || strcmp(name, "hw.cpusubtype") == 0) &&
            [config spoofBoolForKey:@"enableSysctlHardware" defaultValue:YES]) {
            NSString *machine = [config spoofValueForKey:@"machineModel"];
            uint32_t family = 0;
            if ([machine hasPrefix:@"iPhone9,"]) family = 0x892ce4ad; // A10
            else if ([machine hasPrefix:@"iPhone10,"]) family = 0xe81e7ef6; // A11
            else if ([machine hasPrefix:@"iPhone11,"]) family = 0x07d34b9f; // A12
            else if ([machine hasPrefix:@"iPhone12,"]) family = 0x462504d2; // A13
            else if ([machine hasPrefix:@"iPhone13,"]) family = 0x1b588bb3; // A14
            else if ([machine hasPrefix:@"iPhone14,"]) family = 0xda33d83d; // A15
            else if ([machine hasPrefix:@"iPhone15,2"] || [machine hasPrefix:@"iPhone15,3"]) family = 0xda33d83d; // A15 (iPhone 14)
            else if ([machine hasPrefix:@"iPhone15,"]) family = 0x8765d282; // A16
            else if ([machine hasPrefix:@"iPhone16,"]) family = 0x4523c932; // A17

            if (family != 0) {
                if (strcmp(name, "hw.cpufamily") == 0 && *oldlenp >= sizeof(uint32_t)) {
                    memcpy(oldp, &family, sizeof(uint32_t));
                    return result;
                } else if (strcmp(name, "hw.cputype") == 0 && *oldlenp >= sizeof(uint32_t)) {
                    uint32_t ctype = 0x0100000C; // CPU_TYPE_ARM64
                    memcpy(oldp, &ctype, sizeof(uint32_t));
                    return result;
                } else if (strcmp(name, "hw.cpusubtype") == 0 && *oldlenp >= sizeof(uint32_t)) {
                    uint32_t subtype = (family == 0x892ce4ad || family == 0xe81e7ef6) ? 0x0 : 0x2; // A12及以上为 arm64e (2)
...
static bool is_jailbreak_path(const char *path) {
    ...
    if (strstr(path, "/TrollStore.app") || strstr(path, "trollstorehelper") || strstr(path, "/var/jb")) {
        return true;
    }
    ...
    if (strcmp(path, "/bin/sh") == 0 || strcmp(path, "/bin/bash") == 0 || strcmp(path, "/bin/zsh") == 0) {
        return true;
    }
    // Cydia / Sileo / 越狱包管理器
    if (strstr(path, "/Applications/Cydia.app") || strstr(path, "/Applications/Sileo.app") ||
        strstr(path, "/Applications/Zebra.app") || strstr(path, "/usr/bin/cydia") ||
        strstr(path, "/usr/bin/sileo")) {
        return true;
    }
    ...
    if (strstr(path, "MobileSubstrate") || strstr(path, "CydiaSubstrate") ||
        strstr(path, "/usr/lib/libhooker") || strstr(path, "/usr/lib/substitute")) {
        return true;
    }
    // 其他常见越狱检测路径
    if (strstr(path, "/private/var/lib/apt") || strstr(path, "/private/var/stash") ||
        strstr(path, "/Library/MobileSubstrate") || strstr(path, "/var/cache/apt") ||
        strstr(path, "/var/lib/dpkg") || strstr(path, "/var/tmp/cydia.log") ||
        strstr(path, "/jb/") || strstr(path, "/.cydia_no_stash") ||
        strstr(path, "/checkra1n") || strstr(path, "/binpack") ||
        strstr(path, "/usr/lib/libjailbreak") || strstr(path, "/usr/share/jailbreak")) {
        return true;
    }
    return false;
}
...
static int hooked_stat(const char *restrict path, struct stat *restrict buf) {
    ...
    if (original_stat) return original_stat(path, buf);
    return -1;
}
...
- (NSURL *)ec_containerURLForSecurityApplicationGroupIdentifier:
    ...
    if (g_isCloneMode && cloneId && groupIdentifier) {
      ...
      NSString *cloneGroupPath = [realHomeDir
          stringByAppendingPathComponent:
              [NSString stringWithFormat:@"Documents/FakeAppGroup/%@/%@",
                                         cloneId, groupIdentifier]];
...
- (void)ec_setObject:(id)value forKey:(NSString *)key {
    ...
    // (MSSDK 会尝试写入 1180- 系列及全局配置，这会导致 Sandbox 拒绝，引发数百毫秒的死锁)
    if ([key isEqualToString:@"AppleLanguages"] ||
        ...
        [key hasPrefix:@"1180-"] || // 拦截全部 MSSDK 批量风控探针 Key (1180 是 TikTok 的 AID)
...
static OSStatus hooked_SecItemUpdate(CFDictionaryRef query, CFDictionaryRef attributesToUpdate) {
    if (g_isCloneMode) {
      ...
      if (savedItemImmutable) {
        ...
        ec_save_virtual_keychain();
        NSLog(@"[ECFix] 🛡️ 强制拦截 SecItemUpdate -> 更新虚拟 Keychain [%@]",
              key);
        return errSecSuccess;
      }
      return errSecItemNotFound;
        ...
        IMP newResume = imp_implementationWithBlock(^(id self_) {
          @try {
            ...
            if (reqObj) {
              ...
              // 因为上层 (TTNetworkManager 等) 已经基于原始 URL 计算了防篡改签名 (X-Gorgon, X-Argus 等)。
              // 在底层修改任何参数 (如 os_boot_time, ad_user_agent, mcc_mnc) 都会导致服务端签名校验失败，
              // 从而引发严重的风控拦截 (例如提示: "最长尝试次数已用完" Too Many Attempts)。
              // 所有防泄漏必须在更上层 (sysctl hook, NSUserDefaults, CTTelephonyNetworkInfo) 处理。
...
static CFPropertyListRef hooked_CFPreferencesCopyAppValue(CFStringRef key, CFStringRef applicationID) {
    ...
    if (key != NULL) {
        if (CFStringCompare(key, CFSTR("AppleLanguages"), 0) == kCFCompareEqualTo) {
            ...
            if (lang) return (__bridge_retained CFPropertyListRef)@[lang, @"en"];
        }
        if (CFStringCompare(key, CFSTR("AppleLocale"), 0) == kCFCompareEqualTo) {
            NSString *locale = [[SCPrefLoader shared] spoofValueForKey:@"localeIdentifier"];
            if (locale) return (__bridge_retained CFPropertyListRef)locale;
        }
    }
...
static void setupDataIsolationHooks(void) {
    ...
    ec_register_rebinding("CFPreferencesSetValue", (void *)hooked_CFPreferencesSetValue, (void **)&orig_CFPreferencesSetValue);
    ec_register_rebinding("CFPreferencesSetAppValue", (void *)hooked_CFPreferencesSetAppValue, (void **)&orig_CFPreferencesSetAppValue);
    ec_register_rebinding("CFPreferencesSetMultiple", (void *)hooked_CFPreferencesSetMultiple, (void **)&orig_CFPreferencesSetMultiple);
    ec_register_rebinding("CFPreferencesCopyAppValue", (void *)hooked_CFPreferencesCopyAppValue, (void **)&orig_CFPreferencesCopyAppValue);
    ec_register_rebinding("CFPreferencesCopyValue", (void *)hooked_CFPreferencesCopyValue, (void **)&orig_CFPreferencesCopyValue);
    ec_register_rebinding("CFPreferencesCopyMultiple", (void *)hooked_CFPreferencesCopyMultiple, (void **)&orig_CFPreferencesCopyMultiple);
    ec_register_rebinding("CFPreferencesSynchronize", (void *)hooked_CFPreferencesSynchronize, (void **)&orig_CFPreferencesSynchronize);
    ec_register_rebinding("CFPreferencesAppSynchronize", (void *)hooked_CFPreferencesAppSynchronize, (void **)&orig_CFPreferencesAppSynchronize);
    
    // CFLocale 语言与区域泄漏保护
    ec_register_rebinding("CFLocaleCopyPreferredLanguages", (void *)hooked_CFLocaleCopyPreferredLanguages, (void **)&original_CFLocaleCopyPreferredLanguages);
    ec_register_rebinding("CFLocaleCopyCurrent", (void *)hooked_CFLocaleCopyCurrent, (void **)&original_CFLocaleCopyCurrent);
    ...
    */
    ...
    if (sw && sh) {
      ...
      if (!widthMatch || !heightMatch || !scaleMatch) {
        ...
        ECLog(
            @"  ✅ 屏幕参数一致: 真实=%.0fx%.0f 伪装=%.0fx%.0f — 触摸坐标安全",
    ...
    ECLog(@"  [设备机型] 伪装为: %@  (注: 跨代伪装，例如用 A15 伪装 iPhone 7 易引发基准测试风控)", spoofModel ?: @"未配置");
    ...
    ECLog(@"  [区域语言] 伪装为: 国家(Country)=%@, 语言(Lang)=%@, 地区(Region)=%@", spoofCountry?:@"-", spoofLang?:@"-", spoofRegion?:@"-");
    ...
    ECLog(@"  [蜂窝网络] 伪装为: 运营商(Carrier)=%@, MCC=%@, MNC=%@", spoofCarrier?:@"-", spoofMcc?:@"-", spoofMnc?:@"-");
    }
...
#pragma mark - Anti-Debug Specific Hooks
    ...
    ec_register_rebinding("SecItemAdd", (void *)hooked_SecItemAdd, (void **)&original_SecItemAdd);
    ec_register_rebinding("SecItemCopyMatching", (void *)hooked_SecItemCopyMatching, (void **)&original_SecItemCopyMatching);
    ec_register_rebinding("SecItemUpdate", (void *)hooked_SecItemUpdate, (void **)&original_SecItemUpdate);
    ec_register_rebinding("SecItemDelete", (void *)hooked_SecItemDelete, (void **)&original_SecItemDelete);
    ...
    if (odrClass) {
      ...
      Method beginM = class_getInstanceMethod(odrClass, @selector(beginAccessingResourcesWithCompletionHandler:));
      if (beginM) {
        method_setImplementation(beginM, imp_implementationWithBlock(^(id _self, void (^completion)(NSError *)) {
          ECLog(@"🛡️ [ODR] 拦截 beginAccessingResources，静默返回错误");
          if (completion) {
            NSError *err = [NSError errorWithDomain:@"NSBundleResourceRequestErrorDomain"
                                              code:2  // NSBundleResourceRequestLowDiskSpace
                                          userInfo:@{NSLocalizedDescriptionKey: @"ODR disabled"}];
            completion(err);
          }
        }));
      ...
      Method condM = class_getInstanceMethod(odrClass, @selector(conditionallyBeginAccessingResourcesWithCompletionHandler:));
      if (condM) {
        method_setImplementation(condM, imp_implementationWithBlock(^(id _self, void (^completion)(BOOL)) {
...
#import <sys/mman.h>
    ...
    static const unsigned char _ek0[] = {0x1f, 0x19, 0x1e, 0x3f, 0x2c,
                                         0x33, 0x39, 0x3f, 0x09, 0x2a,
                                         0x35, 0x35, 0x3c};
...
static void fixAppStoreReceipt(void) {
    ...
    NSString *receiptDir =
        [tmpDir stringByAppendingPathComponent:@"_MASReceipt"];
    ...
    if (![fm fileExistsAtPath:receiptFile]) {
      ...
      const unsigned char fakeReceipt[] = {
          0x30, 0x80, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d,
          0x01, 0x07, 0x02, 0xa0, 0x80, 0x30, 0x80, 0x02, 0x01, 0x01,
          0x31, 0x00, 0x30, 0x80, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86,
          0xf7, 0x0d, 0x01, 0x07, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static void setupIMPSpoofing(void) {
    ...
    if (!orig_runtime_class_getMethodImpl) {
      ECLog(
          @"  ❌ Phase 9: dlsym 获取 class_getMethodImplementation 失败，跳过");
      return;
    }
                    ...
                    dispatch_after(
                        ...
                        dispatch_get_main_queue(), ^{
                          @try {
                            ...
                            // 完成注册
                            NSURL *probeURL = [NSURL
                                URLWithString:@"https://captive.apple.com/"
                                              @"hotspot-detect.html"];
...
// dylib 加载时自动执行
    ...
    @autoreleasepool {
      ...
      // 2.5 [风控关键修复] 在所有 Hook 生效前，立即覆写进程内 CFPreferences 缓存
      // 原理：CoreFoundation 在 dylib 加载前就已通过 XPC 从 cfprefsd 读取了真实的
      // AppleLanguages (zh-Hans-JP) 并缓存在进程内的 CFPrefsPlistSource 对象中。
      // 我们无法 Hook 框架内部的 XPC 读取，但可以在读取完成后直接覆写缓存值。
      // 这样后续系统框架内部调用 CFPreferencesSynchronize 时，写出去的就是伪装值。
      {
        SCPrefLoader *config = [SCPrefLoader shared];

...
- (NSArray *)allUserAppPaths {
  return allUserAppBundlePaths();
}
...
- (NSError *)errorForCode:(int)code {
  ...
  case 171:
    errorDescription =
        @"A non " APP_NAME @" or a " OTHER_APP_NAME
        @" app with the same identifier is already installed. If you are "
        @"absolutely sure it is not, you can force install it.";
    break;
  case 172:
  ...
  case 173:
    errorDescription =
        @"The app is not signed with a fake CoreTrust certificate and ldid is "
        @"not installed. Install ldid in the settings tab and try again.";
    break;
  case 174:
  ...
  case 179:
    errorDescription =
        @"The app you tried to install has the same identifier as a system app "
        @"already installed on the device. The installation has been prevented "
        @"to protect you from possible bootloops or other issues.";
    break;
  case 180:
    errorDescription =
        @"The app you tried to install has an encrypted main binary, which "
        @"cannot have the CoreTrust bypass applied to it. Please ensure you "
        @"install decrypted apps.";
    break;
  case 181:
  ...
  case 182:
    errorDescription = @"The app was installed successfully, but requires "
                       @"developer mode to be enabled to run. After rebooting, "
                       @"select \"Turn On\" to enable developer mode.";
    break;
  case 183:
  ...
  case 184:
    errorDescription =
        @"The app was installed successfully, but has additional binaries that "
        @"are encrypted (e.g. extensions, plugins). The app itself should "
        @"work, but you may experience broken functionality as a result.";
    break;
  case 185:
    errorDescription = @"Failed to sign the app. The CoreTrust bypass returned "
                       @"a non zero status code.";
  }

  NSError *error =
      [NSError errorWithDomain:TrollStoreErrorDomain
                          code:code
...
- (int)installIpa:(NSString *)pathToIpa
            force:(BOOL)force
              log:(NSString **)logOut {
  return [self installIpa:pathToIpa
                    force:force
...
- (int)installIpa:(NSString *)pathToIpa
  ...
  int installationMethodToUse = 1; // Default to Custom
  if (installationMethod >= 0) {
    installationMethodToUse = installationMethod;
  } else {
    NSNumber *installationMethodToUseNum =
        [trollStoreUserDefaults() objectForKey:@"installationMethod"];
    installationMethodToUse =
        installationMethodToUseNum ? installationMethodToUseNum.intValue : 1;
  }
  ...
  // Add skip signing flag for encrypted apps
  if (skipSigning) {
    [args addObject:@"--skip-signing"];
  }

  // Add registration type if specified
  if (registrationType && registrationType.length > 0) {
    [args addObject:[NSString stringWithFormat:@"--registration-type=%@",
                                               registrationType]];
  }

  // Add custom bundle id if specified (for clone install)
  if (customBundleId && customBundleId.length > 0) {
    [args addObject:[NSString stringWithFormat:@"--custom-bundle-id=%@",
                                               customBundleId]];
  }

  // Add custom display name if specified (for clone install)
  if (customDisplayName && customDisplayName.length > 0) {
    [args addObject:[NSString stringWithFormat:@"--custom-display-name=%@",
                                               customDisplayName]];
  }
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:@"ApplicationsChanged"
                    object:nil];
  return ret;
}
...
- (int)uninstallApp:(NSString *)appId {
  ...
  NSNumber *uninstallationMethodToUseNum =
      [trollStoreUserDefaults() objectForKey:@"uninstallationMethod"];
  int uninstallationMethodToUse =
      uninstallationMethodToUseNum ? uninstallationMethodToUseNum.intValue : 0;
...
- (BOOL)openApplicationWithBundleID:(NSString *)appId {
  return [[LSApplicationWorkspace defaultWorkspace]
      openApplicationWithBundleID:appId];
}
...
- (int)changeAppRegistration:(NSString *)appPath toState:(NSString *)newState {
  if (!appPath || !newState)
    return -200;
  return spawnRoot(rootHelperPath(),

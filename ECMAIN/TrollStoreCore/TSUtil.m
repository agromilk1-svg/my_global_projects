...
int64_t _CTServerConnectionSetCellularUsagePolicy(CFTypeRef *ct,
                                                  NSString *identifier,
...
void chineseWifiFixup(void) {
  _CTServerConnectionSetCellularUsagePolicy(
      ...
      NSBundle.mainBundle.bundleIdentifier, @{
        @"kCTCellularDataUsagePolicy" :
            @"kCTCellularDataUsagePolicyAlwaysAllow",
...
BOOL shouldRegisterAsUserByDefault(void) { return NO; }
...
void printMultilineNSString(NSString *stringToPrint) {
  ...
  NSArray *lines =
      [stringToPrint componentsSeparatedByCharactersInSet:separator];
...
void enumerateProcessesUsingBlock(void (^enumerator)(pid_t pid,
  ...
  for (int i = 0; i < count; i++) {
    @autoreleasepool {
      ...
      if (sysctl((int[]){CTL_KERN, KERN_PROCARGS2, pid}, 3, buffer, &size, NULL,
                 0) == 0) {
        NSString *executablePath =
            [NSString stringWithCString:(buffer + sizeof(int))
...
void killall(NSString *processName, BOOL softly) {
  enumerateProcessesUsingBlock(
      ^(pid_t pid, NSString *executablePath, BOOL *stop) {
...
void github_fetchLatestVersion(
  ...
  NSURLSessionDataTask *task = [NSURLSession.sharedSession
        dataTaskWithURL:githubLatestAPIURL
...
void fetchLatestTrollStoreVersion(
    void (^completionHandler)(NSString *latestVersion)) {
  github_fetchLatestVersion(@"opa334/TrollStore", completionHandler);
}

void fetchLatestLdidVersion(
    void (^completionHandler)(NSString *latestVersion)) {
...
NSArray *trollStoreInstalledAppContainerPathsInternal(NSString *marker) {
  ...
  NSError *error;
  NSArray *containers = [[NSFileManager defaultManager]
      contentsOfDirectoryAtPath:appContainersPath
                          error:&error];
  if (error) {
  ...
  for (NSString *container in containers) {
    NSString *containerPath =
        [appContainersPath stringByAppendingPathComponent:container];
    BOOL isDirectory = NO;
    BOOL exists =
      ...
      if (marker) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:trollStoreMark]) {
          NSString *trollStoreApp =
              [containerPath stringByAppendingPathComponent:@"ECMAIN.app"];
          NSString *trollStoreLiteApp = [containerPath
              stringByAppendingPathComponent:@"ECMAINLite.app"];
          if (![[NSFileManager defaultManager]
                  fileExistsAtPath:trollStoreApp] &&
...
NSArray *trollStoreInstalledAppBundlePathsInternal(NSString *marker) {
  ...
  for (NSString *containerPath in trollStoreInstalledAppContainerPathsInternal(
           marker)) {
    NSArray *items =
    ...
    for (NSString *item in items) {
      if ([item.pathExtension isEqualToString:@"app"]) {
        [appPaths
            addObject:[containerPath stringByAppendingPathComponent:item]];
      }
    }
  }
...
LSApplicationProxy *
findPersistenceHelperApp(PERSISTENCE_HELPER_TYPE allowedTypes) {
  __block LSApplicationProxy *outProxy;

  void (^searchBlock)(LSApplicationProxy *appProxy) =
...
SecStaticCodeRef getStaticCodeRef(NSString *binaryPath) {
  ...
  CFURLRef binaryURL = CFURLCreateWithFileSystemPath(
      kCFAllocatorDefault, (__bridge CFStringRef)binaryPath,
  ...
  if (result != errSecSuccess) {
    NSLog(@"[getStaticCodeRef] failed to create static code for binary %@",
          binaryPath);
    return NULL;
  }
...
NSDictionary *dumpEntitlements(SecStaticCodeRef codeRef) {
  if (codeRef == NULL) {
    NSLog(@"[dumpEntitlements] attempting to dump entitlements without a "
          @"StaticCodeRef");
    return nil;
  }
  ...
  result = SecCodeCopySigningInformation(codeRef, kSecCSRequirementInformation,
                                         &signingInfo);
  ...
  CFDictionaryRef entitlements =
      CFDictionaryGetValue(signingInfo, kSecCodeInfoEntitlementsDict);
...
NSDictionary *dumpEntitlementsFromBinaryAtPath(NSString *binaryPath) {
  // This function is intended for one-shot checks. Main-event functions should
  // retain/release their own SecStaticCodeRefs
...
EXPLOIT_TYPE getDeclaredExploitTypeFromInfoDictionary(NSDictionary *infoDict) {
  ...
  if ([tsPreAppliedExploitType isKindOfClass:[NSNumber class]]) {
    ...
    if (exploitTypeInt > 0) {
      ...
      NSLog(@"[getDeclaredExploitTypeFromInfoDictionary] rejecting "
            @"TSPreAppliedExploitType Info.plist value (%i) which is out of "
            @"range",
            exploitTypeInt);
    }
  }

  // Legacy Info.plist flag - now deprecated, but we treat it as a custom root
  // cert if present
...
void determinePlatformVulnerableExploitTypes(void *context) {
  ...
  } else if (strncmp(os_build, "21A326", 6) >= 0 &&
             strncmp(os_build, "21A331", 6) <= 0) {
    // iOS 17.0 final
    gPlatformVulnerabilities = EXPLOIT_TYPE_CMS_SIGNERINFO_V1;
  } else if (strncmp(os_build, "21A5248v", 8) >= 0 &&
             strncmp(os_build, "21A5326a", 8) <= 0) {
    // iOS 17.0 beta 1 - 8
    gPlatformVulnerabilities = EXPLOIT_TYPE_CMS_SIGNERINFO_V1;
  } else if (strncmp(os_build, "19G5027e", 8) >= 0 &&
             strncmp(os_build, "19G5063a", 8) <= 0) {
    // iOS 15.6 beta 1 - 5
    gPlatformVulnerabilities = (EXPLOIT_TYPE_CUSTOM_ROOT_CERTIFICATE_V1 |
                                EXPLOIT_TYPE_CMS_SIGNERINFO_V1);

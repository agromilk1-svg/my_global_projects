...
extern CGImageRef LICreateIconForImage(CGImageRef image, int variant,
                                       int precomposed);
...
- (void)enumerateArchive:(void (^)(struct archive_entry *entry,
                                   BOOL *stop))enumerateBlock {
  [self openArchive];
    ...
    if (_isArchive) {
      [self enumerateArchive:^(struct archive_entry *entry, BOOL *stop) {
        ...
        if (currentSubpath.pathComponents.count == 3) {
          if ([currentSubpath.pathComponents[0] isEqualToString:@"Payload"] &&
              [currentSubpath.pathComponents[1].pathExtension
                  isEqualToString:@"app"]) {
      ...
      if (!_cachedAppBundleName) {
        ...
        outError = [NSError
            errorWithDomain:TrollStoreErrorDomain
                       code:301
...
- (NSError *)loadInfoDictionary {
  if (_isArchive && _cachedAppBundleName) {
    NSString *mainInfoPlistPath = [NSString
        stringWithFormat:@"Payload/%@/Info.plist", _cachedAppBundleName];
    struct archive_entry *infoDictEntry =
        [self archiveEntryForSubpath:mainInfoPlistPath];
    if (infoDictEntry) {
      ...
      if (read == size) {
        ...
        _cachedInfoDictionary = [NSPropertyListSerialization
            propertyListWithData:infoPlistData
    ...
    __block NSMutableDictionary *pluginInfoDictionaries =
        [NSMutableDictionary new];

    [self enumerateArchive:^(struct archive_entry *entry, BOOL *stop) {
      NSString *currentSubpath =
          [NSString stringWithUTF8String:archive_entry_pathname(entry)];
      if ([currentSubpath isEqualToString:mainInfoPlistPath])
        return;

      if ([currentSubpath.lastPathComponent isEqualToString:@"Info.plist"] &&
          currentSubpath.pathComponents.count == 5) {
        if ([currentSubpath.pathComponents[2] isEqualToString:@"PlugIns"]) {
          ...
          if (read == size) {
            NSData *infoPlistData = [NSData dataWithBytes:buf length:size];
            NSDictionary *pluginPlist = [NSPropertyListSerialization
                propertyListWithData:infoPlistData
                             options:NSPropertyListImmutable
                              format:nil
                               error:nil];
            pluginInfoDictionaries[currentSubpath
    ...
    NSString *mainInfoPlistPath =
        [_path stringByAppendingPathComponent:@"Info.plist"];
    if ([[NSFileManager defaultManager] fileExistsAtPath:mainInfoPlistPath]) {
      _cachedInfoDictionary = [NSDictionary
          dictionaryWithContentsOfURL:[NSURL fileURLWithPath:mainInfoPlistPath]
                                error:nil];
    }
    ...
    NSArray *plugIns = [[NSFileManager defaultManager]
        contentsOfDirectoryAtPath:[_path
                                      stringByAppendingPathComponent:@"PlugIns"]
                            error:nil];
    for (NSString *plugIn in plugIns) {
      NSString *pluginSubpath =
          [NSString stringWithFormat:@"PlugIns/%@", plugIn];
      NSString *pluginInfoDictionaryPath =
          [[_path stringByAppendingPathComponent:pluginSubpath]
              stringByAppendingPathComponent:@"Info.plist"];
      NSDictionary *pluginInfoDictionary =
          [NSDictionary dictionaryWithContentsOfURL:
                            [NSURL fileURLWithPath:pluginInfoDictionaryPath]
                                              error:nil];
  ...
  if (!_cachedInfoDictionary) {
    NSString *errorDescription =
        @"Unable to locate Info.plist inside app bundle.";
    return [NSError
        errorWithDomain:TrollStoreErrorDomain
                   code:302
               userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
...
- (NSError *)loadInstalledState {
  if (!_isArchive) {
    ...
    LSApplicationProxy *appProxy =
        [LSApplicationProxy applicationProxyForBundleURL:bundleURL];
    if (appProxy) {
      if (appProxy && appProxy.isInstalled) {
        _cachedRegistrationState = appProxy.applicationType;
        _cachedDataContainerURL = appProxy.dataContainerURL;
      }
    }
  }
  return nil;
...
- (NSError *)loadEntitlements {
  if (!_cachedEntitlementsByBinarySubpaths) {
    NSMutableDictionary *entitlementsByBinarySubpaths =
        [NSMutableDictionary new];

    if (_isArchive) {
      if (_cachedInfoDictionary) {
        NSString *bundleExecutable =
            _cachedInfoDictionary[@"CFBundleExecutable"];
        NSString *bundleExecutableSubpath =
            [NSString stringWithFormat:@"Payload/%@/%@", _cachedAppBundleName,
                                       bundleExecutable];
        struct archive_entry *mainBinaryEntry =
            [self archiveEntryForSubpath:bundleExecutableSubpath];
        if (!mainBinaryEntry) {
          NSString *errorDescription =
              @"Unable to locate main binary inside app bundle.";
          return [NSError
              errorWithDomain:TrollStoreErrorDomain
                         code:303
                     userInfo:@{NSLocalizedDescriptionKey : errorDescription}];
      ...
      [_cachedInfoDictionariesByPluginSubpaths
          enumerateKeysAndObjectsUsingBlock:^(NSString *pluginSubpath,
                                              NSDictionary *infoDictionary,
                                              BOOL *_Nonnull stop) {
            NSString *pluginExecutable = infoDictionary[@"CFBundleExecutable"];
            NSString *pluginExecutableSubpath = [NSString
                stringWithFormat:@"%@/%@", pluginSubpath, pluginExecutable];
            struct archive_entry *pluginBinaryEntry =
                [self archiveEntryForSubpath:pluginExecutableSubpath];
            if (!pluginBinaryEntry)
              return;
            ...
            if (read == size) {
              NSData *binaryData = [NSData dataWithBytes:buf length:size];
              entitlementsByBinarySubpaths[pluginExecutableSubpath] =
                  dumpEntitlementsFromBinaryData(binaryData);
            }
            free(buf);
          }];
      ...
      if (_cachedInfoDictionary) {
        ...
        NSString *bundleExecutablePath =
            [_path stringByAppendingPathComponent:bundleExecutable];

        if (![[NSFileManager defaultManager]
                fileExistsAtPath:bundleExecutablePath]) {
          NSString *errorDescription =
              @"Unable to locate main binary inside app bundle.";
          return [NSError
              errorWithDomain:TrollStoreErrorDomain
        ...
        entitlementsByBinarySubpaths[bundleExecutable] =
            dumpEntitlementsFromBinaryAtPath(bundleExecutablePath);
      }
      ...
      [_cachedInfoDictionariesByPluginSubpaths
          enumerateKeysAndObjectsUsingBlock:^(NSString *pluginSubpath,
            ...
            NSString *pluginExecutablePath =
                [_path stringByAppendingPathComponent:pluginExecutableSubpath];
            entitlementsByBinarySubpaths[pluginExecutableSubpath] =
                dumpEntitlementsFromBinaryAtPath(pluginExecutablePath);
          }];
    }
...
- (NSError *)loadSize {
  ...
  if (_isArchive) {
    ...
    NSDirectoryEnumerator *enumerator = [[NSFileManager defaultManager]
                   enumeratorAtURL:[NSURL fileURLWithPath:_path]
        includingPropertiesForKeys:@[
          NSURLIsRegularFileKey, NSURLFileAllocatedSizeKey,
    ...
    for (NSURL *itemURL in enumerator) {
      ...
      NSError *error;
      [itemURL getResourceValue:&isRegularFile
                         forKey:NSURLIsRegularFileKey
                          error:&error];

      if (isRegularFile.boolValue) {
        NSNumber *totalFileAllocatedSize;
        [itemURL getResourceValue:&totalFileAllocatedSize
        ...
        if (totalFileAllocatedSize) {
          ...
          [itemURL getResourceValue:&fileAllocatedSize
...
- (NSError *)loadPreviewIcon {
  ...
  CGImageRef liIcon = LICreateIconForImage(
      [[self iconForSize:CGSizeMake(29, 29)] CGImage], imageVariant, 0);
  _cachedPreviewIcon = [[UIImage alloc] initWithCGImage:liIcon
                                                  scale:screenScale
                                            orientation:0];
  ;
  return nil;
...
- (int)openArchive {
  ...
  int r = archive_read_open_filename(_archive, _path.fileSystemRepresentation,
                                     10240);
  return r ? r : 0;
}
...
- (void)loadBasicInfoWithCompletion:(void (^)(NSError *))completionBlock {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
                 ^{
                   if (completionBlock)
                     completionBlock([self sync_loadBasicInfo]);
                 });
}
...
- (void)loadInfoWithCompletion:(void (^)(NSError *))completionBlock {
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
                 ^{
                   if (completionBlock)
                     completionBlock([self sync_loadInfo]);
                 });
}

- (void)enumerateAllInfoDictionaries:(void (^)(NSString *key, NSObject *value,
                                               BOOL *stop))enumerateBlock {
  if (!enumerateBlock)
    return;
  ...
  [_cachedInfoDictionary enumerateKeysAndObjectsUsingBlock:^(
                             NSString *key, NSObject *value, BOOL *stop) {
        ...
        if ([pluginInfoDictionary isKindOfClass:NSDictionary.class]) {
          [pluginInfoDictionary
              enumerateKeysAndObjectsUsingBlock:^(
                  NSString *key, NSObject *value, BOOL *_Nonnull stop_2) {
                enumerateBlock(key, value, &b_stop);
                if (b_stop) {
                  *stop_1 = YES;
                  *stop_2 = YES;
...
- (void)enumerateAllEntitlements:(void (^)(NSString *key, NSObject *value,
                                           BOOL *stop))enumerateBlock {
  if (!enumerateBlock)
    return;
  ...
  [_cachedEntitlementsByBinarySubpaths
      enumerateKeysAndObjectsUsingBlock:^(NSString *binarySubpath,
                                          NSDictionary *binaryInfoDictionary,
                                          BOOL *stop_1) {
        if ([binaryInfoDictionary isKindOfClass:NSDictionary.class]) {
          [binaryInfoDictionary
              enumerateKeysAndObjectsUsingBlock:^(
                  NSString *key, NSObject *value, BOOL *_Nonnull stop_2) {
                enumerateBlock(key, value, &b_stop);
                if (b_stop) {
...
- (void)enumerateAvailableIcons:(void (^)(CGSize iconSize, NSUInteger iconScale,
  ...
  if (_cachedInfoDictionary) {
    ...
    void (^wrapperBlock)(NSString *iconPath, BOOL *stop) = ^(NSString *iconPath,
                                                             BOOL *stop) {
      NSString *currentIconName = iconPath.lastPathComponent;
      NSString *iconSuffix =
          [currentIconName substringFromIndex:[iconName length]];
      NSArray *seperatedIconSuffix =
          [iconSuffix componentsSeparatedByString:@"@"];
      ...
      NSArray *separatedIconSize =
          [currentIconResolution componentsSeparatedByString:@"x"];
      ...
      CGSize iconSize = CGSizeMake(widthNum.unsignedIntegerValue,
                                   heightNum.unsignedIntegerValue);
    ...
    if (_isArchive) {
      NSString *iconPrefix = [NSString
          stringWithFormat:@"Payload/%@/%@", _cachedAppBundleName, iconName];
      [self enumerateArchive:^(struct archive_entry *entry, BOOL *stop) {
        NSString *currentSubpath =
            [NSString stringWithUTF8String:archive_entry_pathname(entry)];
...
- (UIImage *)iconForSize:(CGSize)size {
  ...
  [self enumerateAvailableIcons:^(CGSize iconSize, NSUInteger iconScale,
                                  NSString *iconPath, BOOL *stop) {
    if (CGSizeEqualToSize(iconSize, size) &&
        UIScreen.mainScreen.scale == iconScale) {
      foundIconPath = iconPath;
      // imageToReturn = imageWithSize([UIImage
      // imageWithContentsOfFile:iconPath], size);
      *stop = YES;
    }
  }];
  ...
  if (!foundIconPath) {
    ...
    [self enumerateAvailableIcons:^(CGSize iconSize, NSUInteger iconScale,
      ...
      if (iconSize.width > size.width &&
          iconSize.width < closestIconSize.width) {
  ...
  if (_isArchive) {
    ...
    struct archive_entry *iconEntry =
        [self archiveEntryForSubpath:foundIconPath];
    if (iconEntry) {
...
- (NSString *)sizeString {
  return [NSByteCountFormatter
      stringFromByteCount:_cachedSize
...
- (NSString *)executablePath {
  NSString *executableName = _cachedInfoDictionary[@"CFBundleExecutable"];
  if (!executableName) {
    if (_isArchive) {
      executableName = [_cachedAppBundleName stringByDeletingPathExtension];
    } else {
      executableName =
          [[_path lastPathComponent] stringByDeletingPathExtension];
    }
  ...
  if (_isArchive) {
    return [NSString stringWithFormat:@"Payload/%@/%@", _cachedAppBundleName,
                                      executableName];
  } else {
...
- (NSURL *)dataContainerURL {
  return _cachedDataContainerURL;
}
...
- (NSAttributedString *)detailedInfoTitle {
  ...
  NSMutableDictionary *titleAttributes =
      @{NSFontAttributeName : [UIFont boldSystemFontOfSize:16]}.mutableCopy;
  ...
  if (_cachedPreviewIcon) {
    ...
    [description appendAttributedString:
                     [NSAttributedString
                         attributedStringWithAttachment:previewAttachment]];
...
- (NSAttributedString *)detailedInfoDescription {
  ...
  [self enumerateAllEntitlements:^(NSString *key, NSObject *value, BOOL *stop) {
    if ([key
            isEqualToString:@"com.apple.private.security.container-required"]) {
  ...
  __block NSMutableArray *accessibleContainers =
      [NSMutableArray new]; // array by design, should be ordered
  if (!unrestrictedContainerAccess) {
    __block NSString *dataContainer = nil;

    // If com.apple.private.security.container-required Entitlement is a string,
    // prefer it to CFBundleIdentifier
    [self
        enumerateAllEntitlements:^(NSString *key, NSObject *value, BOOL *stop) {
          if ([key isEqualToString:
    ...
    if (!dataContainer) {
      [self enumerateAllInfoDictionaries:^(NSString *key, NSObject *value,
                                           BOOL *stop) {
    ...
    [self
        enumerateAllEntitlements:^(NSString *key, NSObject *value, BOOL *stop) {
          if ([key isEqualToString:
              ...
              [key isEqualToString:@"com.apple.security.system-groups"]) {
          ...
          if ([serviceID isKindOfClass:NSString.class]) {
            ...
            if (displayName == nil) {
              [allowedTccServices
                  addObject:
                      [serviceID
                          stringByReplacingOccurrencesOfString:@"kTCCService"
                                                    withString:@""]];
            } else {
  ...
  NSMutableParagraphStyle *leftAlignment =
      [[NSMutableParagraphStyle alloc] init];
  leftAlignment.alignment = NSTextAlignmentLeft;

  UIColor *dangerColor = [UIColor
  ...
  if (isUnsandboxed) {
    [description appendAttributedString:
                     [[NSAttributedString alloc]
                         initWithString:@"\nThe app runs unsandboxed and can "
                                        @"access most of the file system."
                             attributes:bodyWarningAttributes]];
  } else {
    [description
        appendAttributedString:
            [[NSAttributedString alloc]
                initWithString:@"\nThe app runs sandboxed and can only access "
  ...
  if (isPlatformApplication && isUnsandboxed && hasPersonaMngmt) {
    [description appendAttributedString:
                     [[NSAttributedString alloc]
                         initWithString:@"\nThe app can spawn its own embedded "
                                        @"binaries with root privileges."
                             attributes:bodyDangerAttributes]];
  } else if (isPlatformApplication && isUnsandboxed) {
    [description appendAttributedString:
                     [[NSAttributedString alloc]
                         initWithString:@"\nThe app can spawn arbitrary "
                                        @"binaries as the mobile user."
                             attributes:bodyWarningAttributes]];
  } else {
    [description
        appendAttributedString:
  ...
  if (allowedTccServices.count) {
    ...
    [description
        appendAttributedString:
            [[NSAttributedString alloc]
                initWithString:@"\nThe app can access the following services "
                               @"without asking for permission:\n"
                    attributes:bodyWarningAttributes]];
    [description appendAttributedString:
                     [[NSAttributedString alloc]
                         initWithString:[NSListFormatter
                                            localizedStringByJoiningStrings:
                                                [allowedTccServices allObjects]]
                             attributes:bodyAttributes]];
  }
  ...
  if (allowedMGKeys.count) {
    ...
    [description appendAttributedString:
                     [[NSAttributedString alloc]
                         initWithString:@"\nThe app can access protected "
                                        @"information about this device:\n"
                             attributes:bodyWarningAttributes]];
    [description
        appendAttributedString:
            [[NSAttributedString alloc]
                initWithString:[NSListFormatter localizedStringByJoiningStrings:
                                                    [allowedMGKeys allObjects]]
                    attributes:bodyAttributes]];
  }
  ...
  if (unrestrictedContainerAccess || accessibleContainers.count) {
    ...
    if (unrestrictedContainerAccess) {
      [description appendAttributedString:
                       [[NSAttributedString alloc]
                           initWithString:@"\nUnrestricted, the app can access "
                                          @"all data containers on the system."
                               attributes:bodyDangerAttributes]];
    } else {
...
- (NSDictionary *)entitlements {
  if (_cachedEntitlementsByBinarySubpaths) {
    if (_isArchive) {
      // ... (Archive logic logic if needed, but simplified for installed apps)
    }
  }

  // For installed apps, parse directly
  if (!_isArchive && _path) {
    NSString *executablePath = [self executablePath];
    if (executablePath) {
      return dumpEntitlementsFromBinaryAtPath(executablePath);
    }
  }

  // Reuse existing logic if available
  if (_cachedEntitlementsByBinarySubpaths.count > 0) {
    // Return entitlements for the main binary
    // This part might need robust logic to find main binary key
    return _cachedEntitlementsByBinarySubpaths.allValues.firstObject;
  }

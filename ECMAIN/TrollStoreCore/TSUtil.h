...
extern void fetchLatestTrollStoreVersion(
...
@property(setter=_setAttributedTitle:, getter=_attributedTitle, nonatomic, copy)
    NSAttributedString *attributedTitle;
@property(setter=_setAttributedMessage:, getter=_attributedMessage, nonatomic,
          copy) NSAttributedString *attributedMessage;
...
typedef enum {
  ...
  PERSISTENCE_HELPER_TYPE_ALL =
      PERSISTENCE_HELPER_TYPE_USER | PERSISTENCE_HELPER_TYPE_SYSTEM
} PERSISTENCE_HELPER_TYPE;

// EXPLOIT_TYPE is defined as a bitmask as some devices are vulnerable to
// multiple exploits
//
// An app that has had one of these exploits applied ahead of time can declare
// which exploit was used via the TSPreAppliedExploitType Info.plist key. The
// corresponding value should be (number of bits to left-shift + 1).
typedef enum {
  // CVE-2022-26766
...
extern LSApplicationProxy *
findPersistenceHelperApp(PERSISTENCE_HELPER_TYPE allowedTypes);

typedef struct __SecCode const *SecStaticCodeRef;

typedef CF_OPTIONS(uint32_t, SecCSFlags) { kSecCSDefaultFlags = 0 };
...
CFDataRef SecCertificateCopyExtensionValue(SecCertificateRef certificate,
                                           ...
                                           bool *isCritical);
void SecPolicySetOptionsValue(SecPolicyRef policy, CFStringRef key,
                              CFTypeRef value);
...
extern EXPLOIT_TYPE
getDeclaredExploitTypeFromInfoDictionary(NSDictionary *infoDict);
